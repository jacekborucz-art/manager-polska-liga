...
const hasScored = matchState.homeGoals.some(g => g.playerName === p.lastName && !g.isMiss);
 ...
 {hasScored && (
              <div className="absolute -top-2 -left-2 w-4 h-4 bg-white rounded-full flex items-center justify-center text-[8px] shadow-lg z-30">
                ⚽
              </div>
            )}
...
{TacticRepository.getById(matchState.awayLineup.tacticId).slots.map((slot, i) => {
 ...
 {hasScored && (
          <div className="absolute -top-2 -right-2 w-4 h-4 bg-white rounded-full flex items-center justify-center text-[8px] shadow-lg z-30">
            ⚽
          </div>
        )}
