...
export const CONFSFDrawView: React.FC = () => {
  const { clubs, fixtures, confirmCONFSFDraw, userTeamId, sessionSeed } = useGame();

  const getClub = (id: string) => clubs.find(c => c.id === id);

  const qfWinners = useMemo(() => CONFDrawService.getQFWinners(fixtures), [fixtures]);

  const pairs = useMemo(() => {
    if (qfWinners.length < 2) return [];
    const shuffled = [...qfWinners];
    let s = sessionSeed ^ 0xfedcba98;
        ...
        <div className={GLASS_CARD + " flex-1 overflow-y-auto p-6"}>
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            <p className="text-[9px] font-black uppercase tracking-[0.4em] text-slate-500 mb-5 text-center">
              2 pary · 1. mecz: 28 marca · Rewanż: 17 kwietnia
            </p>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 flex-1 justify-end">
                      {isUserIn && pair.teamA === userTeamId && (
                        <span className="text-emerald-400 text-[9px] font-black uppercase tracking-[0.3em]">TY</span>
                      )}
