...
export const CONFR16DrawView: React.FC = () => {
  const { clubs, fixtures, confGroups, confirmCONFR16Draw, userTeamId } = useGame();

  const getClub = (id: string) => clubs.find(c => c.id === id);

  const pairs = useMemo(() => {
    if (!confGroups) return [];
    return CONFDrawService.computeCONFR16Pairs(confGroups, fixtures);
  }, [confGroups, fixtures]);

  if (!confGroups) {
    return (
      <div className="h-screen w-full flex items-center justify-center bg-slate-950 text-white">
        <p className="text-slate-500 text-sm">Faza grupowa LK nie została jeszcze zakończona.</p>
      </div>
    );
  }
      ...
      <div className="relative z-10 flex flex-col h-full p-6 gap-4">
        ...
        <div className={GLASS_CARD + " p-4 shrink-0"}>
          ...
          <div className="relative z-10 flex flex-wrap gap-2 justify-center">
            {PAIR_LABELS.map((m, i) => (
              <span key={i} className="text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-300">
                1{m.w} – 2{m.r}
              </span>
            ))}
          </div>
        ...
        <div className={GLASS_CARD + " flex-1 overflow-y-auto p-6"}>
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            <p className="text-[9px] font-black uppercase tracking-[0.4em] text-slate-500 mb-5 text-center">
              8 par · 1. mecz: 21 stycznia · Rewanż: 29 stycznia
            </p>
