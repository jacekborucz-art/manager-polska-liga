...
function maybeDistort(errorRate: number): boolean {
...
function analyzeOpponentTactic(
  ...
  const unavailable = opponentPlayers.filter(p =>
    p.health.status !== HealthStatus.HEALTHY || p.suspensionMatches > 0
  );
...
const TACTIC_MAP: Record<'ofensywnie' | 'neutralnie' | 'defensywnie', string[]> = {
...
function suggestRotation(
  ...
  const swaps: SwapSuggestion[] = rest.map((dropPlayer, i) => {
...
function generateWrittenReport(params: {
  ...
  const xi = opponentLineup.startingXI.map(id => opponentPlayers.find(p => p.id === id)).filter(Boolean) as Player[];
  ...
  const userXiLocal = userLineup.startingXI.map(id => userPlayers.find(p => p.id === id)).filter(Boolean) as Player[];
  ...
  const ATTRS = ['strength','stamina','pace','defending','passing','attacking','finishing','technique','vision','dribbling','heading','positioning'] as const;
  type AttrKey = typeof ATTRS[number];
  const ATTR_PL: Record<string, string> = {
    strength:'Siła', stamina:'Wytrzymałość', pace:'Szybkość', defending:'Obrona',
    passing:'Podanie', attacking:'Atak', finishing:'Wykończenie', technique:'Technika',
    vision:'Wizja', dribbling:'Drybling', heading:'Górna piłka', positioning:'Ustawienie',
  };
  // Podział na sektory na bazie atrybutów
  const isGK  = (p: Player) => p.attributes.goalkeeping >= 60;
  const isDef = (p: Player) => !isGK(p) && p.attributes.defending >= p.attributes.finishing && p.attributes.defending >= p.attributes.attacking;
  const isMid = (p: Player) => !isGK(p) && !isDef(p) && (p.attributes.vision + p.attributes.passing) >= (p.attributes.finishing + p.attributes.attacking);
  const isAtt = (p: Player) => !isGK(p) && !isDef(p) && !isMid(p);
  // Fallback: thirds wg OVR gdy sektor pusty
  const splitThirds = (players: Player[]): [Player[], Player[], Player[]] => {
    const nk = [...players].filter(p => !isGK(p)).sort((a, b) => a.overallRating - b.overallRating);
    const n = nk.length; const t1 = Math.floor(n / 3); const t2 = Math.floor(2 * n / 3);
    return [nk.slice(0, t1), nk.slice(t1, t2), nk.slice(t2)];
  };
  let oppDef  = xi.filter(isDef);     let oppMid  = xi.filter(isMid);     let oppAtt  = xi.filter(isAtt);
  let userDef = userXi.filter(isDef); let userMid = userXi.filter(isMid); let userAtt = userXi.filter(isAtt);
  if (oppDef.length === 0 || oppMid.length === 0 || oppAtt.length === 0)   [oppDef, oppMid, oppAtt]   = splitThirds(xi);
  if (userDef.length === 0 || userMid.length === 0 || userAtt.length === 0) [userDef, userMid, userAtt] = splitThirds(userXi);

  // Funkcja: dla danej grupy zawodników sumuje każdy atrybut, zwraca Record<attr, suma>
  const sumAttrs = (players: Player[]): Record<string, number> => {
    const out: Record<string, number> = {};
    for (const attr of ATTRS) out[attr] = players.reduce((s, p) => s + (p.attributes[attr] ?? 0), 0);
    return out;
  };
  // Funkcja: porównuje dwa RecordAttr, zwraca { usrPts, oppPts, details }
  const compareAttrs = (usr: Record<string, number>, opp: Record<string, number>, applyFuzz = false) => {
    let usrPts = 0; let oppPts = 0;
    const wins: string[] = []; const losses: string[] = [];
    for (const attr of ATTRS) {
      const u = usr[attr]; const o = applyFuzz && err() ? opp[attr] + Math.floor(Math.random() * 4 - 2) : opp[attr];
      if (u > o) { usrPts++; wins.push(ATTR_PL[attr]); }
      else if (o > u) { oppPts++; losses.push(ATTR_PL[attr]); }
    }
    return { usrPts, oppPts, wins, losses };
  };

  const sectorWins: number[] = []; // 1 = my wygrali, 0 = oni
  ...
  type TacRec = { id: string; rationale: string; style: 'atak' | 'kontra' | 'balans' | 'obrona' };
  let tacRec: TacRec;

  if (totalW === 3) {
    const choices: TacRec[] = [
  ...
  const styName: Record<string, string> = { atak: '⚔️ Ofensywna', kontra: '🔄 Kontratak', balans: '⚖️ Zrównoważona', obrona: '🛡️ Defensywna' };

  const altTactics = allTactics
    .filter(t => t.id !== tacRec.id)
    .filter(t => {
      if (tacRec.style === 'atak') return t.attackBias >= 70;
      if (tacRec.style === 'obrona') return t.defenseBias >= 80;
      if (tacRec.style === 'kontra') return t.defenseBias >= 70 && t.attackBias <= 55;
...
function buildMailBody(params: {
  ...
  // ── GLASS CARD ────────────────────────────────────────────────────────────
  const card = (accent: string, labelText: string, content: string, extra = '') =>
    `<div style="${cardBg};border-radius:24px;padding:30px 34px;flex:1;min-width:0;position:relative;overflow:hidden;${extra}">
  ...
  // ── STAT ROW ──────────────────────────────────────────────────────────────
  const statRow = (lbl: string, val: string, vc = '#f8fafc') =>
  ...
  // ── MINI CHIP ─────────────────────────────────────────────────────────────
  const chip = (t: string, bg: string, c: string) =>
    `<span style="font-family:${FONT};background:${bg};color:${c};font-size:15px;font-weight:900;font-style:italic;text-transform:uppercase;letter-spacing:0.06em;padding:6px 16px;border-radius:20px;display:inline-block;margin:3px 4px 3px 0;border:1px solid ${c}22;">${t}</span>`;

  // ── SVG DONUT PIE ─────────────────────────────────────────────────────────
  const pieSvg = (w: number, d: number, l: number): string => {
    const total = w + d + l;
    if (total === 0) return `<svg width="120" height="120" viewBox="0 0 120 120"><circle cx="60" cy="60" r="38" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="22"/><text x="60" y="65" text-anchor="middle" fill="#334155" font-family="Inter,sans-serif" font-size="10" font-weight="700" font-style="italic">—</text></svg>`;
    const R = 38; const cx = 60; const cy = 60;
    const arc = (sa: number, ea: number, color: string) => {
      const gap = 0.06; const s = sa + gap; const e = ea - gap;
      if (e <= s) return '';
  ...
  const formaCard = card('#f59e0b', 'FORMA - OSTATNIE 5 MECZÓW',
    ...
    'align-self:start;flex:none;width:320px;height:370px;padding:16px 20px;overflow:hidden;box-sizing:border-box;'
  );

  // ── TAKTYKA WIZUALNA — replika MatchLiveViewPolishCupSimulation.tsx ─────
  const renderFormation = (tacticId: string, kitPrimary: string, kitSecondary: string): string => {
    ...
    const jersey = (primary: string, secondary: string, size: number = 20): string =>
      `<svg viewBox="0 0 24 24" width="${size}" height="${size}" fill="${primary}" style="display:block;filter:drop-shadow(0 1px 3px rgba(0,0,0,0.7));">
    ...
    const LW  = '2px';
    const ld  = (s: string) => `<div style="position:absolute;${s}"></div>`;
    const linesHtml = [
  ...
  const tacticVisualCard = card('#a78bfa', 'TAKTYKA PRZECIWNIKA',
  ...
  const playersCard = card('#ef4444', '🎯 Kluczowi Zawodnicy',
    (strong ? `<div style="background:rgba(239,68,68,0.07);border:1px solid rgba(239,68,68,0.15);border-radius:16px;padding:16px 20px;margin-bottom:12px;">
  ...
  const rekoCard = card(sc, '💡 Rekomendacja',
    `<div style="background:${sc}12;border:1px solid ${sc}30;border-radius:16px;padding:22px;text-align:center;margin-bottom:20px;">
  ...
  const rotSection = rotation.isOptimal
    ...
    : `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(360px,1fr));gap:14px;">` +
      rotation.swaps.map(swap =>
        `<div style="background:rgba(239,68,68,0.05);border:1px solid rgba(239,68,68,0.15);border-radius:16px;padding:20px 24px;">
