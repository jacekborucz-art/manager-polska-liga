// tests/PolishLeagueSeasonTests.ts
...
var PolishLeagueSeasonService = {
  ...
  buildClubsForCareerStart(sourceClubs, startYear) {
    ...
    const configuredIds2 = Object.values(membership).flatMap((clubIds) => clubIds ?? []);
    const uniqueConfiguredIds = new Set(configuredIds2);
    if (uniqueConfiguredIds.size !== configuredIds2.length) {
      throw new Error(`Konfiguracja polskich lig ${startYear}/${startYear + 1} zawiera powt\xF3rzone kluby.`);
    }
    const missingIds = configuredIds2.filter((clubId) => !clubById.has(clubId));
...
// services/ReserveTeamFinanceService.ts
var MIN_SEASON_FUNDING_RATE = 0.02;
var MAX_SEASON_FUNDING_RATE = 0.05;
var EMERGENCY_THRESHOLD_SHARE = 0.2;
var EMERGENCY_TARGET_SHARE = 0.5;
var EMERGENCY_PARENT_BUDGET_CAP = 0.01;
...
var roundToThousands = (amount) => Math.max(0, Math.floor(amount / 1e3) * 1e3);
var getSeasonFundingRate = (parentClubId, seasonStartYear, sessionSeed) => {
  const basisPointsRange = Math.round((MAX_SEASON_FUNDING_RATE - MIN_SEASON_FUNDING_RATE) * 1e4);
  const offset = stableHash(`${parentClubId}_${seasonStartYear}_${sessionSeed}`) % (basisPointsRange + 1);
  return MIN_SEASON_FUNDING_RATE + offset / 1e4;
};
var prependFinanceLog = (club, log) => [log, ...club.financeHistory ?? []].slice(0, 50);
var getSeasonLabel = (seasonStartYear) => `${seasonStartYear}/${String(seasonStartYear + 1).slice(-2)}`;
var ReserveTeamFinanceService = {
  getSeasonStartYear(date) {
    return date.getMonth() >= 6 ? date.getFullYear() : date.getFullYear() - 1;
  },
  allocateSeasonFunding(clubs, options) {
    const { seasonStartYear, sessionSeed, date, resetReserveBalances = false } = options;
    const clubById = new Map(clubs.map((club) => [club.id, { ...club }]));
    const reserveIdsByParentId = /* @__PURE__ */ new Map();
    clubs.forEach((club) => {
    ...
    reserveIdsByParentId.forEach((reserveClubIds, parentClubId) => {
      ...
      const alreadyFunded = reserveClubIds.every(
        (reserveClubId) => clubById.get(reserveClubId)?.reserveTeamSeasonGrantYear === seasonStartYear
      );
      ...
      reserveClubIds.forEach((reserveClubId, index) => {
        ...
        const funding = index === reserveClubIds.length - 1 ? totalFunding - distributedFunding : baseShare;
        const previousBalance = resetReserveBalances ? 0 : reserveClub.budget;
        distributedFunding += funding;
        const reserveIncomeLog = {
        ...
        clubById.set(reserveClubId, {
          ...
          reserveTeamEmergencySupportYear: void 0,
          financeHistory: prependFinanceLog(reserveClub, reserveIncomeLog)
        });
      });
      const reserveNames = reserveClubIds.map((reserveClubId) => clubById.get(reserveClubId)?.name).filter((name) => !!name).join(", ");
      const rateLabel = (fundingRate * 100).toFixed(2).replace(".", ",");
      const parentExpenseLog = {
  ...
  ensureSeasonFunding(clubs, options) {
    const needsMigration = clubs.some(
      (club) => ReserveTeamLeagueService.isReserveClub(club.id) && club.reserveTeamSeasonGrantYear !== options.seasonStartYear
    );
  ...
  applyEmergencySupport(clubs, date) {
    ...
    clubs.forEach((originalReserveClub) => {
      ...
      const parentClub = parentClubId ? clubById.get(parentClubId) : void 0;
...
// tests/PolishLeagueSeasonTests.ts
var clubs2025 = PolishLeagueSeasonService.buildClubsForCareerStart(STATIC_CLUBS, 2025);
import_node_assert.strict.equal(clubs2025.filter((club) => club.leagueId === "L_PL_1").length, 18);
import_node_assert.strict.equal(clubs2025.filter((club) => club.leagueId === "L_PL_2").length, 18);
import_node_assert.strict.equal(clubs2025.filter((club) => club.leagueId === "L_PL_3").length, 18);
...
var clubs2026 = PolishLeagueSeasonService.buildClubsForCareerStart(STATIC_CLUBS, 2026);
var activeLeagueIds = ["L_PL_1", "L_PL_2", "L_PL_3"];
activeLeagueIds.forEach((leagueId, index) => {
  const leagueClubs = clubs2026.filter((club) => club.leagueId === leagueId);
  import_node_assert.strict.equal(leagueClubs.length, 18, `${leagueId} musi zawiera\u0107 18 klub\xF3w`);
  import_node_assert.strict.ok(leagueClubs.every((club) => club.tier === index + 1));
  import_node_assert.strict.ok(leagueClubs.every((club) => club.isDefaultActive));
});
var configuredIds = activeLeagueIds.flatMap(
  (leagueId) => clubs2026.filter((club) => club.leagueId === leagueId).map((club) => club.id)
);
import_node_assert.strict.equal(new Set(configuredIds).size, 54, "klub nie mo\u017Ce wyst\u0119powa\u0107 jednocze\u015Bnie w dw\xF3ch ligach");
import_node_assert.strict.equal(clubs2026.find((club) => club.id === "PL_WISLA_KRAKOW")?.leagueId, "L_PL_1");
import_node_assert.strict.equal(clubs2026.find((club) => club.id === "PL_SLASK_WROCLAW")?.leagueId, "L_PL_1");
import_node_assert.strict.equal(clubs2026.find((club) => club.id === "PL_WIECZYSTA_KRAKOW")?.leagueId, "L_PL_1");
import_node_assert.strict.equal(clubs2026.find((club) => club.id === "PL_ARKA_GDYNIA")?.leagueId, "L_PL_2");
import_node_assert.strict.equal(clubs2026.find((club) => club.id === "PL_TERMALICA_NIECIECZA")?.leagueId, "L_PL_2");
import_node_assert.strict.equal(clubs2026.find((club) => club.id === "PL_LECHIA_GDANSK")?.leagueId, "L_PL_2");
var slaskReserves = clubs2026.find((club) => club.id === "PL_SLASK_WROCLAW_II");
var legiaReserves = clubs2026.find((club) => club.id === "PL_LEGIA_WARSZAWA_II");
import_node_assert.strict.equal(slaskReserves?.leagueId, "L_PL_3");
import_node_assert.strict.equal(legiaReserves?.leagueId, "L_PL_3");
import_node_assert.strict.notEqual(slaskReserves?.id, clubs2026.find((club) => club.id === "PL_SLASK_WROCLAW")?.id);
import_node_assert.strict.notEqual(legiaReserves?.id, clubs2026.find((club) => club.id === "PL_LEGIA_WARSZAWA")?.id);
var slaskSecondLeagueTable = clubs2025.filter((club) => club.leagueId === "L_PL_3").map((club, index) => ({ ...club, stats: { ...club.stats, points: 100 - index } }));
var slaskReservesIndex = slaskSecondLeagueTable.findIndex((club) => club.id === "PL_SLASK_WROCLAW_II");
var secondPlacedClub = slaskSecondLeagueTable[1];
...
var shiftedPromotion = ReserveTeamLeagueService.selectPromotionPlaces(
...
import_node_assert.strict.equal(
  ...
  false,
  "pierwsza dru\u017Cyna nie mo\u017Ce kupowa\u0107 ani wypo\u017Cycza\u0107 zawodnik\xF3w ze swoich rezerw"
);
import_node_assert.strict.equal(
  ReserveTeamLeagueService.canRecruitPlayerFrom("PL_LECH_POZNAN", "PL_LEGIA_WARSZAWA_II"),
  true,
  "rezerwy nadal mog\u0105 sprzedawa\u0107 swoich zawodnik\xF3w innym klubom"
);
var financeParent = {
...
var financeReserve = {
  ...
  transferBudget: 2e6,
  financeHistory: [],
  reserveTeamSeasonGrant: void 0,
  reserveTeamSeasonGrantRate: void 0,
  reserveTeamSeasonGrantYear: void 0,
  reserveTeamEmergencySupportYear: void 0
};
var clubsAfterReserveFunding = ReserveTeamFinanceService.allocateSeasonFunding(
...
var fundedParent = clubsAfterReserveFunding.find((club) => club.id === financeParent.id);
var fundedReserve = clubsAfterReserveFunding.find((club) => club.id === financeReserve.id);
import_node_assert.strict.ok(
  fundedReserve.budget >= 2e5 && fundedReserve.budget <= 5e5,
...
import_node_assert.strict.equal(fundedReserve.transferBudget, 0, "rezerwy nie mog\u0105 otrzyma\u0107 bud\u017Cetu transferowego");
import_node_assert.strict.ok(
  fundedParent.financeHistory?.some(
    (log) => log.type === "EXPENSE" && log.amount === -fundedReserve.budget && log.description.includes("Finansowanie dru\u017Cyny rezerw")
  ),
  "raport finansowy pierwszej dru\u017Cyny musi pokaza\u0107 wydatek na rezerwy"
);
var clubsAfterLoadFundingCheck = ReserveTeamFinanceService.ensureSeasonFunding(
...
var reserveWithLowBalance = clubsAfterReserveFunding.map(
  (club) => club.id === fundedReserve.id ? { ...club, budget: 0 } : club
);
var clubsAfterEmergencySupport = ReserveTeamFinanceService.applyEmergencySupport(
  reserveWithLowBalance,
  new Date(2025, 9, 15)
);
var emergencyParent = clubsAfterEmergencySupport.find((club) => club.id === financeParent.id);
var emergencyReserve = clubsAfterEmergencySupport.find((club) => club.id === financeReserve.id);
import_node_assert.strict.ok(emergencyReserve.budget > 0, "znaczny spadek salda rezerw musi uruchomi\u0107 wsparcie pierwszej dru\u017Cyny");
import_node_assert.strict.ok(
  emergencyParent.financeHistory?.some(
    (log) => log.type === "EXPENSE" && log.description.includes("Awaryjne wsparcie finansowe dru\u017Cyny rezerw")
  ),
  "awaryjne wsparcie rezerw musi znale\u017A\u0107 si\u0119 w wydatkach pierwszej dru\u017Cyny"
);
var clubsAfterRepeatedEmergencyCheck = ReserveTeamFinanceService.applyEmergencySupport(
...
import_node_assert.strict.equal(
  ReserveTeamLeagueService.canEnterLeague(
    ...
    "L_PL_3",
    clubs2025,
    /* @__PURE__ */ new Map([["PL_LKS_LODZ", "L_PL_3"]])
  ),
  false,
  "rezerwy z 3. ligi nie mog\u0105 wej\u015B\u0107 do 2. ligi zajmowanej przez pierwszy zesp\xF3\u0142"
);
var oldSavePlayoffResolution = ReserveTeamLeagueService.resolvePlayoffWinner(
...
var parentDropsToLigaOne = ReserveTeamLeagueService.createLeagueProjection(clubs2026, [
...
var parentPromotesToEkstraklasa = ReserveTeamLeagueService.createLeagueProjection(clubs2025, [
...
var parentDropsToLigaTwo = ReserveTeamLeagueService.createLeagueProjection(clubs2025, [
...
var parentPromotesToLigaOne = ReserveTeamLeagueService.createLeagueProjection(clubs2025, [
...
var guaranteedCupClubIds = clubs2025.filter((club) => ["L_PL_1", "L_PL_2", "L_PL_3"].includes(club.leagueId)).map((club) => club.id);
var cupParticipantsSeedA = PolishCupDrawService.getInitialParticipants(clubs2025, 12345, 2025);
var cupParticipantsSeedARepeat = PolishCupDrawService.getInitialParticipants(clubs2025, 12345, 2025);
var cupParticipantsSeedB = PolishCupDrawService.getInitialParticipants(clubs2025, 98765, 2025);
import_node_assert.strict.equal(cupParticipantsSeedA.length, 128, "Puchar Polski musi rozpoczyna\u0107 128 klub\xF3w");
import_node_assert.strict.equal(new Set(cupParticipantsSeedA).size, 128, "uczestnicy Pucharu Polski nie mog\u0105 si\u0119 powtarza\u0107");
import_node_assert.strict.ok(
...
import_node_assert.strict.notDeepEqual(
