...
// tests/MatchEngineV2SpatialContinuityTests.ts
...
var updateContext = (previous2, side, possession, ballZone, second, possessionChanged) => {
  let phase;
  if (possessionChanged) {
    phase = side === possession ? "TRANSITION_ATTACK" : "TRANSITION_DEFEND";
  } else if ((previous2.phase === "TRANSITION_ATTACK" || previous2.phase === "TRANSITION_DEFEND") && second - previous2.phaseChangedAtSecond < TRANSITION_DURATION_SECONDS) {
    phase = previous2.phase;
  } else {
    phase = settledPhase(side, possession, ballZone);
  }
  if (phase === previous2.phase) return previous2;
  return {
    phase,
    previousPhase: previous2.phase,
    phaseChangedAtSecond: second
  };
};
...
var resolve = ({
  ...
  const elapsed = Math.max(0.2, second - previous2.lastUpdatedSecond);
  if (latestCue?.atSecond === second && travellingKind(latestCue)) {
    ...
    return {
      ...
      velocity: velocityBetween(previous2, target, elapsed),
      intendedReceiverId: latestCue.kind === "PASS" || latestCue.kind === "CROSS" ? latestCue.secondaryPlayerId : void 0,
      lastTouchPlayerId: latestCue.actorId ?? previous2.lastTouchPlayerId,
  ...
  if (latestCue?.atSecond === second && controlledKind(latestCue)) {
    ...
    return {
      ...
      y: ownerPoint.y,
      z: 0,
      velocity: carrier?.playerId === ownerId ? { x: carrier.velocity.x, y: carrier.velocity.y, z: 0 } : velocityBetween(previous2, ownerPoint, elapsed),
      ownerId,
      lastTouchPlayerId: ownerId ?? previous2.lastTouchPlayerId,
      lastUpdatedSecond: second,
      phase: "CONTROLLED"
    };
  }
  ...
  return {
    ...
    y: fallback.y,
    z: 0,
    velocity: velocityBetween(previous2, fallback, elapsed),
    lastTouchPlayerId: previous2.lastTouchPlayerId,
    lastUpdatedSecond: second,
    phase: "LOOSE"
  };
};
...
var placeInitialKickOff = (players, live) => {
  ...
  const kicker2 = candidates.find((player) => player.role === "MID") ?? candidates.find((player) => player.role === "FWD") ?? candidates[0];
  const support = candidates.find((player) => player.playerId !== kicker2?.playerId && player.role === "FWD") ?? candidates.find((player) => player.playerId !== kicker2?.playerId);
  if (!kicker2) return;
  const direction = side === "HOME" ? 1 : -1;
  kicker2.position = { x: PITCH_WIDTH2 / 2, y: PITCH_LENGTH2 / 2 };
  kicker2.target = clonePoint(kicker2.position);
  kicker2.returningToMovementZone = true;
  if (support) {
...
var MatchEngineV2SpatialService = {
  ...
  synchronize: (spatial, live) => {
    ...
    ["HOME", "AWAY"].forEach((side) => {
      const previous2 = spatial.teamContexts[side];
...
var terminalCues = (mode, cues) => {
  ...
  candidates.forEach((cue) => {
    ...
    const previous2 = bestBySequence.get(key);
    if (!previous2 || priorityFor(cue) >= priorityFor(previous2)) bestBySequence.set(key, cue);
  });
...
var sceneOrigin = (active, playerId, fallback) => active.playerOrigins?.[playerId] ?? fallback;
...
var activateLiveCue = (controller, pending, startedAtMs, ballOrigin, snapshot2) => {
  ...
  const playerOrigins = Object.fromEntries(Object.values(snapshot2.spatial.players).map((player) => {
    const previous2 = controller.lastFramePlayers[player.playerId]?.position ?? player.position;
    return [player.playerId, { ...isFirstScriptedScene ? player.anchor : previous2 }];
  }));
...
var rebaseActionTracks = (controller, completed, snapshot2, playback) => {
  const affectedPlayerIds = usesStructuredRestartShape(completed.cue) ? Object.keys(completed.playerOrigins ?? {}) : [completed.cue.actorId, completed.cue.secondaryPlayerId].filter((playerId) => Boolean(playerId));
  affectedPlayerIds.forEach((playerId) => {
    ...
    const authoritative = snapshot2.spatial.players[playerId];
...
// tests/MatchEngineV2SpatialContinuityTests.ts
var sample = CupSampleMatchFactory.makeInput(2442, "EQUAL");
...
var activeKickoffPlayers = Object.values(kickoff.spatial.players).filter((player) => player.isOnPitch);
var outfieldAtCentre = activeKickoffPlayers.filter(
  (player) => player.role !== "GK" && Math.abs(player.position.y - 52.5) < 3.6
);
import_strict.default.ok(outfieldAtCentre.length <= 2, `Przy rozpocz\u0119ciu w \u015Brodku znalaz\u0142o si\u0119 ${outfieldAtCentre.length} zawodnik\xF3w.`);
var kickoffSide = runtime.core.state.firstHalfKickOffSide;
var kicker = outfieldAtCentre.find((player) => player.side === kickoffSide) ?? activeKickoffPlayers.find((player) => player.side === kickoffSide && player.role !== "GK");
var kickoffCue = {
  id: "continuity_kickoff_cue",
  sourceEventId: "continuity_kickoff_event",
  sourceEventType: "KICK_OFF" /* KICK_OFF */,
  kind: "RESTART",
  atSecond: 5,
  side: kickoffSide,
...
var playing = MatchEngineV2PlaybackService.setPaused(MatchEngineV2PlaybackService.create(), false);
var frameController = MatchEngineV2FrameControllerService.create(kickoff, 0);
MatchEngineV2FrameControllerService.advance(frameController, kickoffScene, playing, 100);
var kickoffFrame = MatchEngineV2FrameControllerService.advance(frameController, kickoffScene, playing, 570);
var renderedAtCentre = activeKickoffPlayers.filter(
  (player) => player.role !== "GK" && Math.abs(kickoffFrame.players[player.playerId].position.y - 52.5) < 3.6
);
import_strict.default.ok(renderedAtCentre.length <= 2, `SVG ustawi\u0142 ${renderedAtCentre.length} zawodnik\xF3w w jednej linii przy \u015Brodku.`);
var previous = kickoff;
var maximumTickTravel = 0;
for (let second = 5; second <= 20 * 60; second += 5) {
  ...
  Object.values(current.spatial.players).filter((player) => player.isOnPitch).forEach((player) => {
    ...
    import_strict.default.ok(travel <= 9.5, `${player.playerId} przemie\u015Bci\u0142 si\u0119 o ${travel.toFixed(2)} m w jednym ticku.`);
    import_strict.default.ok(Math.hypot(player.velocity.x, player.velocity.y) <= player.metresPerSecond + 0.01);
    import_strict.default.ok(Number.isFinite(player.facingRadians));
  });
  const ball = current.spatial.ball;
  if (ball.phase === "CONTROLLED") {
    import_strict.default.ok(ball.ownerId, "Kontrolowana pi\u0142ka musi mie\u0107 dok\u0142adnie jednego w\u0142a\u015Bciciela.");
    const owner = current.spatial.players[ball.ownerId];
    import_strict.default.ok(owner?.isOnPitch);
    import_strict.default.ok(Math.hypot(ball.x - owner.position.x, ball.y - owner.position.y) < 0.01);
  } else {
    import_strict.default.equal(ball.ownerId, void 0, "Pi\u0142ka w locie, lu\u017Ana lub martwa nie mo\u017Ce mie\u0107 w\u0142a\u015Bciciela.");
  }
  previous = current;
}
var finalSnapshot = previous;
var spatialPasses = finalSnapshot.result.events.filter(
  (event) => event.type === "PASS_COMPLETED" /* PASS_COMPLETED */ && typeof event.detail?.passDistance === "number"
);
import_strict.default.ok(spatialPasses.length >= 3, "Decyzje poda\u0144 musz\u0105 korzysta\u0107 ze wsp\xF3\u0142rz\u0119dnych boiska.");
spatialPasses.forEach((event) => {
  import_strict.default.equal(typeof event.detail?.startX, "number");
  import_strict.default.equal(typeof event.detail?.startY, "number");
  import_strict.default.equal(typeof event.detail?.endX, "number");
  import_strict.default.equal(typeof event.detail?.endY, "number");
  import_strict.default.equal(typeof event.detail?.laneClearance, "number");
  import_strict.default.ok(event.detail?.passDistance > 0);
});
var connectedCues = finalSnapshot.spatial.visualCues.filter((cue) => cue.sequenceId);
...
var travellingBall = MatchEngineV2BallService.resolve({
  previous: baseBall,
  latestCue: passCue,
  fallback: passCue.end,
  second: 15
});
import_strict.default.equal(travellingBall.phase, "TRAVELLING");
import_strict.default.equal(travellingBall.ownerId, void 0);
import_strict.default.equal(travellingBall.intendedReceiverId, passCue.secondaryPlayerId);
