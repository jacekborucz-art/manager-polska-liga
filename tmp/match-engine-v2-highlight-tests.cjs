...
// tests/MatchEngineV2HighlightScriptTests.ts
...
var MatchEngineV2HighlightScriptService = {
  ...
  materialize: (terminal2, snapshot3) => {
    const script = MatchEngineV2HighlightScriptService.selectScript(terminal2);
    const outcome = cueOutcome(terminal2);
    if (!script || !outcome) return [];
    const attackingSide = outcome === "FOUL" ? terminal2.side === "HOME" ? "AWAY" : "HOME" : terminal2.side ?? "HOME";
    ...
    const recordedAttackerId = outcome === "FOUL" ? terminal2.secondaryPlayerId : terminal2.actorId;
    ...
    const sequenceId = `highlight_${terminal2.id}_${script.id}`;
    ...
    return script.steps.map((scriptStep, index) => {
      ...
      return {
        ...
        sourceEventId: last ? terminal2.sourceEventId : `${terminal2.sourceEventId}_script_${index + 1}`,
        sequenceId,
        sourceEventType: last ? terminal2.sourceEventType : eventTypeForStep(scriptStep.kind),
        ...
        setPieceKind: terminal2.setPieceKind,
        kind: last ? terminal2.kind : cueKindForStep(scriptStep.kind),
        atSecond: terminal2.atSecond,
        side: attackingSide,
        actorId,
        secondaryPlayerId,
        start: mirroredPoint(scriptStep.start, away),
        end: mirroredPoint(last ? terminalEnd(outcome, scriptStep, terminal2.id) : scriptStep.end, away),
...
// tests/MatchEngineV2HighlightScriptTests.ts
import_strict.default.equal(MATCH_ENGINE_V2_GOAL_SCRIPTS.length, 10, "Pi\u0119\u0107 og\xF3lnych scenariuszy gola plus dwa ro\u017Cne i trzy wolne.");
import_strict.default.equal(MATCH_ENGINE_V2_OFFSIDE_SCRIPTS.length, 10);
import_strict.default.equal(MATCH_ENGINE_V2_MISS_SCRIPTS.length, 10, "Pi\u0119\u0107 og\xF3lnych scenariuszy niecelnego strza\u0142u plus dwa ro\u017Cne i trzy wolne.");
import_strict.default.equal(MATCH_ENGINE_V2_SAVE_SCRIPTS.length, 10, "Pi\u0119\u0107 og\xF3lnych scenariuszy obrony plus dwa ro\u017Cne i trzy wolne.");
import_strict.default.equal(MATCH_ENGINE_V2_FOUL_SCRIPTS.length, 26);
import_strict.default.equal(MATCH_ENGINE_V2_CORNER_RESTART_SCRIPTS.length, 3, "Trzy autorskie scenariusze ro\u017Cnego bez strza\u0142u (kr\xF3tko rozegrany, wybicie, wybicie i kontra).");
import_strict.default.equal(MATCH_ENGINE_V2_FREE_KICK_RESTART_SCRIPTS.length, 2, "Dwa autorskie scenariusze wolnego bez strza\u0142u (kr\xF3tkie rozegranie, wybicie i odzyskanie).");
var allScripts = Object.values(MATCH_ENGINE_V2_HIGHLIGHT_SCRIPTS).flat();
import_strict.default.equal(allScripts.length, 71);
import_strict.default.equal(new Set(allScripts.map((script) => script.id)).size, 71, "Ka\u017Cdy scenariusz musi mie\u0107 unikalny identyfikator.");
allScripts.forEach((script) => {
  import_strict.default.ok(script.steps.length >= 2 && script.steps.length <= 6);
  script.steps.forEach((scriptStep, index) => {
    import_strict.default.ok(scriptStep.start.x >= 0 && scriptStep.start.x <= 68);
    import_strict.default.ok(scriptStep.start.y >= 0 && scriptStep.start.y <= 105);
    import_strict.default.ok(scriptStep.end.x >= 0 && scriptStep.end.x <= 68);
    import_strict.default.ok(scriptStep.end.y >= 0 && scriptStep.end.y <= 105);
    import_strict.default.ok(scriptStep.durationMs >= 350);
    const next = script.steps[index + 1];
    if (next) import_strict.default.deepEqual(scriptStep.end, next.start, `${script.id}: pi\u0142ka nie mo\u017Ce przeskakiwa\u0107 mi\u0119dzy scenami.`);
  });
});
var sample = CupSampleMatchFactory.makeInput(120120, "EQUAL");
...
var homeAttacker = Object.values(snapshot2.spatial.players).find((player) => player.side === "HOME" && player.role === "FWD");
var awayDefender = Object.values(snapshot2.spatial.players).find((player) => player.side === "AWAY" && player.role === "DEF");
var terminal = (outcome, sequence, setPieceKind) => ({
  id: `terminal_${outcome}_${sequence}`,
  sourceEventId: `event_${outcome}_${sequence}`,
  sequenceId: `sequence_${outcome}_${sequence}`,
  sourceEventType: outcome === "GOAL" ? "GOAL" /* GOAL */ : outcome === "OFFSIDE" ? "OFFSIDE" /* OFFSIDE */ : outcome === "SAVE" ? "SAVE" /* SAVE */ : outcome === "FOUL" ? "FOUL" /* FOUL */ : outcome === "CORNER_RESTART" ? "CORNER_TAKEN" /* CORNER_TAKEN */ : outcome === "FREE_KICK_RESTART" ? "FREE_KICK" /* FREE_KICK */ : "SHOT" /* SHOT */,
  kind: outcome === "GOAL" ? "GOAL" : outcome === "SAVE" ? "SAVE" : outcome === "FOUL" ? "FOUL" : outcome === "OFFSIDE" || outcome === "CORNER_RESTART" || outcome === "FREE_KICK_RESTART" ? "RESTART" : "SHOT",
  atSecond: 600 + sequence,
  side: outcome === "FOUL" ? "AWAY" : "HOME",
  actorId: outcome === "FOUL" ? awayDefender.playerId : homeAttacker.playerId,
  secondaryPlayerId: outcome === "FOUL" ? homeAttacker.playerId : void 0,
  setPieceKind,
  start: { x: 34, y: 82 },
  end: { x: 34, y: 104 },
  durationMs: 800
});
var terminals = ["GOAL", "OFFSIDE", "MISS", "SAVE", "FOUL", "CORNER_RESTART", "FREE_KICK_RESTART"].map((outcome, index) => terminal(outcome, index));
var sawAttackingGroupBehavior = false;
var sawDefendingGroupBehavior = false;
terminals.forEach((sourceCue) => {
  ...
  import_strict.default.equal(firstSelection?.id, secondSelection?.id, "Ten sam zapis meczu musi wybra\u0107 ten sam scenariusz.");
  const presentation = MatchEngineV2HighlightScriptService.materialize(sourceCue, snapshot2);
  import_strict.default.equal(presentation.length, firstSelection?.steps.length);
  import_strict.default.ok(presentation.every((cue) => cue.scriptedHighlight));
  import_strict.default.ok(presentation.every((cue) => cue.side === "HOME"), "Scena faulu r\xF3wnie\u017C ma pokazywa\u0107 atak dru\u017Cyny faulowanej.");
  import_strict.default.equal(presentation.at(-1)?.sourceEventId, sourceCue.sourceEventId);
  const terminalScene = presentation.at(-1);
  if (sourceCue.sourceEventType === "SAVE" /* SAVE */) {
    const goalkeeper = snapshot2.spatial.players[terminalScene.secondaryPlayerId ?? ""];
    import_strict.default.equal(goalkeeper?.side, "AWAY");
    import_strict.default.equal(goalkeeper?.role, "GK");
    import_strict.default.ok(terminalScene.end.y < 105, "Obroniony strza\u0142 nie mo\u017Ce wygl\u0105da\u0107 jak pi\u0142ka wpadaj\u0105ca do bramki.");
  }
  if (sourceCue.sourceEventType === "SHOT" /* SHOT */) {
    import_strict.default.ok(terminalScene.end.x === 18 || terminalScene.end.x === 50, "Niecelny strza\u0142 powinien min\u0105\u0107 \u015Bwiat\u0142o bramki.");
  }
  if (sourceCue.sourceEventType === "FOUL" /* FOUL */) import_strict.default.deepEqual(terminalScene.end, terminalScene.start);
  if (sourceCue.sourceEventType === "FOUL" /* FOUL */) {
    import_strict.default.ok(!terminalScene.commentaryTemplate?.includes("STRZELA"), "Faul nie mo\u017Ce by\u0107 opisany jak strza\u0142.");
  }
  if (sourceCue.sourceEventType === "OFFSIDE" /* OFFSIDE */) {
    import_strict.default.ok(terminalScene.commentaryTemplate?.includes("spalone"), "Spalone musi mie\u0107 w\u0142asny komentarz, nie tekst strza\u0142u.");
  }
  if (sourceCue.sourceEventType === "GOAL" /* GOAL */) {
    import_strict.default.equal(
      terminalScene.actorId,
      sourceCue.actorId,
      "Strzelec w scenie musi zosta\u0107 tym samym zawodnikiem, kt\xF3ry naprawd\u0119 strzeli\u0142 gola w meczu \u2014 nawet w wieloetapowym, r\u0119cznie napisanym scenariuszu."
    );
  }
  presentation.forEach((cue, index) => {
    import_strict.default.equal(cue.highlightSceneIndex, index + 1);
    import_strict.default.equal(cue.highlightSceneCount, presentation.length);
    import_strict.default.ok(cue.actorId);
    import_strict.default.ok(cue.commentaryTemplate, `Krok ${index + 1} scenariusza ${firstSelection?.id} musi mie\u0107 komentarz.`);
    ...
    (cue.supportingRuns ?? []).forEach((run) => {
      import_strict.default.ok(
        snapshot2.spatial.players[run.playerId],
        `Dodatkowy bieg w kroku ${index + 1} scenariusza ${firstSelection?.id} wskazuje na nieistniej\u0105cego zawodnika.`
      );
      import_strict.default.notEqual(run.playerId, cue.actorId, "Zawodnik wykonuj\u0105cy dodatkowy bieg nie mo\u017Ce by\u0107 jednocze\u015Bnie aktorem tego samego kroku.");
      import_strict.default.notEqual(run.playerId, cue.secondaryPlayerId, "Zawodnik wykonuj\u0105cy dodatkowy bieg nie mo\u017Ce by\u0107 jednocze\u015Bnie odbiorc\u0105 tego samego kroku.");
    });
    if ((cue.kind === "PASS" || cue.kind === "CROSS") && cue.secondaryPlayerId) {
      import_strict.default.notEqual(cue.actorId, cue.secondaryPlayerId, "Zawodnik nie mo\u017Ce poda\u0107 pi\u0142ki sam do siebie.");
    }
    const next = presentation[index + 1];
    if (next) import_strict.default.deepEqual(cue.end, next.start, "Pi\u0142ka musi kontynuowa\u0107 lot z poprzedniego punktu.");
  });
});
var verifyAuthoredScenarioIdentity = (outcome, expectedScriptCount) => {
  const seenScriptIds = /* @__PURE__ */ new Set();
  for (let sample2 = 0; sample2 < 60 && seenScriptIds.size < expectedScriptCount; sample2 += 1) {
    const cue = terminal(outcome, sample2);
    ...
    const presentation = MatchEngineV2HighlightScriptService.materialize(cue, snapshot2);
    const finalScene = presentation.at(-1);
    import_strict.default.equal(finalScene.actorId, cue.actorId, `${script.id}: strzelec musi zosta\u0107 tym samym zawodnikiem przez ca\u0142y scenariusz.`);
    if (outcome === "SAVE") {
      const goalkeeper = snapshot2.spatial.players[finalScene.secondaryPlayerId ?? ""];
      import_strict.default.equal(goalkeeper?.role, "GK", `${script.id}: obron\u0119 musi zapisywa\u0107 prawdziwy bramkarz dru\u017Cyny broni\u0105cej.`);
    }
    const sameSideOtherActorIds = new Set(
      presentation.filter((sceneCue) => sceneCue.actorId && sceneCue.actorId !== finalScene.actorId).map((sceneCue) => sceneCue.actorId)
    );
    presentation.forEach((sceneCue) => {
      (sceneCue.supportingRuns ?? []).forEach((run) => {
        import_strict.default.ok(snapshot2.spatial.players[run.playerId], `${script.id}: dodatkowy bieg wskazuje na nieistniej\u0105cego zawodnika.`);
      });
    });
    if (script.id === "BOX_ONE_TWO") {
      import_strict.default.ok(sameSideOtherActorIds.size >= 1, "BOX_ONE_TWO musi zaanga\u017Cowa\u0107 przynajmniej jednego innego pomocnika ni\u017C strzelec.");
    }
  }
  import_strict.default.equal(
    seenScriptIds.size,
...
var GENERIC_SHOT_SCRIPT_COUNT = 5;
...
import_strict.default.ok(sawAttackingGroupBehavior, "Przynajmniej jeden krok musi mie\u0107 zachowanie grupowe dru\u017Cyny atakuj\u0105cej.");
import_strict.default.ok(sawDefendingGroupBehavior, "Przynajmniej jeden krok musi mie\u0107 zachowanie grupowe dru\u017Cyny broni\u0105cej.");
var verifyAuthoredSetPieceScenarioIdentity = (outcome, setPieceKind, requiredSetPieceKind, expectedScriptCount) => {
  const seenScriptIds = /* @__PURE__ */ new Set();
  for (let sample2 = 0; sample2 < 60 && seenScriptIds.size < expectedScriptCount; sample2 += 1) {
    const cue = { ...terminal(outcome, 1e3, setPieceKind), sequenceId: `sequence_${outcome}_${setPieceKind}_probe_${sample2}` };
    const script = MatchEngineV2HighlightScriptService.selectScript(cue);
    if (!script || seenScriptIds.has(script.id)) continue;
    import_strict.default.equal(script.requiredSetPieceKind, requiredSetPieceKind, `${script.id}: musi by\u0107 otagowany jako ${requiredSetPieceKind}.`);
    seenScriptIds.add(script.id);
    const presentation = MatchEngineV2HighlightScriptService.materialize(cue, snapshot2);
    const finalScene = presentation.at(-1);
    import_strict.default.equal(finalScene.actorId, cue.actorId, `${script.id}: strzelec musi zosta\u0107 tym samym zawodnikiem przez ca\u0142y scenariusz.`);
...
["GOAL", "SAVE", "MISS"].forEach((outcome) => {
...
var verifyAuthoredRestartScenarioCoverage = (outcome, expectedScriptCount) => {
  const seenScriptIds = /* @__PURE__ */ new Set();
  for (let sample2 = 0; sample2 < 60 && seenScriptIds.size < expectedScriptCount; sample2 += 1) {
    const cue = { ...terminal(outcome, 2e3), sequenceId: `sequence_${outcome}_probe_${sample2}` };
    const script = MatchEngineV2HighlightScriptService.selectScript(cue);
    if (!script || seenScriptIds.has(script.id)) continue;
    seenScriptIds.add(script.id);
    const presentation = MatchEngineV2HighlightScriptService.materialize(cue, snapshot2);
...
var highXgMiss = { ...lowXgMiss, id: "terminal_high_xg_miss", sourceEventId: "event_high_xg_miss", atSecond: 720, xG: 0.55 };
var postMiss = { ...lowXgMiss, id: "terminal_post_miss", sourceEventId: "event_post_miss", sourceEventType: "SHOT_POST" /* SHOT_POST */, atSecond: 730, xG: 0.05 };
var penaltyMiss = { ...lowXgMiss, id: "terminal_penalty_miss", sourceEventId: "event_penalty_miss", sourceEventType: "PENALTY_MISSED" /* PENALTY_MISSED */, atSecond: 740, xG: 0.76 };
var lowXgSave = {
  id: "terminal_low_xg_save",
  sourceEventId: "event_low_xg_save",
  sourceEventType: "SAVE" /* SAVE */,
  kind: "SAVE",
  atSecond: 750,
  side: "HOME",
  actorId: homeAttacker.playerId,
...
var highXgSave = { ...lowXgSave, id: "terminal_high_xg_save", sourceEventId: "event_high_xg_save", atSecond: 760, xG: 0.62 };
var tieringCues = [...terminals, restartOnly, lowXgMiss, highXgMiss, postMiss, penaltyMiss, lowXgSave, highXgSave];
var allActions = MatchEngineV2HighlightScriptService.selectTerminalCues("ALL_ACTIONS", tieringCues);
import_strict.default.equal(allActions.length, 14, "WSZYSTKIE AKCJE pokazuj\u0105 ka\u017Cdy z siedmiu wynik\xF3w (w tym rozegrany ro\u017Cny/wolny bez strza\u0142u) plus siedem nowych sytuacji strza\u0142owych/restart\xF3w.");
import_strict.default.ok(allActions.some((cue) => cue.id === restartOnly.id), "Samodzielny rzut ro\u017Cny bez strza\u0142u musi by\u0107 widoczny w trybie WSZYSTKIE AKCJE.");
var keyMoments = MatchEngineV2HighlightScriptService.selectTerminalCues("KEY_MOMENTS", tieringCues);
var keyMomentIds = keyMoments.map((cue) => cue.id).sort();
...
import_strict.default.ok(!keyMomentIds.includes(lowXgMiss.id), "Niecelny strza\u0142 o niskim xG nie jest kluczowy.");
import_strict.default.ok(!keyMomentIds.includes(lowXgSave.id), "Obrona przy niskim xG nie jest kluczowa.");
import_strict.default.ok(!keyMomentIds.includes(restartOnly.id), "Sam rzut ro\u017Cny bez strza\u0142u nie jest kluczowy.");
import_strict.default.ok(!keyMomentIds.includes(terminals[5].id), "Rozegrany rzut ro\u017Cny bez strza\u0142u nie jest kluczowy.");
import_strict.default.ok(!keyMomentIds.includes(terminals[6].id), "Rozegrany rzut wolny bez strza\u0142u nie jest kluczowy.");
var commentaryOnly = MatchEngineV2HighlightScriptService.selectTerminalCues("COMMENTARY_ONLY", tieringCues);
import_strict.default.equal(commentaryOnly.length, 0, "TYLKO KOMENTARZ nie pokazuje \u017Cadnej sceny.");
var fullMatch = MatchEngineV2HighlightScriptService.selectTerminalCues("FULL_MATCH", tieringCues);
import_strict.default.deepEqual(fullMatch, tieringCues, "PE\u0141NY MECZ musi zachowa\u0107 wszystkie zapisane zdarzenia pi\u0142karskie.");
var playback = MatchEngineV2PlaybackService.create({ transmissionMode: "ALL_ACTIONS" });
import_strict.default.deepEqual(MatchEngineV2PlaybackService.selectVisibleCues(playback, tieringCues), allActions);
