...
// tests/PolishRegionalRelegationMatrixTests.ts
...
var table = (clubs, leagueId) => clubs.filter((club) => club.leagueId === leagueId && club.isDefaultActive).sort(
...
var shuffledForFourthLeaguePlayoff = (clubs, sessionSeed, seasonNumber, groupId) => [...clubs].sort(
...
var PolishThirdLeagueSeasonTransitionService = {
  resolve(clubs, players, sessionSeed, seasonNumber, projectedLeagueByClubId, overrides = {}, fourthLeagueState) {
    const clubById = new Map(clubs.map((club) => [club.id, club]));
    ...
    const promotedChampionIds = champions.filter((club) => ReserveTeamLeagueService.canEnterLeague(club.id, "L_PL_3", clubs, projectedLeagueByClubId)).map((club) => club.id);
    ...
    const forcedReserveRelegationIds = new Set(
      projectedLeagueByClubId ? ReserveTeamLeagueService.findSameLeagueConflicts(clubs, projectedLeagueByClubId).filter(
    ...
    const supplementaryCandidates = THIRD_LEAGUE_GROUP_IDS.flatMap((groupId) => groupTables[groupId].map((club, index) => ({ club, tablePosition: index + 1 }))).filter(
      (candidate) => !promotedSet.has(candidate.club.id) && ReserveTeamLeagueService.canEnterLeague(candidate.club.id, "L_PL_3", clubs, projectedLeagueByClubId)
    ).sort(
    ...
    if (fourthLeagueState) {
      THIRD_LEAGUE_GROUP_IDS.forEach((groupId) => {
        ...
        const regionalQualifiers = regionalLeagueIds.map((leagueId) => {
          const ranking = PolishFourthLeagueService.getTable(clubs, leagueId).filter(
            (club) => PolishFourthLeagueService.canReserveEnterThirdLeague(club, clubs) && ReserveTeamLeagueService.canEnterLeague(club.id, groupId, clubs, projectedLeagueByClubId)
          );
          if (ranking.length < 2) {
...
// tests/PolishRegionalRelegationMatrixTests.ts
var GROUP_REGIONS = {
  L_PL_4_G1: ["\u0142\xF3dzkie", "mazowieckie", "podlaskie", "warmi\u0144sko-mazurskie"],
...
var REGIONAL_PATTERNS = ["BALANCED", "CONCENTRATED", "SPLIT"];
var SCENARIOS = (() => {
  const scenarios = [];
  let scenarioIndex = 0;
  for (let groupOne = 0; groupOne <= 4; groupOne++) {
    for (let groupTwo = 0; groupTwo <= 4 - groupOne; groupTwo++) {
      for (let groupThree = 0; groupThree <= 4 - groupOne - groupTwo; groupThree++) {
        ...
        const secondLeagueDropGroups = counts.flatMap(
          (count, groupIndex) => Array.from({ length: count }, () => THIRD_LEAGUE_GROUP_IDS[groupIndex])
        );
        scenarios.push({
...
var rankScenarioWorld = (source, scenario) => {
  const thirdLeagueRankById = /* @__PURE__ */ new Map();
  ...
  return source.map((club) => {
    ...
    if (thirdRank) {
      ...
      const bottomStart = 8;
      const polishVoivodeship = thirdRank.position >= bottomStart ? regionForBottomPosition(regions, thirdRank.position - bottomStart, scenario.thirdLeagueBottomPattern) : regions[thirdRank.position % regions.length];
      return {
        ...club,
        polishVoivodeship,
        stats: {
    ...
    if (secondRank !== void 0) {
      const directDropIndex = secondRank - 14;
      const polishVoivodeship = directDropIndex >= 0 ? GROUP_REGIONS[scenario.secondLeagueDropGroups[directDropIndex]][0] : club.polishVoivodeship;
      return {
        ...club,
        polishVoivodeship,
        stats: {
...
SCENARIOS.forEach((scenario, scenarioIndex) => {
  const seed = 92e4 + scenarioIndex * 100;
  ...
  const challengers = preliminary.playoffMatches.filter((match) => match.stage === "RUNNERS_UP").map((match) => match.winnerClubId);
  const secondLeagueTable = rankedClubs.filter((club) => club.leagueId === "L_PL_3").sort(
    (left, right) => right.stats.points - left.stats.points || right.stats.goalDifference - left.stats.goalDifference || right.stats.goalsFor - left.stats.goalsFor || left.id.localeCompare(right.id)
  );
  const incumbents = secondLeagueTable.slice(12, 14);
  ...
  const expectedSecondLeagueDrops = new Map(
  ...
  THIRD_LEAGUE_GROUP_IDS.forEach((groupId) => {
    ...
    const relegatedFromGroup = resolution.relegatedToFeederIds.map((clubId) => rankedClubs.find((club) => club.id === clubId)).filter((club) => club.leagueId === groupId);
    import_strict.default.equal(
    ...
    const nextThirdLeagueSize = rankedClubs.filter(
      (club) => (resolution.nextLeagueByClubId.get(club.id) ?? club.leagueId) === groupId
    ).length;
    import_strict.default.equal(nextThirdLeagueSize, 18, `${scenario.name}: ${groupId} must finish with 18 clubs`);
  });
  ...
  const poolIdsBeforeRebalance = new Map(
    Object.entries(FOURTH_LEAGUE_FEEDER_BY_VOIVODESHIP).map(([voivodeship, poolId]) => [
      voivodeship,
  ...
  Object.entries(FOURTH_LEAGUE_BY_VOIVODESHIP).forEach(([voivodeship, leagueId]) => {
    const poolId = FOURTH_LEAGUE_FEEDER_BY_VOIVODESHIP[voivodeship];
    ...
    const actualFourthLeagueDrops = [...previousFourthLeagueIds].filter((clubId) => rebalanced.find((club) => club.id === clubId)?.leagueId !== leagueId).length;
    const promotedFromRegionalPool = rebalanced.filter((club) => club.leagueId === leagueId && previousPoolIds.has(club.id));
    import_strict.default.equal(
    ...
    import_strict.default.ok(
      rebalanced.filter((club) => club.leagueId === leagueId || club.leagueId === poolId).every((club) => club.polishVoivodeship === voivodeship),
