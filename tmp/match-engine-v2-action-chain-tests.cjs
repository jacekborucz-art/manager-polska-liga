...
// tests/MatchEngineV2ActionChainTests.ts
...
var totals = new Map(trackedTypes.map((type) => [type, 0]));
var validatedGoalChains = 0;
var totalMatches = 0;
var reboundShots = 0;
for (let seed = 1; seed <= 36; seed += 1) {
  ...
  const snapshot2 = MatchEngineV2.advanceTo(runtime, 120 * 60);
  import_strict.default.equal(snapshot2.isFinished, true);
  import_strict.default.ok(snapshot2.result.finalState.firstHalfAddedTimeSeconds >= 60);
  import_strict.default.ok(snapshot2.result.finalState.secondHalfAddedTimeSeconds >= 60);
  const events = snapshot2.result.events;
  const players = [
  ...
  const controls = events.filter((event) => event.type === "BALL_CONTROL" /* BALL_CONTROL */);
  const dribbles = events.filter((event) => event.type === "DRIBBLING" /* DRIBBLING */);
  const tackles = events.filter((event) => event.type === "TACKLE_WON" /* TACKLE_WON */);
  const crosses = events.filter((event) => event.type === "CROSS_NEAR_POST" /* CROSS_NEAR_POST */ || event.type === "CROSS_FAR_POST" /* CROSS_FAR_POST */);
  const blockedCrosses = events.filter((event) => event.type === "CROSS_BLOCKED" /* CROSS_BLOCKED */);
  const blockedShots = events.filter((event) => event.type === "SHOT_BLOCKED" /* SHOT_BLOCKED */);
  const rebounds = events.filter((event) => event.type === "REBOUND_WON" /* REBOUND_WON */);
  const followUpShots = events.filter((event) => event.detail?.reboundShot === true);
  reboundShots += followUpShots.length;
  import_strict.default.equal(players.reduce((sum, player) => sum + player.controls, 0), controls.length);
  import_strict.default.equal(players.reduce((sum, player) => sum + player.dribblesAttempted, 0), dribbles.length);
  import_strict.default.equal(players.reduce((sum, player) => sum + player.dribblesCompleted, 0), dribbles.length);
  import_strict.default.equal(players.reduce((sum, player) => sum + player.tacklesWon, 0), tackles.length);
  import_strict.default.equal(players.reduce((sum, player) => sum + player.crossesAttempted, 0), crosses.length + blockedCrosses.length);
  import_strict.default.equal(players.reduce((sum, player) => sum + player.crossesCompleted, 0), crosses.length);
  import_strict.default.equal(players.reduce((sum, player) => sum + player.shotsBlocked, 0), blockedShots.length);
  import_strict.default.equal(players.reduce((sum, player) => sum + player.reboundsWon, 0), rebounds.length);
  const teamStats = [snapshot2.result.stats.HOME, snapshot2.result.stats.AWAY];
  const interceptions = events.filter((event) => event.type === "MISPLACED_PASS" /* MISPLACED_PASS */);
  import_strict.default.equal(
    teamStats.reduce((sum, team) => sum + team.passesAttempted, 0),
    events.filter((event) => event.type === "PASS_COMPLETED" /* PASS_COMPLETED */).length + interceptions.length
  );
  import_strict.default.equal(teamStats.reduce((sum, team) => sum + team.dribblesCompleted, 0), dribbles.length);
  import_strict.default.equal(teamStats.reduce((sum, team) => sum + team.tacklesWon, 0), tackles.length);
  import_strict.default.equal(teamStats.reduce((sum, team) => sum + team.crossesAttempted, 0), crosses.length + blockedCrosses.length);
  import_strict.default.equal(teamStats.reduce((sum, team) => sum + team.blocks, 0), blockedShots.length + blockedCrosses.length);
  import_strict.default.equal(teamStats.reduce((sum, team) => sum + team.reboundsWon, 0), rebounds.length);
  rebounds.forEach((rebound) => {
    const sequenceId = rebound.detail?.sequenceId;
    import_strict.default.equal(typeof sequenceId, "string");
    const reboundIndex = events.indexOf(rebound);
    const source = events.slice(0, reboundIndex).findLast(
      (event) => event.detail?.sequenceId === sequenceId && (event.type === "SAVE" /* SAVE */ || event.type === "ONE_ON_ONE_SAVE" /* ONE_ON_ONE_SAVE */ || event.type === "SHOT_POST" /* SHOT_POST */ || event.type === "SHOT_BAR" /* SHOT_BAR */ || event.type === "SHOT_BLOCKED" /* SHOT_BLOCKED */)
    );
    import_strict.default.ok(source, "A rebound must reference a preceding loose-ball shot.");
  });
  followUpShots.forEach((followUp) => {
    ...
    import_strict.default.ok(buildup.some((event) => event.type === "REBOUND_WON" /* REBOUND_WON */ && event.playerId === followUp.playerId));
    import_strict.default.ok(buildup.some((event) => event.type === "BALL_CONTROL" /* BALL_CONTROL */ && event.playerId === followUp.playerId));
    import_strict.default.equal(followUp.detail?.assistEligible, false);
  });
  const goals = events.filter((event) => event.type === "GOAL" /* GOAL */ || event.type === "ONE_ON_ONE_GOAL" /* ONE_ON_ONE_GOAL */);
  goals.forEach((goal) => {
    const sequenceId = goal.detail?.sequenceId;
    import_strict.default.equal(typeof sequenceId, "string");
    import_strict.default.equal(typeof goal.detail?.goalkeeperId, "string");
    ...
    import_strict.default.ok(buildup.some(
      (event) => event.type === "BALL_CONTROL" /* BALL_CONTROL */ && event.playerId === attackingShooterId
    ), "A goal must follow the scorer's first touch in the same sequence.");
    import_strict.default.ok(buildup.every((event) => event.second === goal.second));
  ...
  snapshot2.spatial.visualCues.forEach((cue) => {
    import_strict.default.ok(authoritativeIds.has(cue.id));
    const midpoint = MatchEngineV2TrajectoryService.sampleCue(cue, cue.durationMs / 2);
    import_strict.default.ok(midpoint.x >= 0 && midpoint.x <= snapshot2.spatial.pitchWidth);
    import_strict.default.ok(midpoint.y >= 0 && midpoint.y <= snapshot2.spatial.pitchLength);
    import_strict.default.ok(midpoint.z >= 0);
  });
}
import_strict.default.ok(validatedGoalChains >= 20, `Only ${validatedGoalChains} goal chains were available.`);
import_strict.default.ok(reboundShots >= 3, `Only ${reboundShots} rebound shots were generated.`);
for (const type of trackedTypes) {
  import_strict.default.ok((totals.get(type) ?? 0) > 0, `No ${type} event was generated in ${totalMatches} matches.`);
}
