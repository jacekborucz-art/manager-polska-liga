...
// tests/MatchEngineV2RuleInvariantTests.ts
...
var CupDisciplineResolver = {
  ...
  resolveContact: ({
    ...
    const advantage2 = referee.advantageTendency / 100;
    ...
    const advantageChance = clamp(advantage2 * 0.55 + danger * 0.18 - strictness * 0.1, 0.03, 0.68);
    const advantagePlayed = initialType === "FOUL" /* FOUL */ && ctx.random(salt + 7) < advantageChance;
    const secondYellow2 = initialType === "YELLOW_CARD" /* YELLOW_CARD */ && Boolean(fouler && (ctx.state.yellowCards[fouler.id] ?? 0) >= 1);
    const type = advantagePlayed ? "ADVANTAGE_PLAYED" /* ADVANTAGE_PLAYED */ : secondYellow2 ? "RED_CARD" /* RED_CARD */ : initialType;
...
// tests/MatchEngineV2RuleInvariantTests.ts
var ordinaryGoalTypes = /* @__PURE__ */ new Set([
  "GOAL" /* GOAL */,
  "ONE_ON_ONE_GOAL" /* ONE_ON_ONE_GOAL */,
  "PENALTY_SCORED" /* PENALTY_SCORED */
]);
var assertCommonInvariants = (result) => {
  import_strict.default.equal(result.finalState.phase, "FINISHED");
  import_strict.default.equal(new Set(result.events.map((event) => event.id)).size, result.events.length, "Event ids must be unique.");
  result.events.forEach((event, index) => {
    import_strict.default.ok(event.second >= 0 && event.second <= result.finalState.second);
    if (index > 0) import_strict.default.ok(event.second >= result.events[index - 1].second, "Events must be chronological.");
  });
  ...
  import_strict.default.equal(result.homeScore, homeGoals);
  import_strict.default.equal(result.awayScore, awayGoals);
  import_strict.default.equal(result.stats.HOME.goals, result.homeScore);
  import_strict.default.equal(result.stats.AWAY.goals, result.awayScore);
  import_strict.default.equal(
    result.stats.HOME.corners + result.stats.AWAY.corners,
    result.events.filter((event) => event.type === "CORNER" /* CORNER */).length
  );
  const byId = new Map(result.events.map((event) => [event.id, event]));
  const injuryEvents = result.events.filter(
    (event) => event.type === "INJURY_LIGHT" /* INJURY_LIGHT */ || event.type === "INJURY_SEVERE" /* INJURY_SEVERE */
  );
  import_strict.default.equal(
  ...
  result.events.filter((event) => event.type === "MEDICAL_TREATMENT" /* MEDICAL_TREATMENT */).forEach((treatment) => {
    const injury = result.events.find((event) => event.id === treatment.detail?.sourceInjuryId);
    import_strict.default.ok(injury, "Medical treatment must reference an injury event.");
    import_strict.default.equal(injury?.playerId, treatment.playerId);
    import_strict.default.equal(injury?.second, treatment.second);
  });
  result.events.filter((event) => event.type === "ADVANTAGE_PLAYED" /* ADVANTAGE_PLAYED */).forEach((advantage2) => {
    import_strict.default.equal(advantage2.detail?.advantagePlayed, true);
    import_strict.default.ok(!result.events.some(
      (event) => event.second === advantage2.second && (event.type === "FREE_KICK" /* FREE_KICK */ || event.type === "FREE_KICK_DANGEROUS" /* FREE_KICK_DANGEROUS */ || event.type === "PENALTY_AWARDED" /* PENALTY_AWARDED */)
    ), "Advantage cannot also create a stopped-ball restart.");
  });
  ["HOME", "AWAY"].forEach((side) => {
    const substitutions = result.events.filter((event) => event.type === "SUBSTITUTION" /* SUBSTITUTION */ && event.side === side);
    import_strict.default.equal(new Set(substitutions.map((event) => event.playerId)).size, substitutions.length, "A substitute cannot enter twice.");
    import_strict.default.equal(new Set(substitutions.map((event) => event.secondaryPlayerId)).size, substitutions.length, "A player cannot leave twice.");
    substitutions.forEach((event, index) => {
      ...
      import_strict.default.ok(!earlierDepartures.has(event.playerId), "A substituted player cannot return from the bench.");
    });
  });
  const kickOffs = result.events.filter((event) => event.type === "KICK_OFF" /* KICK_OFF */);
  import_strict.default.ok(kickOffs.length >= 2, "Both regulation halves require an explicit kick-off.");
  import_strict.default.equal(kickOffs[0].second, 0);
  import_strict.default.equal(kickOffs[0].side, result.finalState.firstHalfKickOffSide);
  const halfTimeKickOff = kickOffs.find((event) => event.detail?.restartReason === "HALF_START");
  import_strict.default.ok(halfTimeKickOff, "The second-half kick-off is missing.");
  import_strict.default.equal(
    halfTimeKickOff?.second,
    45 * 60 + result.finalState.firstHalfAddedTimeSeconds
  );
  import_strict.default.notEqual(halfTimeKickOff?.side, result.finalState.firstHalfKickOffSide);
  result.events.filter((event) => event.type === "CORNER_TAKEN" /* CORNER_TAKEN */ || event.type === "GOAL_KICK" /* GOAL_KICK */).forEach((restart) => {
    const sourceId = restart.detail?.sourceEventId;
    import_strict.default.equal(typeof sourceId, "string", `Restart ${restart.id} has no source event.`);
    const source = byId.get(sourceId);
    import_strict.default.ok(source, `Restart ${restart.id} references a missing source event.`);
    import_strict.default.ok((source?.second ?? 0) <= restart.second);
    if (restart.type === "CORNER_TAKEN" /* CORNER_TAKEN */) {
      import_strict.default.equal(source?.type, "CORNER" /* CORNER */);
      import_strict.default.equal(source?.side, restart.side);
    } else {
      import_strict.default.notEqual(source?.side, restart.side);
    }
  });
  result.events.filter(
    (event) => event.type === "FREE_KICK" /* FREE_KICK */ || event.type === "FREE_KICK_DANGEROUS" /* FREE_KICK_DANGEROUS */ || event.type === "PENALTY_AWARDED" /* PENALTY_AWARDED */
  ).forEach((award) => {
    const sourceId = award.detail?.sourceContactId;
    import_strict.default.equal(typeof sourceId, "string");
    const source = byId.get(sourceId);
    import_strict.default.ok(source, "Every foul restart must reference a real contact.");
    import_strict.default.equal(source?.second, award.second);
    import_strict.default.ok(
      source?.type === "FOUL" /* FOUL */ || source?.type === "YELLOW_CARD" /* YELLOW_CARD */ || source?.type === "RED_CARD" /* RED_CARD */
    );
    import_strict.default.notEqual(source?.side, award.side, "A fouling team cannot receive its own restart.");
    const illegalTurnover = result.events.find(
      (event) => event.second === award.second && (event.type === "MISPLACED_PASS" /* MISPLACED_PASS */ || event.type === "TACKLE_WON" /* TACKLE_WON */)
    );
    import_strict.default.equal(illegalTurnover, void 0, "A foul and a normal turnover cannot resolve the same tick.");
  });
  result.events.filter((event) => event.type === "PENALTY_SCORED" /* PENALTY_SCORED */ || event.type === "PENALTY_MISSED" /* PENALTY_MISSED */).filter((event) => event.detail?.isShootout !== true).forEach((penalty) => {
    import_strict.default.equal(penalty.detail?.setPieceKind, "PENALTY");
    import_strict.default.equal(penalty.detail?.assistEligible, false);
    import_strict.default.ok(result.events.some(
      (event) => event.type === "PENALTY_AWARDED" /* PENALTY_AWARDED */ && event.second === penalty.second && event.detail?.sequenceId === penalty.detail?.sequenceId
    ));
  });
};
var disciplineSample = CupSampleMatchFactory.makeInput(2199, "EQUAL");
var disciplineState = createInitialCupRuntimeState(disciplineSample);
var disciplineFatigue = disciplineState.fatigue;
var disciplineHome = CupTeamProfileService.buildProfile(disciplineSample.home, disciplineFatigue, {});
var disciplineAway = CupTeamProfileService.buildProfile(disciplineSample.away, disciplineFatigue, {});
disciplineAway.activePlayers.forEach((player) => {
  disciplineState.yellowCards[player.id] = 1;
});
var secondYellow = CupDisciplineResolver.resolveContact({
  ctx: {
...
import_strict.default.equal(secondYellow?.type, "RED_CARD" /* RED_CARD */);
import_strict.default.equal(secondYellow?.detail?.secondYellow, true);
var advantage = CupDisciplineResolver.resolveContact({
  ctx: {
    input: disciplineSample,
    config: DEFAULT_CUP_ENGINE_CONFIG,
...
import_strict.default.equal(advantage?.type, "ADVANTAGE_PLAYED" /* ADVANTAGE_PLAYED */);
import_strict.default.equal(advantage?.detail?.advantagePlayed, true);
var leaguePenaltyAwards = 0;
var leagueFreeKicks = 0;
var leagueCorners = 0;
var leagueThrowIns = 0;
var leagueOffsides = 0;
var leagueDribbles = 0;
for (let seed = 1; seed <= 60; seed += 1) {
  ...
  import_strict.default.equal(result.decidedByPenalties, false);
  import_strict.default.equal(result.penaltyShootout, void 0);
  import_strict.default.ok(result.finalState.firstHalfAddedTimeSeconds >= 60 && result.finalState.firstHalfAddedTimeSeconds <= 420);
  import_strict.default.ok(result.finalState.secondHalfAddedTimeSeconds >= 60 && result.finalState.secondHalfAddedTimeSeconds <= 420);
  import_strict.default.equal(
  ...
  import_strict.default.equal(result.finalState.addedTimeSeconds % 5, 0);
  import_strict.default.equal(result.finalState.second, 90 * 60 + result.finalState.addedTimeSeconds);
  ...
  result.events.filter((event) => event.second >= firstHalfEnd && event.second < normalTimeEnd).forEach((event) => {
    import_strict.default.equal(
      event.minute,
  ...
  import_strict.default.ok(result.finalState.substitutionsUsed.HOME <= LEAGUE_MATCH_RULES_V2.maxSubstitutions);
  import_strict.default.ok(result.finalState.substitutionsUsed.AWAY <= LEAGUE_MATCH_RULES_V2.maxSubstitutions);
  leaguePenaltyAwards += result.events.filter((event) => event.type === "PENALTY_AWARDED" /* PENALTY_AWARDED */).length;
  leagueFreeKicks += result.events.filter(
    (event) => event.type === "FREE_KICK" /* FREE_KICK */ || event.type === "FREE_KICK_DANGEROUS" /* FREE_KICK_DANGEROUS */
  ).length;
  leagueCorners += result.events.filter((event) => event.type === "CORNER" /* CORNER */).length;
  leagueThrowIns += result.events.filter((event) => event.type === "THROW_IN" /* THROW_IN */).length;
  leagueOffsides += result.events.filter((event) => event.type === "OFFSIDE" /* OFFSIDE */).length;
  leagueDribbles += result.events.filter((event) => event.type === "DRIBBLING" /* DRIBBLING */).length;
}
var shootouts = 0;
var extraTimeMatches = 0;
for (let seed = 1; seed <= 24; seed += 1) {
  ...
  assertCommonInvariants(result);
  import_strict.default.ok(result.winner === "HOME" || result.winner === "AWAY");
  if (result.finalState.second > 90 * 60 + result.finalState.addedTimeSeconds) extraTimeMatches += 1;
  if (result.decidedByPenalties) {
    shootouts += 1;
    import_strict.default.equal(result.homeScore, result.awayScore);
    import_strict.default.ok(result.penaltyShootout && result.penaltyShootout.length >= 6);
    import_strict.default.notEqual(result.penaltyScore?.home, result.penaltyScore?.away);
  }
}
import_strict.default.ok(leagueFreeKicks > 0, "No foul-created free kick was generated.");
import_strict.default.ok(leaguePenaltyAwards > 0, "No foul-created penalty was generated.");
import_strict.default.ok(leagueCorners > 0, "No corner was generated.");
import_strict.default.ok(leagueThrowIns > 0, "No throw-in was generated.");
import_strict.default.ok(leagueOffsides > 0, "No offside was generated.");
import_strict.default.ok(leagueDribbles > 0, "No individual dribble was generated.");
import_strict.default.ok(extraTimeMatches > 0, "Knockout rules never reached extra time.");
import_strict.default.ok(shootouts > 0, "Knockout rules never reached a shootout.");
