...
var PolishCupDrawService = {
  ...
  getInitialParticipants: (clubs) => {
    ...
    const selectedTier4 = [...tier4Pool].sort((a, b) => a.id.localeCompare(b.id)).slice(0, 74).map((c) => c.id);
...
var individualInstructionProfile = (team, active, fatigue) => {
  ...
  const weightedAverage = (score) => instructed.reduce((sum, entry) => {
    ...
    const playerInfluence = clamp3((entry.player.attributes.mentality ?? 50) * 0.45 + (entry.player.attributes.workRate ?? 50) * 0.25 + liveFatigue * 0.3, 25, 100) / 100;
  ...
  return {
    buildUp: clamp3(1 + shortPassingTilt * 0.055 - tempoTilt * 0.018, 0.94, 1.07),
    midfieldControl: clamp3(1 + shortPassingTilt * 0.05 - directPassingTilt * 0.018, 0.94, 1.07),
    progression: clamp3(1 + tempoTilt * 0.05 + directPassingTilt * 0.03 + attackingTilt * 0.025, 0.93, 1.08),
    chanceCreation: clamp3(1 + attackingTilt * 0.055 + directPassingTilt * 0.02, 0.93, 1.09),
    finishing: clamp3(1 + attackingTilt * 0.026 + tempoTilt * 0.014, 0.96, 1.05),
    defensiveShape: clamp3(1 - attackingTilt * 0.03 + markingTilt * 0.046, 0.93, 1.08),
    pressing: clamp3(1 + pressingTilt * 0.105 + tempoTilt * 0.018, 0.96, 1.12),
    counterThreat: clamp3(1 + directPassingTilt * 0.06 + tempoTilt * 0.04, 0.94, 1.1),
    disciplineRisk: clamp3(1 + pressingTilt * 0.075 + markingTilt * 0.055 + Math.max(0, tempoTilt) * 0.028, 0.95, 1.13),
    lineHeightDelta: clamp3(pressingTilt * 3.4 + attackingTilt * 2, -2.8, 5.2)
  };
};
var CupTeamProfileService = {
  ...
  buildProfile: (team, fatigue, redCards, injuries = {}) => {
    ...
    return {
      ...
      lineHeight: clamp3(shape.lineHeight + individual.lineHeightDelta, 20, 88),
...
var CupInjuryResolver = {
  ...
  maybeCreateInjury: ({
    ...
    const injuryChance = clamp3(15e-4 + pitchRisk + weatherRisk + fatigueRisk + contactIntensity * 0.01, 5e-4, 0.035);
...
var buildIntent = (ctx, attacking) => {
  ...
  const individualTempo = clamp3(individualTilt.tempo / activeCount, -0.25, 0.25);
  const individualRisk = clamp3(individualTilt.risk / activeCount, -0.25, 0.25);
  const individualVerticality = clamp3(individualTilt.verticality / activeCount, -0.25, 0.25);
  ...
  return {
    ...
    tempo: clamp3(tempo + intensityTempo, 0.24, 0.8),
    verticality: clamp3(verticality, 0.22, 0.8),
    widthUse
  };
};
...
var CupShadowSimulationService = {
  simulateFromMatchContext: (ctx, options) => {
    ...
    const input = options.halfTimeTalks ? { ...adapted.input, halfTimeTalks: options.halfTimeTalks } : adapted.input;
    const result = CupMatchEngineV2.simulate(input);
    const matchSummary = CupMatchReportAdapter.fromMatchContext(ctx, input, result, {
  ...
  createLiveMatch: (ctx, options) => {
    ...
    return { live, ctx, options, diagnostics: adapted.diagnostics };
  },
...
var audit = CupShadowAuditService.run(makeCases());
...
import_strict.default.equal(audit.summary.matches, 256);
