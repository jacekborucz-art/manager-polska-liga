...
var CupDisciplineResolver = {
  ...
  resolveContact: ({
    ...
    const foulChance = clamp(
      (0.026 + defending.disciplineRisk * 7e-4 + defending.pressing * 32e-5 + danger * 0.045 + strictness * 0.018 - consistencyNoise) * ctx2.state.coachEffects[defending.side].foulMultiplier,
      4e-3,
      0.16
    );
    ...
    const initialType = ctx2.random(salt + 3) < redChance ? "RED_CARD" /* RED_CARD */ : ctx2.random(salt + 4) < yellowChance ? "YELLOW_CARD" /* YELLOW_CARD */ : "FOUL" /* FOUL */;
    ...
    const advantagePlayed = initialType === "FOUL" /* FOUL */ && ctx2.random(salt + 7) < advantageChance;
    const secondYellow = initialType === "YELLOW_CARD" /* YELLOW_CARD */ && Boolean(fouler && (ctx2.state.yellowCards[fouler.id] ?? 0) >= 1);
    ...
    return {
      ...
      minute: CupMatchClockService.eventMinute(ctx2.state, ctx2.config),
      side: defending.side,
      type,
      playerId: fouler?.id,
...
var CupInjuryResolver = {
  ...
  maybeCreateInjury: ({
    ...
    const injuryChance = clamp(
      (15e-4 + pitchRisk + weatherRisk + fatigueRisk + contactIntensity * 0.01) * ctx2.state.coachEffects[profile.side].injuryMultiplier,
      5e-4,
      0.035
    );
    const eligiblePlayers = profile.activePlayers.filter((player) => !ctx2.state.injuries[player.id]);
    if (ctx2.random(salt) > injuryChance || eligiblePlayers.length === 0) return null;
...
var baseEvent = (ctx2, sequenceId, suffix) => ({
...
var CupActionSequenceService = {
  ...
  createProgressionEvents: ({
    ...
    if (decision.action === "DRIBBLE") {
      const dribbleBase = baseEvent(ctx2, sequenceId, "carry");
      return [{
        ...dribbleBase,
        side: attacking.side,
    ...
    const eventBase = baseEvent(ctx2, sequenceId, "delivery");
    ...
    const delivery = crossingDecision ? {
      ...eventBase,
      side: attacking.side,
      type: ctx2.random(281) < 0.58 ? "CROSS_NEAR_POST" /* CROSS_NEAR_POST */ : "CROSS_FAR_POST" /* CROSS_FAR_POST */,
      zone: toZone,
      pattern: intent.pattern,
      playerId: passer.id,
    ...
    const controlBase = baseEvent(ctx2, sequenceId, "control");
    ...
    if (toZone !== "GK" && toZone !== "DEFENSE" && ctx2.random(282) < dribbleChance) {
      const dribbleBase = baseEvent(ctx2, sequenceId, "dribble");
      events.push({
        ...dribbleBase,
        side: attacking.side,
        type: "DRIBBLING" /* DRIBBLING */,
  ...
  createTurnoverEvent: ({
    ...
    const tackle = Boolean(tackler && loser && ctx2.random(283) < tackleChance);
    const eventBase = baseEvent(ctx2, sequenceId, tackle ? "tackle" : "interception");
    return {
      ...eventBase,
      side: defending.side,
      ...
      text: tackler && loser ? tackle ? `${tackler.lastName} wygrywa pojedynek z zawodnikiem ${loser.lastName}.` : `${tackler.lastName} przechwytuje niedok\u0142adne podanie zawodnika ${loser.lastName}.` : `${defending.side === "HOME" ? ctx2.input.home.name : ctx2.input.away.name} odbiera pi\u0142k\u0119.`,
      detail: {
        ...eventBase.detail,
  ...
  createChanceBuildup: ({
    ...
    if (creator && creator.id !== shooter.id) {
      const deliveryBase = baseEvent(ctx2, sequenceId, "chance_delivery");
      const cross = intent.pattern === "WING_PLAY" || isWideZone(ctx2.state.ballZone);
      events.push({
        ...deliveryBase,
        side: attacking.side,
        type: cross ? ctx2.random(284) < 0.58 ? "CROSS_NEAR_POST" /* CROSS_NEAR_POST */ : "CROSS_FAR_POST" /* CROSS_FAR_POST */ : "PASS_COMPLETED" /* PASS_COMPLETED */,
        zone: chance.zone,
    ...
    const controlBase = baseEvent(ctx2, sequenceId, "chance_control");
    events.push({
      ...controlBase,
      side: attacking.side,
      type: "BALL_CONTROL" /* BALL_CONTROL */,
    ...
    if (chance.kind === "ONE_ON_ONE" || intent.pattern === "COUNTER") {
      const dribbleBase = baseEvent(ctx2, sequenceId, "chance_dribble");
      events.push({
        ...dribbleBase,
        side: attacking.side,
        type: "DRIBBLING" /* DRIBBLING */,
...
var CupSetPieceResolver = {
  ...
  createSetPieceChance: ({
    ...
    if (ctx2.random(503) > chanceProbability) return null;
    ...
    return {
      ...
      pressure: ctx2.state.pressure[attacking.side],
      angle: kind === "FREE_KICK_DIRECT" ? 0.44 : 0.62,
      distance: kind === "PENALTY" ? 11 : kind === "FREE_KICK_DIRECT" ? 22 : 9
    };
  },
  eventForAward: (ctx2, side, kind) => ({
    id: `cupv2_setpiece_award_${ctx2.state.second}_${kind}`,
    second: ctx2.state.second,
    minute: CupMatchClockService.eventMinute(ctx2.state, ctx2.config),
    side,
    ...
    pattern: "SET_PIECE",
    text: `${side === "HOME" ? ctx2.input.home.name : ctx2.input.away.name} otrzymuje sta\u0142y fragment gry.`
  })
};
...
var weatherShotPenalty = (ctx2) => Math.max(0, (ctx2.input.environment.weather?.weatherIntensity ?? 0) * 15.5);
var registerShotStats = (ctx2, attacking, defending, shot) => {
...
var resolveSetPieceDelivery = ({
  ...
  const events = [];
  const chance = CupSetPieceResolver.createSetPieceChance({ ctx: ctx2, attacking, defending, kind });
  if (!chance) {
    return {
  ...
  const shot = CupShotResolver.resolveShot({
    ...
    weatherPenalty: weatherShotPenalty(ctx2),
    scoreDiff: attacking.side === "HOME" ? ctx2.state.homeScore - ctx2.state.awayScore : ctx2.state.awayScore - ctx2.state.homeScore,
    roll: (salt) => ctx2.random(500 + salt)
  });
  registerShotStats(ctx2, attacking, defending, shot);
  ...
  if (shot.corner) {
    const cornerAward = CupSetPieceResolver.eventForAward(ctx2, attacking.side, "CORNER");
...
var resolveFoulRestart = ({
  ...
  const kind = ctx2.state.ballZone === "BOX" ? "PENALTY" : ctx2.state.ballZone === "WIDE_LEFT" || ctx2.state.ballZone === "WIDE_RIGHT" ? "FREE_KICK_WIDE" : ctx2.state.ballZone === "FINAL_THIRD" && ctx2.random(295) < 0.46 ? "FREE_KICK_DIRECT" : "FREE_KICK_WIDE";
  const award = CupSetPieceResolver.eventForAward(ctx2, attacking.side, kind);
  award.detail = { ...award.detail ?? {}, sequenceId, sourceContactId, setPieceKind: kind };
  const delivery = resolveSetPieceDelivery({ ctx: ctx2, attacking, defending, intent, sequenceId, kind, sourceContactId });
...
var CupActionBuilder = {
  ...
  simulateTick: (ctx2) => {
    ...
    if (ctx2.state.possessionReason === "CORNER_DELIVERY") {
      const sequenceId2 = `cupv2_corner_delivery_${ctx2.state.second}_${ctx2.state.possession}`;
      return resolveSetPieceDelivery({ ctx: ctx2, attacking, defending, intent, sequenceId: sequenceId2, kind: "CORNER" });
    }
    const scoreDiff = attacking.side === "HOME" ? ctx2.state.homeScore - ctx2.state.awayScore : ctx2.state.awayScore - ctx2.state.homeScore;
    const footballMinute = CupMatchClockService.eventMinute(ctx2.state, ctx2.config) - 1;
    const halfTimeResponseDecay = ctx2.state.phase === "SECOND_HALF" ? clamp(1 - Math.max(0, footballMinute - 45) / 52, 0.12, 1) : 0;
    const halfTimeResponse = ctx2.state.halfTimeResponse[attacking.side] * halfTimeResponseDecay;
    const attackingCoach = ctx2.state.coachEffects[attacking.side];
    const defendingCoach = ctx2.state.coachEffects[defending.side];
    ...
    const actionCadence = clamp(
      (0.19 + intent.tempo * 0.08 + intent.risk * 0.05 + Math.abs(ctx2.state.momentum) * 45e-5 + attackingCoach.initiativeModifier * 0.55) * leadingGameControlDampener * trailingUrgency,
      0.08,
      0.34
    );
    ...
    const decision = CupPlayerDecisionService.selectPossessionDecision({
      ...
      zone: ctx2.state.ballZone,
      pattern: intent.pattern,
      fatigue: ctx2.state.fatigue,
      currentCarrierId: ctx2.state.ballCarrierId,
      instructions: attacking.side === "HOME" ? ctx2.input.home.instructions : ctx2.input.away.instructions,
      roll: ctx2.random
    });
    const sequenceId = `cupv2_sequence_${ctx2.state.second}_${attacking.side}`;
    ...
    if (!progressed) {
      ...
      return {
        ...
        events: [{
          ...
          text: eventType === "THROW_IN" /* THROW_IN */ ? `${decision.passer?.lastName ?? (attacking.side === "HOME" ? ctx2.input.home.name : ctx2.input.away.name)} wznawia gr\u0119 z autu.` : `${attacking.side === "HOME" ? ctx2.input.home.name : ctx2.input.away.name} utrzymuje pi\u0142k\u0119, ale akcja zwalnia.`,
    ...
    if (!chance) {
      ...
      if (ctx2.random(26) < cornerChance) {
        ...
        if (decision.passer && decision.presser && (ctx2.state.ballZone === "WIDE_LEFT" || ctx2.state.ballZone === "WIDE_RIGHT" || intent.pattern === "WING_PLAY")) {
          events.push({
    ...
    registerShotStats(ctx2, attacking, defending, shot);
    const shotEventId = `cupv2_shot_${ctx2.state.second}`;
    events.push({
      id: shotEventId,
      second: ctx2.state.second,
      minute: CupMatchClockService.eventMinute(ctx2.state, ctx2.config),
    ...
    const followUpChance = rebound?.side === attacking.side && ctx2.random(292) < 0.16;
    if (rebound && followUpChance) {
      ...
      const reboundShot = CupShotResolver.resolveShot({
        ...
        attacking,
        defending,
        shooterFatigue: ctx2.state.fatigue[rebound.player.id] ?? rebound.player.condition,
        keeperFatigue: defending.goalkeeper ? ctx2.state.fatigue[defending.goalkeeper.id] ?? defending.goalkeeper.condition : 80,
        weatherPenalty: weatherShotPenalty(ctx2),
        scoreDiff,
        ...
        roll: (salt) => ctx2.random(400 + salt)
      });
      registerShotStats(ctx2, attacking, defending, reboundShot);
...
var CupShadowSimulationService = {
  simulateFromMatchContext: (ctx2, options) => {
    ...
    const matchSummary = CupMatchReportAdapter.fromMatchContext(ctx2, adapted.input, result, {
  ...
  tickLiveMatch: (handle2, targetSecond) => {
    CupMatchEngineV2.advanceLiveMatch(handle2.live, targetSecond);
    const result = CupMatchEngineV2.snapshotLiveMatch(handle2.live);
    const matchSummary = CupMatchReportAdapter.fromMatchContext(handle2.ctx, handle2.live.input, result, {
      userTeamId: handle2.options.userSide === "AWAY" ? handle2.ctx.awayClub.id : handle2.ctx.homeClub.id
    });
    const summary = summarizeResult(handle2.live.input, result);
    return {
      input: handle2.live.input,
      result,
      matchSummary,
      diagnostics: handle2.diagnostics,
      summary,
      legacy: handle2.options.legacy,
      diff: handle2.options.legacy ? compareWithLegacy(summary, handle2.options.legacy) : void 0,
      initialLineup: handle2.live.initialLineup
    };
  }
};
...
var sample = CupSampleMatchFactory.makeInput(11, "EQUAL");
var ctx = {
  fixture: {
  ...
  homeClub: makeClub(sample.home.clubId, "Olimpia Testowa", 55, 9e3, 60),
  awayClub: makeClub(sample.away.clubId, "Rak\xF3w Testowy", 55, 18e3, 60),
  homePlayers: sample.home.players,
  awayPlayers: sample.away.players,
  homeAdvantage: true,
  competition: "POLISH_CUP" /* POLISH_CUP */
};
var handle = CupShadowSimulationService.createLiveMatch(ctx, {
...
var kickoffReport = CupShadowSimulationService.tickLiveMatch(handle, 0);
import_strict.default.equal(kickoffReport.result.finalState.second, 0);
import_strict.default.equal(kickoffReport.result.events.length, 0, "Na minucie 0 mecz nie powinien mie\u0107 \u017Cadnych zdarze\u0144 zasymulowanych z g\xF3ry");
import_strict.default.equal(kickoffReport.result.winner, void 0, "Zwyci\u0119zca nie mo\u017Ce by\u0107 znany na starcie");
var reportAt20 = CupShadowSimulationService.tickLiveMatch(handle, 20 * 60);
import_strict.default.equal(reportAt20.result.finalState.second, 20 * 60);
var eventsBefore20 = [...reportAt20.result.events];
handle.live.input.home.instructions = { ...handle.live.input.home.instructions, tempo: "FAST", mindset: "OFFENSIVE" };
var startingPlayerId = handle.live.input.home.lineup.startingXI.find((id) => Boolean(id));
var benchPlayerId = handle.live.input.home.lineup.bench[0];
import_strict.default.ok(startingPlayerId, "Test wymaga zawodnika w sk\u0142adzie startowym");
import_strict.default.ok(benchPlayerId, "Test wymaga zawodnika na \u0142awce");
var substitutionApplied = CupMatchEngineV2.applyManualSubstitution(handle.live, "HOME", startingPlayerId, benchPlayerId);
import_strict.default.equal(substitutionApplied, true, "Zmiana powinna si\u0119 uda\u0107 dla zawodnika ze sk\u0142adu i z \u0142awki");
import_strict.default.ok(!handle.live.input.home.lineup.startingXI.includes(startingPlayerId), "Schodz\u0105cy zawodnik nie mo\u017Ce zosta\u0107 w sk\u0142adzie");
import_strict.default.ok(handle.live.input.home.lineup.startingXI.includes(benchPlayerId), "Wchodz\u0105cy zawodnik musi trafi\u0107 do sk\u0142adu");
import_strict.default.ok(handle.live.input.home.lineup.bench.includes(startingPlayerId), "Schodz\u0105cy zawodnik trafia na \u0142awk\u0119 (techniczne miejsce), ale to nie znaczy, \u017Ce mo\u017Ce wr\xF3ci\u0107");
var secondPlayerToBringOn = handle.live.input.home.lineup.bench.find((id) => id !== startingPlayerId);
import_strict.default.ok(secondPlayerToBringOn, "Test wymaga drugiego dost\u0119pnego zawodnika na \u0142awce");
var illegalReturnAttempt = CupMatchEngineV2.applyManualSubstitution(handle.live, "HOME", secondPlayerToBringOn, startingPlayerId);
import_strict.default.equal(illegalReturnAttempt, false, "Zawodnik, kt\xF3ry ju\u017C zszed\u0142 z boiska, nie mo\u017Ce zosta\u0107 wprowadzony ponownie");
import_strict.default.ok(!handle.live.input.home.lineup.startingXI.includes(startingPlayerId), "Zablokowana pr\xF3ba nie mo\u017Ce jednak wpu\u015Bci\u0107 go na boisko");
var redCardedPlayerId = handle.live.input.home.lineup.startingXI.find((id) => Boolean(id));
import_strict.default.ok(redCardedPlayerId, "Test wymaga zawodnika w sk\u0142adzie do oznaczenia czerwon\u0105 kartk\u0105");
handle.live.state.redCards[redCardedPlayerId] = true;
var remainingBenchPlayerId = handle.live.input.home.lineup.bench.find((id) => id !== startingPlayerId);
import_strict.default.ok(remainingBenchPlayerId, "Test wymaga wolnego zawodnika na \u0142awce");
var substitutionForRedCardAttempt = CupMatchEngineV2.applyManualSubstitution(handle.live, "HOME", redCardedPlayerId, remainingBenchPlayerId);
import_strict.default.equal(substitutionForRedCardAttempt, false, "Nie mo\u017Cna wprowadzi\u0107 zmiany za zawodnika z czerwon\u0105 kartk\u0105");
import_strict.default.ok(handle.live.input.home.lineup.startingXI.includes(redCardedPlayerId), "Zawodnik z czerwon\u0105 kartk\u0105 zostaje na swoim miejscu w sk\u0142adzie (dru\u017Cyna gra w os\u0142abieniu)");
var reportAt40 = CupShadowSimulationService.tickLiveMatch(handle, 40 * 60);
import_strict.default.equal(reportAt40.result.finalState.second, 40 * 60);
var eventsBefore20AfterFurtherSim = reportAt40.result.events.filter((event) => event.second < 20 * 60);
...
eventsBefore20.filter((event) => event.second < 20 * 60).forEach((event, index) => {
  import_strict.default.equal(eventsBefore20AfterFurtherSim[index]?.id, event.id, `Zdarzenie #${index} sprzed zmiany musi zosta\u0107 identyczne (id: ${event.id})`);
});
var manualSubEvent = reportAt40.result.events.find((event) => event.id.startsWith("cupv2_manual_substitution_"));
import_strict.default.ok(manualSubEvent, "R\u0119czna zmiana zawodnika musi wygenerowa\u0107 zdarzenie w logu meczu");
...
import_strict.default.ok(finalResult, "Mecz musi si\u0119 kiedy\u015B zako\u0144czy\u0107 (winner ustalony albo karne)");
import_strict.default.ok(finalResult.winner === "HOME" || finalResult.winner === "AWAY", "Mecz pucharowy musi mie\u0107 zwyci\u0119zc\u0119");
import_strict.default.ok(Object.keys(finalResult.playerStats.HOME).length > 0, "Statystyki graczy HOME musz\u0105 by\u0107 policzone na koniec");
import_strict.default.ok(Object.keys(finalResult.playerStats.AWAY).length > 0, "Statystyki graczy AWAY musz\u0105 by\u0107 policzone na koniec");
