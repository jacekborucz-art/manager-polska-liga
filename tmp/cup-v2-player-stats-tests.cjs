...
var resolveSetPieceDelivery = ({
  ...
  events2.push(...CupActionSequenceService.createChanceBuildup({
    ctx,
    attacking,
  ...
  return {
    ...
    restartSourceEventId: shot.corner ? events2.at(-1)?.id : setPieceShotId,
  ...
  simulateTick: (ctx) => {
    ...
    if (ctx.random(21) < turnoverProbability) {
      ...
      if (contact) {
        ...
        return {
          ...restart,
          events: [...events2, ...restart.events]
        };
      }
      events2.push(CupActionSequenceService.createTurnoverEvent({
        ctx,
        attacking,
        defending,
        intent,
    ...
    if (nextZone !== "FINAL_THIRD" && nextZone !== "BOX" && nextZone !== "WIDE_LEFT" && nextZone !== "WIDE_RIGHT") {
      events2.push(...CupActionSequenceService.createProgressionEvents({
        ctx,
        attacking,
        defending,
        intent,
    ...
    if (!chance) {
      ...
      const cornerEvent = events2.find((event) => event.type === "CORNER" /* CORNER */);
      return {
  ...
  aggregate: ({
    ...
    events2.forEach((event) => {
      ...
      if (event.type === "PASS_COMPLETED" /* PASS_COMPLETED */ && event.playerId) {
        const passer = ensureStats(stats2, lookup, event.playerId, side);
        if (passer) {
      ...
      if (event.type === "MISPLACED_PASS" /* MISPLACED_PASS */) {
        const winner = event.playerId ? ensureStats(stats2, lookup, event.playerId, side) : void 0;
        const loser = event.secondaryPlayerId ? ensureStats(stats2, lookup, event.secondaryPlayerId) : void 0;
      ...
      if (event.type === "BALL_CONTROL" /* BALL_CONTROL */ && event.playerId) {
        const receiver = ensureStats(stats2, lookup, event.playerId, side);
        if (receiver) receiver.controls += 1;
      }
      if (event.type === "DRIBBLING" /* DRIBBLING */ && event.playerId) {
        const dribbler = ensureStats(stats2, lookup, event.playerId, side);
        if (dribbler) {
      ...
      if (event.type === "TACKLE_WON" /* TACKLE_WON */) {
        const tackler = event.playerId ? ensureStats(stats2, lookup, event.playerId, side) : void 0;
        const dispossessed = event.secondaryPlayerId ? ensureStats(stats2, lookup, event.secondaryPlayerId) : void 0;
        if (tackler) {
      ...
      if (event.type === "CROSS_NEAR_POST" /* CROSS_NEAR_POST */ || event.type === "CROSS_FAR_POST" /* CROSS_FAR_POST */) {
        const crosser = event.playerId ? ensureStats(stats2, lookup, event.playerId, side) : void 0;
        if (crosser) {
...
var applyEventToState = (state, event) => {
  ...
  if (eventType === "FREE_KICK" /* FREE_KICK */ || eventType === "FREE_KICK_DANGEROUS" /* FREE_KICK_DANGEROUS */) stats2.freeKicks += 1;
  if (eventType === "PENALTY_AWARDED" /* PENALTY_AWARDED */) stats2.penalties += 1;
  if (eventType === "FOUL" /* FOUL */) stats2.fouls += 1;
  if (eventType === "ADVANTAGE_PLAYED" /* ADVANTAGE_PLAYED */) stats2.fouls += 1;
  ...
  if (eventType === "PASS_COMPLETED" /* PASS_COMPLETED */) {
    stats2.passesAttempted += 1;
    stats2.passesCompleted += 1;
  }
  if (eventType === "MISPLACED_PASS" /* MISPLACED_PASS */) {
    opponentStats.passesAttempted += 1;
    stats2.turnoversWon += 1;
    opponentStats.turnoversLost += 1;
  }
  if (eventType === "DRIBBLING" /* DRIBBLING */) {
    stats2.dribblesAttempted += 1;
    if (event.detail?.succeeded !== false) stats2.dribblesCompleted += 1;
  }
  ...
  if (eventType === "CROSS_NEAR_POST" /* CROSS_NEAR_POST */ || eventType === "CROSS_FAR_POST" /* CROSS_FAR_POST */) {
    stats2.crossesAttempted += 1;
    if (event.detail?.completed !== false) stats2.crossesCompleted += 1;
  }
  ...
  if (eventType === "REBOUND_WON" /* REBOUND_WON */) stats2.reboundsWon += 1;
};
...
var buildResult = (input, state, initialLineup, shootout2) => {
  ...
  const winner = shootout2?.winner ?? (state.homeScore > state.awayScore ? "HOME" : state.awayScore > state.homeScore ? "AWAY" : void 0);
  return {
    homeScore: state.homeScore,
    awayScore: state.awayScore,
    winner,
    decidedByPenalties: Boolean(shootout2),
    penaltyScore: shootout2?.penaltyScore,
    penaltyShootout: shootout2?.attempts,
    stats: state.stats,
    playerStats,
    events: state.events,
    finalState: state
...
var cloneResult = (result) => ({
  ...
  playerStats: {
    HOME: Object.fromEntries(Object.entries(result.playerStats.HOME).map(([id, stats2]) => [id, { ...stats2 }])),
    AWAY: Object.fromEntries(Object.entries(result.playerStats.AWAY).map(([id, stats2]) => [id, { ...stats2 }]))
  },
