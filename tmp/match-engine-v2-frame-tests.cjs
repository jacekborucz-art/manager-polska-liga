...
// tests/MatchEngineV2FrameControllerTests.ts
...
var nearestOpponentDistance = (spatial, side, playerId2) => {
  const point2 = spatial.players[playerId2];
  if (!point2) return 8;
...
var spatialShotWeight = (spatial, side, playerId2) => {
  const point2 = spatial?.players[playerId2];
  ...
  const pressure = nearestOpponentDistance(spatial, side, playerId2);
...
var baseEvent = (ctx, sequenceId2, suffix) => ({
  id: `${sequenceId2}_${suffix}`,
  second: ctx.state.second,
  minute: CupMatchClockService.eventMinute(ctx.state, ctx.config),
  detail: { sequenceId: sequenceId2 }
});
...
var CupActionSequenceService = {
  ...
  createProgressionEvents: ({
    ...
    if (decision.action === "DRIBBLE") {
      const dribbleBase = baseEvent(ctx, sequenceId2, "carry");
      return [{
        ...dribbleBase,
        side: attacking.side,
    ...
    const eventBase = baseEvent(ctx, sequenceId2, "delivery");
    ...
    const controlBase = baseEvent(ctx, sequenceId2, "control");
    ...
    if (toZone !== "GK" && toZone !== "DEFENSE" && ctx.random(282) < dribbleChance) {
      const dribbleBase = baseEvent(ctx, sequenceId2, "dribble");
      events.push({
        ...dribbleBase,
        side: attacking.side,
        type: "DRIBBLING" /* DRIBBLING */,
  ...
  createTurnoverEvent: ({
    ...
    const eventBase = baseEvent(ctx, sequenceId2, tackle ? "tackle" : "interception");
    return {
      ...eventBase,
      side: defending.side,
  ...
  createChanceBuildup: ({
    ...
    if (creator && creator.id !== shooter.id) {
      const deliveryBase = baseEvent(ctx, sequenceId2, "chance_delivery");
    ...
    const controlBase = baseEvent(ctx, sequenceId2, "chance_control");
    events.push({
      ...controlBase,
      side: attacking.side,
      type: "BALL_CONTROL" /* BALL_CONTROL */,
    ...
    if (chance.kind === "ONE_ON_ONE" || intent.pattern === "COUNTER") {
      const dribbleBase = baseEvent(ctx, sequenceId2, "chance_dribble");
      events.push({
        ...dribbleBase,
        side: attacking.side,
        type: "DRIBBLING" /* DRIBBLING */,
...
var distanceToSegment = (point2, start, end) => {
  const segmentX = end.x - start.x;
  const segmentY = end.y - start.y;
  const lengthSquared = segmentX * segmentX + segmentY * segmentY;
  if (lengthSquared < 1e-3) return Math.hypot(point2.x - start.x, point2.y - start.y);
  const projection = clamp(
    ((point2.x - start.x) * segmentX + (point2.y - start.y) * segmentY) / lengthSquared,
    0,
    1
  );
  return Math.hypot(
    point2.x - (start.x + segmentX * projection),
    point2.y - (start.y + segmentY * projection)
  );
};
...
var CupPlayerDecisionService = {
  ...
  selectPossessionDecision: ({
    ...
    const receiver = selectWeighted(receivers, (player) => weightedScore(player.attributes, {
      ...
      workRate: 0.09,
      strength: 0.07
    }) * receiverRoleWeight(player, zone, pattern) * receiverConnectionWeight(passer, player, zone, pattern) * spatialReceiverWeight(spatialConnection(spatial, attacking.side, passer, player)) * fitnessFactor(player, fatigue), roll(272));
    ...
    const presser = selectWeighted(defending.outfieldPlayers, (player) => weightedScore(player.attributes, {
      ...
      strength: 0.12,
      mentality: 0.13
    }) * fitnessFactor(player, fatigue) * (passerPoint && spatial?.players[player.id] ? clamp(18 / Math.max(2, Math.hypot(
...
var resolveSetPieceDelivery = ({
  ...
  const setPieceShotId = `${sequenceId2}_set_piece_shot`;
  ...
  if (shot.corner) {
    ...
    cornerAward.detail = { sequenceId: sequenceId2, sourceShotId: setPieceShotId };
...
var resolveFoulRestart = ({
  ...
  award.detail = { ...award.detail ?? {}, sequenceId: sequenceId2, sourceContactId, setPieceKind: kind };
  const delivery = resolveSetPieceDelivery({ ctx, attacking, defending, intent, sequenceId: sequenceId2, kind, sourceContactId });
...
var CupActionBuilder = {
  ...
  simulateTick: (ctx) => {
    ...
    if (ctx.state.possessionReason === "CORNER_DELIVERY") {
      const sequenceId3 = `cupv2_corner_delivery_${ctx.state.second}_${ctx.state.possession}`;
      return resolveSetPieceDelivery({ ctx, attacking, defending, intent, sequenceId: sequenceId3, kind: "CORNER" });
    }
    ...
    const sequenceId2 = `cupv2_sequence_${ctx.state.second}_${attacking.side}`;
    ...
    if (!progressed) {
      ...
      return {
        ...
        events: [{
          ...
          detail: eventType === "THROW_IN" /* THROW_IN */ ? { sequenceId: sequenceId2, restartReason: "THROW_IN" } : { sequenceId: sequenceId2 }
        }]
      };
    }
      ...
      if (ctx.random(26) < cornerChance) {
        ...
        if (decision.passer && decision.presser && (ctx.state.ballZone === "WIDE_LEFT" || ctx.state.ballZone === "WIDE_RIGHT" || intent.pattern === "WING_PLAY")) {
          events.push({
            ...
            detail: { sequenceId: sequenceId2, completed: false }
          });
        }
        events.push({
    ...
    if (rebound && followUpChance) {
      const controlId = `${sequenceId2}_rebound_control`;
      events.push({
        id: controlId,
        second: ctx.state.second,
        minute: CupMatchClockService.eventMinute(ctx.state, ctx.config),
        ...
        detail: { sequenceId: sequenceId2, reboundControl: true }
      });
      const reboundChance = {
        side: attacking.side,
        kind: "HALF_CHANCE",
      ...
      finalShotEventId = `${sequenceId2}_rebound_shot`;
      events.push({
        id: finalShotEventId,
        second: ctx.state.second,
        minute: CupMatchClockService.eventMinute(ctx.state, ctx.config),
...
var ensureStats = (stats, lookup, playerId2, fallbackSide) => {
  const found = lookup.get(playerId2);
  if (!found && !fallbackSide) return void 0;
  const side = found?.side ?? fallbackSide;
  const player = found?.player;
  if (!player) return void 0;
  if (!stats[side][playerId2]) {
    stats[side][playerId2] = createPlayerStats(player, side, false);
  }
  return stats[side][playerId2];
};
...
var applyPlayerEventToState = (state, eventType, playerId2) => {
  if (!playerId2) return;
  if (eventType === "YELLOW_CARD" /* YELLOW_CARD */) {
    state.yellowCards[playerId2] = (state.yellowCards[playerId2] ?? 0) + 1;
  }
  if (eventType === "RED_CARD" /* RED_CARD */) {
    state.redCards[playerId2] = true;
  }
  if (eventType === "INJURY_LIGHT" /* INJURY_LIGHT */) {
    state.injuries[playerId2] = "LIGHT";
    state.fatigue[playerId2] = Math.min(state.fatigue[playerId2] ?? 58, 58);
  }
  if (eventType === "INJURY_SEVERE" /* INJURY_SEVERE */) {
    state.injuries[playerId2] = "SEVERE";
    state.fatigue[playerId2] = Math.min(state.fatigue[playerId2] ?? 34, 34);
  }
};
...
var velocityBetween = (start, end, elapsedSeconds) => ({
...
var createTeamPlayers = (team, side) => {
  ...
  const players = {};
  team.lineup.startingXI.forEach((playerId2, index) => {
    if (!playerId2) return;
    const slot = tactic.slots[index] ?? tactic.slots[0];
    const player = team.players.find((item) => item.id === playerId2);
...
var placeInitialKickOff = (players, live) => {
  ...
  const team = side === "HOME" ? live.input.home : live.input.away;
  const candidates = team.lineup.startingXI.filter((playerId2) => Boolean(playerId2)).map((playerId2) => players[playerId2]).filter((player) => Boolean(player) && player.role !== "GK");
...
var synchronizeLineups = (spatial, live) => {
  ["HOME", "AWAY"].forEach((side) => {
    ...
    const missingActivePlayer = [...active].filter((playerId2) => !spatial.players[playerId2]);
    if (!missingActivePlayer.length) return;
    const rebuilt = createTeamPlayers(team, side);
    missingActivePlayer.forEach((playerId2) => {
      const player = rebuilt[playerId2];
      if (player) spatial.players[playerId2] = player;
    });
  });
};
...
var carryTarget = (start, event, side, fallbackZone) => {
...
var projectNewEvents = (spatial, live) => {
  ...
  newEvents.forEach((event) => {
    ...
    const previousCue = cues2.at(-1) ?? spatial.visualCues.at(-1);
    const sequenceId2 = typeof event.detail?.sequenceId === "string" ? event.detail.sequenceId : void 0;
    const continuesSequence = Boolean(sequenceId2 && previousCue?.sequenceId === sequenceId2);
    ...
    const start = restartAtCentre ? { x: PITCH_WIDTH2 / 2, y: PITCH_LENGTH2 / 2 } : restartAtGoal ? { x: PITCH_WIDTH2 / 2, y: side === "HOME" ? 5.5 : PITCH_LENGTH2 - 5.5 } : restartFromThrowIn ? clonePoint(fallbackTarget) : continuesSequence && previousCue ? clonePoint(previousCue.end) : clonePoint(recordedStart ?? ballCursor);
    let target;
    if (restartAtCentre) {
      ...
      const receivingTarget = receiverPoint ?? {
        x: start.x < PITCH_WIDTH2 / 2 ? 8 : PITCH_WIDTH2 - 8,
        y: clamp4(start.y + (side === "HOME" ? 5 : -5), 6, PITCH_LENGTH2 - 6)
      };
      ...
      target = kind === "CONTROL" ? moveTowards(start, fallbackTarget, 1.4) : clonePoint(start);
  ...
  spatial.visualCues = [...spatial.visualCues, ...cues2].slice(-1200);
};
var MatchEngineV2SpatialService = {
  ...
  synchronize: (spatial, live) => {
    ...
    const startPositions = Object.fromEntries(Object.entries(spatial.players).map(([playerId2, player]) => [
      playerId2,
...
var quadratic = (start, control, end, progress) => {
...
var MatchEngineV2TrajectoryService = {
  ...
  samplePlayerMovement: (start, end, elapsedMs, durationMs) => {
    const progress = smoothStep(clamp(elapsedMs / Math.max(1, durationMs), 0, 1));
    return {
      x: clamp(start.x + (end.x - start.x) * progress, 0, PITCH_WIDTH3),
      y: clamp(start.y + (end.y - start.y) * progress, 0, PITCH_LENGTH3)
    };
  }
};
...
var step = (kind, actor, receiver, start, end, durationMs, behaviors) => ({ kind, actor, receiver, start, end, durationMs, ...behaviors });
...
var terminalCues = (mode, cues2) => {
  if (mode === "COMMENTARY_ONLY") return [];
  if (mode === "FULL_MATCH") return [...cues2];
  const candidates = cues2.filter((cue) => {
...
var MatchEngineV2HighlightScriptService = {
  ...
  materialize: (terminal, snapshot2) => {
    ...
    const activePlayers4 = Object.values(snapshot2.spatial.players).filter((player) => player.isOnPitch && player.side === attackingSide);
    const defendingSide = attackingSide === "HOME" ? "AWAY" : "HOME";
    const defendingActivePlayers = Object.values(snapshot2.spatial.players).filter((player) => player.isOnPitch && player.side === defendingSide);
    ...
    const scorer = recordedAttackerId && snapshot2.spatial.players[recordedAttackerId]?.side === attackingSide ? recordedAttackerId : byRole.FORWARD[0]?.playerId ?? byRole.MIDFIELDER[0]?.playerId;
    ...
    const sequenceId2 = `highlight_${terminal.id}_${script.id}`;
    ...
    return script.steps.map((scriptStep, index) => {
      ...
      return {
        ...
        supportingRuns: (scriptStep.supportingRuns ?? []).map((run) => {
          const playerId2 = resolveRunPlayer(run);
          return playerId2 ? { playerId: playerId2, start: mirroredPoint(run.start, away), end: mirroredPoint(run.end, away) } : void 0;
...
var MatchEngineV2PlaybackService = {
   ...
   */
  selectVisibleCues: (state, cues2) => {
    if (state.transmissionMode === "COMMENTARY_ONLY") return [];
    if (state.transmissionMode === "FULL_MATCH") return [...cues2];
    const authored = cues2.filter((cue) => cue.scriptedHighlight);
    ...
    return MatchEngineV2HighlightScriptService.selectTerminalCues(state.transmissionMode, cues2);
  }
};

// services/match/engines/v2/MatchEngineV2FrameControllerService.ts
var PLAYER_MOVE_MS = 680;
var MAX_TRACK_STEP_METRES = 7.5;
var MAX_VISIBLE_METRES_PER_MS = 0.014;
var REPLAY_INTRO_MS = 650;
var REPLAY_OUTRO_MS = 700;
var REPLAY_CUE_SPEED = 1.8;
var GOAL_CELEBRATION_MS = 4e3;
var LIVE_MIN_CUE_MS = 72;
var HIGHLIGHT_SETTLE_SECONDS = 6;
var DEFAULT_APPROACH_SHARE = 0.18;
var DEFAULT_APPROACH_CAP_MS = 110;
var MATCH_SECONDS_PER_MATCH_MINUTE2 = 60;
var samePoint = (left, right) => Math.abs(left.x - right.x) < 1e-3 && Math.abs(left.y - right.y) < 1e-3;
var sampleTrack = (track, visualClockMs) => MatchEngineV2TrajectoryService.samplePlayerMovement(
  track.start,
  track.end,
  visualClockMs - track.startedAtMs,
  track.durationMs
);
var interpolatePoint = (start, end, progress) => {
...
var movePointTowards = (start, end, maximumDistance) => {
  const dx = end.x - start.x;
  const dy = end.y - start.y;
  const distance = Math.hypot(dx, dy);
  if (distance <= maximumDistance || distance < 1e-3) return { ...end };
  const ratio = maximumDistance / distance;
  return { x: start.x + dx * ratio, y: start.y + dy * ratio };
};
var maximumVisibleTravel = (active, multiplier = 1) => clamp(active.durationMs * MAX_VISIBLE_METRES_PER_MS * multiplier, 1.2, 10);
var applyActionChoreography = (players, active, ball, presentationProgress, approachProgress) => {
  const cue = active.cue;
  const actor = cue.actorId ? players[cue.actorId] : void 0;
  const receiver = cue.secondaryPlayerId ? players[cue.secondaryPlayerId] : void 0;
  const progress = clamp(ball.progress, 0, 1);
  if (actor) {
    const actorOrigin = active.actorOrigin ?? actor.position;
    const placeActor = (desired) => {
...
var isCornerCue = (cue) => cue.setPieceKind === "CORNER" || cue.sourceEventType === "CORNER" /* CORNER */ || cue.sourceEventType === "CORNER_TAKEN" /* CORNER_TAKEN */;
var isThrowInCue = (cue) => cue.sourceEventType === "THROW_IN" /* THROW_IN */;
var isFreeKickCue = (cue) => cue.setPieceKind === "FREE_KICK_WIDE" || cue.setPieceKind === "FREE_KICK_DIRECT" || cue.sourceEventType === "FREE_KICK" /* FREE_KICK */ || cue.sourceEventType === "FREE_KICK_DANGEROUS" /* FREE_KICK_DANGEROUS */;
var isPenaltyCue = (cue) => cue.setPieceKind === "PENALTY" || cue.sourceEventType === "PENALTY_AWARDED" /* PENALTY_AWARDED */;
var isOffsideCue = (cue) => cue.sourceEventType === "OFFSIDE" /* OFFSIDE */;
var isCentreRestart = (cue) => cue.sourceEventType === "KICK_OFF" /* KICK_OFF */;
var usesStructuredRestartShape = (cue) => isCornerCue(cue) || isThrowInCue(cue) || isFreeKickCue(cue) || isPenaltyCue(cue) || isOffsideCue(cue) || isCentreRestart(cue);
var sceneOrigin = (active, playerId2, fallback) => active.playerOrigins?.[playerId2] ?? fallback;
var applyStepSupportingRuns = (players, active, presentationProgress) => {
...
var applyStructuredRestartShape = (players, active, snapshot2, presentationProgress) => {
  const cue = active.cue;
  if (!usesStructuredRestartShape(cue)) return;
  const attackingSide = cue.side ?? snapshot2.spatial.players[cue.actorId ?? ""]?.side;
  ...
  const spatialPlayers = Object.values(snapshot2.spatial.players).filter((player) => player.isOnPitch && players[player.playerId]);
  ...
  const move = (playerId2, target) => {
    const rendered = players[playerId2];
    if (!rendered) return;
    const origin = sceneOrigin(active, playerId2, rendered.position);
    rendered.position = interpolatePoint(origin, target, shapeProgress);
  };
  const bySetPiecePriority = (left, right) => {
    const priority = { FWD: 0, MID: 1, DEF: 2, GK: 3 };
  ...
  if (isCentreRestart(cue)) {
    ...
    const support = attackingOutfield.filter((player) => player.playerId !== cue.actorId).sort(
      (left, right) => Math.hypot(left.position.x - 34, left.position.y - centreY) - Math.hypot(right.position.x - 34, right.position.y - centreY)
    )[0];
  ...
  if (isCornerCue(cue) || cue.setPieceKind === "FREE_KICK_WIDE") {
    const restartPoint = cue.sourceEventType === "CORNER" /* CORNER */ ? cue.end : cue.start;
...
var applyGoalCelebration = (players, cue, progress, snapshot2, visualClockMs) => {
  const actor = cue.actorId ? players[cue.actorId] : void 0;
  const scoringSide = cue.side ?? (cue.actorId ? snapshot2.spatial.players[cue.actorId]?.side : void 0);
  if (!actor || !scoringSide) return;
  const gather = progress < 0.45 ? progress / 0.45 : progress < 0.78 ? 1 : Math.max(0, (1 - progress) / 0.22);
  const goalDirection = scoringSide === "HOME" ? 1 : -1;
  const celebrationPoint = {
    x: cue.start.x <= 34 ? 10 : 58,
    y: scoringSide === "HOME" ? 99 : 6
  };
  const teammates = Object.values(snapshot2.spatial.players).filter((player) => player.isOnPitch && player.side === scoringSide && player.role !== "GK" && players[player.playerId]).sort((left, right) => {
...
var MIN_PLAYER_SEPARATION_METRES = 2.6;
var DECLUMP_PASSES = 3;
var declumpPlayers = (players) => {
  ...
  for (let pass = 0; pass < DECLUMP_PASSES; pass += 1) {
    ...
    for (let i = 0; i < ids.length; i += 1) {
      for (let j = i + 1; j < ids.length; j += 1) {
        ...
        movedAny = true;
        const nx = distance > 1e-3 ? dx / distance : ids[i] < ids[j] ? 1 : -1;
...
var createTracks = (snapshot2) => Object.fromEntries(Object.values(snapshot2.spatial.players).map((player) => [player.playerId, {
...
var synchronizeTracks = (controller, snapshot2, playback) => {
  const snapshotChanged = snapshot2.second !== controller.lastSnapshotSecond;
  Object.values(snapshot2.spatial.players).forEach((player) => {
    ...
    const current = controller.lastFramePlayers[player.playerId]?.position ?? sampleTrack(existing, controller.visualClockMs);
...
var collectNewCues = (controller, snapshot2, playback) => {
  const settledThroughSecond = snapshot2.isFinished ? Number.POSITIVE_INFINITY : snapshot2.second - HIGHLIGHT_SETTLE_SECONDS;
  const visible = MatchEngineV2PlaybackService.selectVisibleCues(playback, snapshot2.spatial.visualCues).filter((cue) => cue.atSecond <= snapshot2.second).filter(
    (cue) => playback.transmissionMode === "FULL_MATCH" || cue.scriptedHighlight === true || cue.atSecond <= settledThroughSecond
  ...
  newlyVisible.forEach((cue) => {
    ...
    const materialized = cue.scriptedHighlight || playback.transmissionMode === "FULL_MATCH" ? [cue] : MatchEngineV2HighlightScriptService.materialize(cue, snapshot2);
    ...
    presentationCues.forEach((presentationCue) => {
      controller.pendingLiveCues.push({
        cue: presentationCue,
        durationMs: presentationCue.kind === "GOAL" ? GOAL_CELEBRATION_MS : Math.max(LIVE_MIN_CUE_MS, presentationCue.durationMs) * playback.sceneSpeed
      });
    });
...
var preservesRecordedRestartOrigin = (cue) => isCentreRestart(cue) || cue.sourceEventType === "THROW_IN" /* THROW_IN */ || cue.scriptedHighlight === true && cue.highlightSceneIndex === 1;
var activateLiveCue = (controller, pending, startedAtMs, ballOrigin, snapshot2) => {
  const cue = preservesRecordedRestartOrigin(pending.cue) ? pending.cue : {
    ...pending.cue,
  ...
  const playerOrigins = Object.fromEntries(Object.values(snapshot2.spatial.players).map((player) => {
  ...
  if (isFirstScriptedScene) {
    cue.supportingRuns?.forEach((run) => {
      playerOrigins[run.playerId] = { ...run.start };
    });
  }
  const rawActorOrigin = cue.actorId && playerOrigins[cue.actorId] ? { ...playerOrigins[cue.actorId] } : void 0;
  const approachBudgetMetres = DEFAULT_APPROACH_CAP_MS * MAX_VISIBLE_METRES_PER_MS;
  const cueWithReachableStart = rawActorOrigin && !usesStructuredRestartShape(cue) ? { ...cue, start: movePointTowards(rawActorOrigin, cue.start, approachBudgetMetres) } : cue;
  return {
    cue: cueWithReachableStart,
    durationMs: pending.durationMs,
    startedAtMs,
    actorOrigin: rawActorOrigin,
    receiverOrigin: cue.secondaryPlayerId && playerOrigins[cue.secondaryPlayerId] ? { ...playerOrigins[cue.secondaryPlayerId] } : void 0,
    playerOrigins
  };
};
var rebaseActionTracks = (controller, completed, snapshot2, playback) => {
  const affectedPlayerIds = usesStructuredRestartShape(completed.cue) ? Object.keys(completed.playerOrigins ?? {}) : [completed.cue.actorId, completed.cue.secondaryPlayerId].filter((playerId2) => Boolean(playerId2));
  affectedPlayerIds.forEach((playerId2) => {
    if (!playerId2) return;
    const track = controller.playerTracks[playerId2];
    const rendered = controller.lastFramePlayers[playerId2];
    const authoritative = snapshot2.spatial.players[playerId2];
...
var updateLiveCue = (controller, snapshot2, playback) => {
  ...
  while (controller.activeLiveCue && controller.visualClockMs - controller.activeLiveCue.startedAtMs >= controller.activeLiveCue.durationMs) {
    ...
    rebaseActionTracks(controller, completed, snapshot2, playback);
    const next = controller.pendingLiveCues.shift();
    controller.activeLiveCue = next ? activateLiveCue(controller, next, controller.visualClockMs - overrun, continuityBall, snapshot2) : null;
  }
};
var replayDuration = (replay) => REPLAY_INTRO_MS + replay.cues.reduce((sum, cue) => sum + cue.durationMs * REPLAY_CUE_SPEED, 0) + REPLAY_OUTRO_MS;
var updateReplay = (controller, playback) => {
  ...
  if (controller.activeReplay && controller.visualClockMs - controller.activeReplay.startedAtMs >= replayDuration(controller.activeReplay)) {
    controller.activeReplay = null;
    const next = controller.replayQueue.shift();
    if (next) controller.activeReplay = { ...next, startedAtMs: controller.visualClockMs };
  }
};
var sampleReplay = (controller) => {
...
var makeFrame = (controller, snapshot2) => {
  ...
  const liveElapsedMs = liveActive ? clamp(controller.visualClockMs - liveActive.startedAtMs, 0, liveActive.durationMs) : 0;
  const presentationProgress = replaySample.cue ? clamp(replaySample.elapsedMs / Math.max(1, replaySample.cue.durationMs), 0, 1) : liveActive ? clamp(liveElapsedMs / Math.max(1, liveActive.durationMs), 0, 1) : 1;
  const approachShare = liveActive && liveActive.cue.kind !== "GOAL" ? Math.min(DEFAULT_APPROACH_SHARE, DEFAULT_APPROACH_CAP_MS / Math.max(1, liveActive.durationMs)) : 0;
  const approachProgress = approachShare > 0 ? clamp(presentationProgress / approachShare, 0, 1) : 1;
  const actionProgress = replaySample.cue ? presentationProgress : liveActive?.cue.kind === "GOAL" ? clamp(liveElapsedMs / Math.max(1, liveActive.cue.durationMs), 0, 1) : approachShare < 1 ? clamp((presentationProgress - approachShare) / (1 - approachShare), 0, 1) : 1;
  const players = Object.fromEntries(Object.entries(controller.playerTracks).map(([playerId2, track]) => [playerId2, {
    playerId: playerId2,
    position: sampleTrack(track, controller.visualClockMs),
    isOnPitch: track.isOnPitch
  }]));
  const activeForChoreography = liveActive ?? (replaySample.cue ? {
  ...
  if (activeForChoreography?.cue.scriptedHighlight && activeForChoreography.playerOrigins) {
    Object.entries(activeForChoreography.playerOrigins).forEach(([playerId2, origin]) => {
      if (players[playerId2]) players[playerId2].position = { ...origin };
    });
  }
  const desiredIdleBall = snapshot2.spatial.ball.ownerId && players[snapshot2.spatial.ball.ownerId] ? players[snapshot2.spatial.ball.ownerId].position : snapshot2.spatial.ball;
  const idleElapsedMs = clamp(controller.visualClockMs - controller.lastFrameAtMs, 0, 50);
  const idleBallPoint = controller.lastFrameAtMs === 0 ? desiredIdleBall : movePointTowards(controller.lastFrameBall, desiredIdleBall, idleElapsedMs * 0.03);
  const ball = activeCue ? MatchEngineV2TrajectoryService.sampleCue(activeCue, activeCue.durationMs * actionProgress) : {
    x: idleBallPoint.x,
    y: idleBallPoint.y,
    z: Math.max(0, controller.lastFrameBall.z - idleElapsedMs * 6e-3),
    progress: 1,
    finished: true
  };
  const liveGoalCelebration = !controller.activeReplay && controller.activeLiveCue?.cue.kind === "GOAL";
  const goalCelebrationProgress = liveGoalCelebration ? clamp(
    (controller.visualClockMs - controller.activeLiveCue.startedAtMs) / Math.max(1, controller.activeLiveCue.durationMs),
    0,
    1
  ) : 0;
  ...
  const frame = {
    ...
    goalCelebration: {
      ...
      side: liveGoalCelebration ? controller.activeLiveCue?.cue.side : void 0,
    ...
    blockSimulation: Boolean(
      controller.activeReplay || liveGoalCelebration || controller.activeLiveCue || controller.pendingLiveCues.length > 0
    )
  };
  controller.lastFramePlayers = Object.fromEntries(Object.entries(players).map(([playerId2, player]) => [playerId2, {
    ...player,
    position: { ...player.position }
  }]));
...
var MatchEngineV2FrameControllerService = {
  create: (snapshot2, wallClockMs = 0) => {
    const playerTracks = createTracks(snapshot2);
    const initialPlayers = Object.fromEntries(Object.entries(playerTracks).map(([playerId2, track]) => [playerId2, {
      playerId: playerId2,
    ...
    return {
      ...
      lastSnapshotSecond: snapshot2.second,
      clockSourceSecond: snapshot2.second,
      clockBaselineFootballSecond: snapshot2.displayClock.minute * 60 + snapshot2.displayClock.secondInMinute,
      idleElapsedMsSinceBaseline: 0,
      cosmeticDisplaySecond: snapshot2.displayClock.minute * 60 + snapshot2.displayClock.secondInMinute,
      playerTracks,
      ...
      seenCueIds: new Set(snapshot2.spatial.visualCues.map((cue) => cue.id)),
      seenGoalCueIds: new Set(snapshot2.spatial.visualCues.filter((cue) => cue.kind === "GOAL").map((cue) => cue.id)),
      pendingLiveCues: [],
      activeLiveCue: null,
      replayQueue: [],
      activeReplay: null,
      ...
      lastFrameBall: {
        x: snapshot2.spatial.ball.x,
        y: snapshot2.spatial.ball.y,
        z: snapshot2.spatial.ball.z,
        progress: 1,
        finished: true
      },
      lastFrameAtMs: 0
  ...
  advance: (controller, snapshot2, playback, wallClockMs) => {
    ...
    synchronizeTracks(controller, snapshot2, playback);
    collectNewCues(controller, snapshot2, playback);
    updateLiveCue(controller, snapshot2, playback);
    updateReplay(controller, playback);
    controller.lastSnapshotSecond = snapshot2.second;
    if (snapshot2.second !== controller.clockSourceSecond) {
      controller.clockSourceSecond = snapshot2.second;
      controller.clockBaselineFootballSecond = snapshot2.displayClock.minute * 60 + snapshot2.displayClock.secondInMinute;
      controller.idleElapsedMsSinceBaseline = 0;
    }
    const isIdleForClock = !playback.paused && !controller.activeLiveCue && !controller.activeReplay && controller.pendingLiveCues.length === 0;
    if (isIdleForClock) controller.idleElapsedMsSinceBaseline += elapsedWallMs;
    controller.cosmeticDisplaySecond = controller.clockBaselineFootballSecond + controller.idleElapsedMsSinceBaseline / 1e3 * (MATCH_SECONDS_PER_MATCH_MINUTE2 / playback.speed);
    return makeFrame(controller, snapshot2);
  },
  read: (controller, snapshot2) => makeFrame(controller, snapshot2),
  needsAnimation: (controller, playback) => Boolean(controller.activeReplay || !playback.paused)
};
...
var getPlayerMatchDayBias = (entropySeed, playerId2) => {
  const roll = unitFrom(entropySeed ^ hashString(playerId2) ^ 2654435769);
  if (roll < 0.08) return -0.72 - unitFrom(hashString(playerId2) ^ entropySeed ^ 2738958700) * 0.3;
  if (roll > 0.94) return 0.55 + unitFrom(hashString(playerId2) ^ entropySeed ^ 3355524772) * 0.25;
...
var UserCoachShoutService = {
  ...
  getEffects: ({
    ...
    activePlayers4.forEach((player) => {
      ...
      const instability = clamp6((55 - mentality) / 260 + (45 - morale) / 300 + (70 - fatigue) / 350, -0.02, 0.1);
...
// tests/MatchEngineV2FrameControllerTests.ts
var sample = CupSampleMatchFactory.makeInput(919, "EQUAL");
var runtime = MatchEngineV2.createMatch({
...
var kickoff = MatchEngineV2.snapshot(runtime);
var playing = MatchEngineV2PlaybackService.setPaused(
  MatchEngineV2PlaybackService.create({ transmissionMode: "ALL_ACTIONS", goalReplays: true }),
  false
);
var playerId = Object.values(kickoff.spatial.players).find((player) => player.side === "HOME" && player.role === "MID").playerId;
var originalAnchor = kickoff.spatial.players[playerId].anchor;
var originalPosition = kickoff.spatial.players[playerId].position;
var targetAuthoritativePosition = {
...
var interpolationController = MatchEngineV2FrameControllerService.create(kickoff, 0);
MatchEngineV2FrameControllerService.advance(interpolationController, movedSnapshot, playing, 100);
var midway = MatchEngineV2FrameControllerService.advance(interpolationController, movedSnapshot, playing, 330);
var middlePosition = midway.players[playerId].position;
import_strict.default.ok(middlePosition.x > originalPosition.x && middlePosition.x < targetAuthoritativePosition.x);
import_strict.default.ok(middlePosition.y > originalPosition.y && middlePosition.y < targetAuthoritativePosition.y);
var finishedMovement = MatchEngineV2FrameControllerService.advance(interpolationController, movedSnapshot, playing, 800);
var boundedMovementDistance = Math.hypot(
...
import_strict.default.ok(boundedMovementDistance <= 7.51, "Jedna aktualizacja nie mo\u017Ce przenie\u015B\u0107 zawodnika przez p\xF3\u0142 boiska.");
import_strict.default.ok(Math.hypot(
...
var receiverId = Object.values(kickoff.spatial.players).find(
  (player) => player.side === "HOME" && player.role !== "GK" && player.playerId !== playerId
).playerId;
var actionCue = {
  id: "visual_continuous_pass",
  sourceEventId: "continuous_pass",
  sourceEventType: "PASS_COMPLETED" /* PASS_COMPLETED */,
...
var actionController = MatchEngineV2FrameControllerService.create(kickoff, 0);
MatchEngineV2FrameControllerService.advance(actionController, actionSnapshot, playing, 100);
var actionFrame = MatchEngineV2FrameControllerService.advance(actionController, actionSnapshot, playing, 450);
import_strict.default.equal(actionFrame.activeCue?.id, actionCue.id);
import_strict.default.ok(Math.hypot(
...
var dribbleCue = {
  ...
  sourceEventType: "DRIBBLING" /* DRIBBLING */,
  kind: "DRIBBLE",
  secondaryPlayerId: void 0,
...
var dribbleController = MatchEngineV2FrameControllerService.create(kickoff, 0);
MatchEngineV2FrameControllerService.advance(dribbleController, dribbleSnapshot, playing, 100);
var dribbleFrame = MatchEngineV2FrameControllerService.advance(dribbleController, dribbleSnapshot, playing, 450);
import_strict.default.ok(Math.abs(dribbleFrame.players[playerId].position.x - dribbleFrame.ball.x) < 1e-3);
import_strict.default.ok(Math.abs(dribbleFrame.players[playerId].position.y - dribbleFrame.ball.y) < 1e-3);
var groupBehaviorCue = {
  ...actionCue,
...
var groupBehaviorSnapshot = {
  ...kickoff,
  second: 5,
  spatial: { ...kickoff.spatial, visualCues: [groupBehaviorCue] }
};
var homeSupportPlayer = Object.values(kickoff.spatial.players).find(
  (player) => player.side === "HOME" && player.role === "MID" && player.playerId !== playerId && player.playerId !== receiverId
);
var awayDefenderPlayer = Object.values(kickoff.spatial.players).find((player) => player.side === "AWAY" && player.role === "DEF");
var groupBehaviorController = MatchEngineV2FrameControllerService.create(kickoff, 0);
MatchEngineV2FrameControllerService.advance(groupBehaviorController, groupBehaviorSnapshot, playing, 100);
var groupBehaviorFrame = MatchEngineV2FrameControllerService.advance(groupBehaviorController, groupBehaviorSnapshot, playing, 500);
...
var provisionalShot = {
  ...
  sourceEventType: "SHOT" /* SHOT */,
  kind: "SHOT",
  atSecond: 5,
  actorId: playerId,
  secondaryPlayerId: void 0,
  scriptedHighlight: void 0,
  xG: 0.72
};
var conclusiveGoal = {
  ...provisionalShot,
  ...
  sourceEventType: "GOAL" /* GOAL */,
  kind: "GOAL",
  atSecond: 6
};
var finishController = MatchEngineV2FrameControllerService.create(kickoff, 0);
var keyMomentsPlaying = MatchEngineV2PlaybackService.setPaused(
  MatchEngineV2PlaybackService.create({ transmissionMode: "KEY_MOMENTS" }),
  false
);
var shotOnlySnapshot = {
  ...kickoff,
  second: 5,
  spatial: { ...kickoff.spatial, visualCues: [provisionalShot] }
};
var beforeOutcomeSettles = MatchEngineV2FrameControllerService.advance(finishController, shotOnlySnapshot, keyMomentsPlaying, 100);
import_strict.default.equal(beforeOutcomeSettles.activeCue, null, "Nie wolno odtwarza\u0107 strza\u0142u, zanim jego wynik zostanie rozstrzygni\u0119ty.");
var settledGoalSnapshot = {
  ...kickoff,
  second: 12,
...
import_strict.default.ok(queuedFinishIds.length > 0, "Rozstrzygni\u0119ty gol powinien utworzy\u0107 jedn\u0105 sekwencj\u0119 kluczowej akcji.");
import_strict.default.ok(queuedFinishIds.every((id) => id.includes(conclusiveGoal.id)), "Sekwencja nie mo\u017Ce zawiera\u0107 osobnej prezentacji wcze\u015Bniejszego strza\u0142u.");
var homeGoalkeeperId = Object.values(kickoff.spatial.players).find((player) => player.side === "HOME" && player.role === "GK").playerId;
var awayGoalkeeperId = Object.values(kickoff.spatial.players).find((player) => player.side === "AWAY" && player.role === "GK").playerId;
var frameForRestart = (cue) => {
...
var restartCue = (overrides) => ({
  ...actionCue,
  id: "restart-scene",
  sourceEventId: "restart-scene-event",
  sourceEventType: "CORNER" /* CORNER */,
  kind: "RESTART",
  atSecond: 15,
  side: "HOME",
  actorId: playerId,
...
var averageY = (frame, side) => {
  const points = Object.values(kickoff.spatial.players).filter((player) => player.side === side && player.role !== "GK").map((player) => frame.players[player.playerId].position.y);
  return points.reduce((sum, value) => sum + value, 0) / points.length;
};
var averageKickoffY = (side) => {
  const points = Object.values(kickoff.spatial.players).filter((player) => player.side === side && player.role !== "GK").map((player) => player.position.y);
  return points.reduce((sum, value) => sum + value, 0) / points.length;
};
var PITCH_DIAGONAL_METRES = Math.hypot(68, 105);
var assertRestartSpeedCap = (frame) => {
  Object.values(kickoff.spatial.players).forEach((player) => {
    ...
    import_strict.default.ok(distance <= PITCH_DIAGONAL_METRES + 0.01, `${player.playerId} przemie\u015Bci\u0142 si\u0119 o ${distance.toFixed(2)} m w jednej scenie.`);
  });
};
var cornerFrame = frameForRestart(restartCue({ id: "corner-scene", setPieceKind: "CORNER" }));
import_strict.default.ok(averageY(cornerFrame, "HOME") > averageKickoffY("HOME"), "Atakuj\u0105cy musz\u0105 ruszy\u0107 w stron\u0119 pola karnego.");
import_strict.default.ok(averageY(cornerFrame, "AWAY") > averageKickoffY("AWAY"), "Obro\u0144cy musz\u0105 przesun\u0105\u0107 krycie w stron\u0119 w\u0142asnej bramki.");
assertRestartSpeedCap(cornerFrame);
import_strict.default.ok(cornerFrame.players[awayGoalkeeperId].position.y > 100, "Bramkarz broni\u0105cy ro\u017Cnego musi pozosta\u0107 przy bramce.");
var throwFrame = frameForRestart(restartCue({
  id: "throw-scene",
  sourceEventType: "THROW_IN" /* THROW_IN */,
  setPieceKind: void 0,
  start: { x: 0.5, y: 61 },
  end: { x: 7.5, y: 66 }
}));
import_strict.default.ok(throwFrame.players[playerId].position.x < kickoff.spatial.players[playerId].position.x, "Wykonawca autu musi ruszy\u0107 do linii bocznej.");
assertRestartSpeedCap(throwFrame);
var directFreeKickFrame = frameForRestart(restartCue({
  id: "free-kick-scene",
  sourceEventType: "FREE_KICK_DANGEROUS" /* FREE_KICK_DANGEROUS */,
  setPieceKind: "FREE_KICK_DIRECT",
  end: { x: 32, y: 82 }
}));
var defendersApproachingWall = Object.values(kickoff.spatial.players).filter((player) => player.side === "AWAY" && player.role !== "GK").filter((player) => Math.abs(directFreeKickFrame.players[player.playerId].position.y - 91.15) < Math.abs(player.position.y - 91.15));
import_strict.default.ok(defendersApproachingWall.length >= 4, "Co najmniej czterech obro\u0144c\xF3w musi rozpocz\u0105\u0107 ustawianie muru.");
assertRestartSpeedCap(directFreeKickFrame);
var penaltyFrame = frameForRestart(restartCue({
  id: "penalty-scene",
  sourceEventType: "PENALTY_AWARDED" /* PENALTY_AWARDED */,
...
import_strict.default.ok(penaltyFrame.players[awayGoalkeeperId].position.y > 101, "Bramkarz przy karnym musi sta\u0107 na linii bramkowej.");
import_strict.default.ok(penaltyFrame.players[homeGoalkeeperId].position.y < 6, "Drugi bramkarz nie mo\u017Ce opuszcza\u0107 w\u0142asnej bramki.");
assertRestartSpeedCap(penaltyFrame);
var offsideFrame = frameForRestart(restartCue({
  id: "offside-scene",
  sourceEventType: "OFFSIDE" /* OFFSIDE */,
  setPieceKind: void 0,
  end: { x: 38, y: 79 }
}));
var offsideDefenderLine = Object.values(kickoff.spatial.players).filter((player) => player.side === "AWAY" && player.role !== "GK").filter((player) => Math.abs(offsideFrame.players[player.playerId].position.y - 79) < Math.abs(player.position.y - 79));
import_strict.default.ok(offsideDefenderLine.length >= 5, "Obro\u0144cy musz\u0105 rozpocz\u0105\u0107 ustawianie wsp\xF3lnej linii spalonego.");
assertRestartSpeedCap(offsideFrame);
var thirdPlayerId = Object.values(kickoff.spatial.players).find(
  (player) => player.side === "HOME" && player.role !== "GK" && player.playerId !== playerId && player.playerId !== receiverId
).playerId;
var chainedPassA = {
  ...actionCue,
...
var chainedController = MatchEngineV2FrameControllerService.create(kickoffForChain, 0);
var previousChainFrame = MatchEngineV2FrameControllerService.read(chainedController, kickoffForChain);
var maximumBallFrameStep = 0;
var maximumReceiverFrameStep = 0;
var maximumPlayerFrameStep = 0;
var maximumPlayerFrameStepContext = "";
var sawSecondPass = false;
for (let wallClockMs = 16; wallClockMs <= 760; wallClockMs += 16) {
  ...
  if (chainFrame.activeCue?.id === chainedPassB.id) {
    sawSecondPass = true;
    import_strict.default.deepEqual(chainFrame.activeCue.start, chainedPassA.end, "Drugie podanie musi rozpocz\u0105\u0107 si\u0119 dok\u0142adnie tam, gdzie sko\u0144czy\u0142o si\u0119 pierwsze.");
  }
  previousChainFrame = chainFrame;
}
import_strict.default.equal(sawSecondPass, true);
import_strict.default.ok(maximumBallFrameStep < 3, `Pi\u0142ka przeskoczy\u0142a o ${maximumBallFrameStep.toFixed(2)} m mi\u0119dzy klatkami.`);
import_strict.default.ok(maximumReceiverFrameStep < 3, `Zawodnik przeskoczy\u0142 o ${maximumReceiverFrameStep.toFixed(2)} m mi\u0119dzy klatkami.`);
import_strict.default.ok(maximumPlayerFrameStep < 3, `Znacznik zawodnika przeskoczy\u0142 o ${maximumPlayerFrameStep.toFixed(2)} m mi\u0119dzy klatkami (${maximumPlayerFrameStepContext}).`);
import_strict.default.equal(chainedController.pendingLiveCues.length, 0, "Kolejka jednego ticka nie mo\u017Ce narasta\u0107 za zegarem meczu.");
import_strict.default.equal(chainedController.activeLiveCue?.cue.id, chainedPassB.id);
var completedChainFrame = MatchEngineV2FrameControllerService.advance(chainedController, chainedSnapshot, playing, 1500);
import_strict.default.equal(chainedController.activeLiveCue, null, "Kluczowa scena powinna zako\u0144czy\u0107 si\u0119 po czasie zapisanym w scenariuszu.");
import_strict.default.equal(completedChainFrame.blockSimulation, false);
var scorerId = Object.values(kickoff.spatial.players).find((player) => player.side === "HOME" && player.role === "FWD").playerId;
var celebrationCue = {
  ...actionCue,
...
var celebrationSnapshot = {
  ...kickoff,
  second: 5,
  spatial: { ...kickoff.spatial, visualCues: [celebrationCue] }
};
var celebrationPlayback = MatchEngineV2PlaybackService.setGoalReplays(playing, false);
var celebrationController = MatchEngineV2FrameControllerService.create(kickoff, 0);
MatchEngineV2FrameControllerService.advance(celebrationController, celebrationSnapshot, celebrationPlayback, 100);
var celebrationMiddle = MatchEngineV2FrameControllerService.advance(celebrationController, celebrationSnapshot, celebrationPlayback, 2100);
import_strict.default.equal(celebrationMiddle.goalCelebration.active, true);
import_strict.default.equal(celebrationMiddle.goalCelebration.side, "HOME");
import_strict.default.ok(celebrationMiddle.goalCelebration.progress > 0.45 && celebrationMiddle.goalCelebration.progress < 0.55);
var celebrationBeforeEnd = MatchEngineV2FrameControllerService.advance(celebrationController, celebrationSnapshot, celebrationPlayback, 4099);
import_strict.default.equal(celebrationBeforeEnd.goalCelebration.active, true, "Celebracja powinna trwa\u0107 pe\u0142ne cztery sekundy.");
var celebrationAfterEnd = MatchEngineV2FrameControllerService.advance(celebrationController, celebrationSnapshot, celebrationPlayback, 4101);
import_strict.default.equal(celebrationAfterEnd.goalCelebration.active, false);
var pausedController = MatchEngineV2FrameControllerService.create(kickoff, 0);
var paused = MatchEngineV2PlaybackService.create();
MatchEngineV2FrameControllerService.advance(pausedController, movedSnapshot, paused, 100);
var pausedFrame = MatchEngineV2FrameControllerService.advance(pausedController, movedSnapshot, paused, 800);
import_strict.default.deepEqual(pausedFrame.players[playerId].position, originalPosition, "Pauza musi zatrzyma\u0107 interpolacj\u0119.");
var sequenceId = "sequence_goal_test";
var cues = [
  {
    id: "visual_pass_test",
  ...
  },
  {
    ...
    sourceEventType: "SHOT_ON_TARGET" /* SHOT_ON_TARGET */,
    sequenceId,
    kind: "SHOT",
    atSecond: 86,
    side: "HOME",
...
var replayController = MatchEngineV2FrameControllerService.create(kickoff, 0);
var scoreBeforeReplay = { ...goalSnapshot.result };
var replayFrame = MatchEngineV2FrameControllerService.advance(replayController, goalSnapshot, playing, 100);
for (let wallClockMs = 200; wallClockMs <= 12e3 && !replayFrame.replay.active; wallClockMs += 100) {
  replayFrame = MatchEngineV2FrameControllerService.advance(replayController, goalSnapshot, playing, wallClockMs);
}
import_strict.default.equal(replayFrame.replay.active, true, "Gol powinien uruchomi\u0107 powt\xF3rk\u0119 po zako\u0144czeniu akcji na \u017Cywo.");
import_strict.default.equal(replayFrame.blockSimulation, true);
import_strict.default.equal(replayFrame.replay.goalCueId, "visual_goal_test");
import_strict.default.ok(replayFrame.replay.cueCount >= 2 && replayFrame.replay.cueCount <= 5, "Powt\xF3rka powinna u\u017Cy\u0107 ca\u0142ego scenariusza gola.");
import_strict.default.deepEqual(goalSnapshot.result, scoreBeforeReplay, "Kontroler klatek nie mo\u017Ce zmieni\u0107 wyniku ani zdarze\u0144 meczu.");
var replayEnded = false;
for (let wallClockMs = 12100; wallClockMs <= 25e3; wallClockMs += 100) {
...
import_strict.default.equal(replayEnded, true, "Powt\xF3rka musi zako\u0144czy\u0107 si\u0119 w ograniczonym czasie.");
var disabledController = MatchEngineV2FrameControllerService.create(kickoff, 0);
var withoutReplays = MatchEngineV2PlaybackService.setGoalReplays(playing, false);
for (let wallClockMs = 100; wallClockMs <= 5e3; wallClockMs += 100) {
  const frame = MatchEngineV2FrameControllerService.advance(disabledController, goalSnapshot, withoutReplays, wallClockMs);
  import_strict.default.equal(frame.replay.active, false);
  import_strict.default.equal(frame.blockSimulation, Boolean(frame.goalCelebration.active || frame.activeCue || frame.replay.active));
}
var restoredController = MatchEngineV2FrameControllerService.create(goalSnapshot, 0);
for (let wallClockMs = 100; wallClockMs <= 2500; wallClockMs += 100) {
  ...
  import_strict.default.equal(frame.replay.active, false, "Wczytanie meczu nie mo\u017Ce odtworzy\u0107 starych goli.");
}
var deterministicA = MatchEngineV2FrameControllerService.create(kickoff, 0);
var deterministicB = MatchEngineV2FrameControllerService.create(kickoff, 0);
var samplesA = [100, 280, 610, 990].map(
  (time) => MatchEngineV2FrameControllerService.advance(deterministicA, movedSnapshot, playing, time)
);
var samplesB = [100, 280, 610, 990].map(
  (time) => MatchEngineV2FrameControllerService.advance(deterministicB, movedSnapshot, playing, time)
);
import_strict.default.deepEqual(samplesA, samplesB, "Te same migawki i czasy musz\u0105 wygenerowa\u0107 identyczne klatki.");
var [clumpPlayerIdA, clumpPlayerIdB] = Object.values(kickoff.spatial.players).filter((player) => player.side === "HOME" && player.role !== "GK").slice(0, 2).map((player) => player.playerId);
var clumpPoint = { x: 34, y: 52.5 };
var clumpController = MatchEngineV2FrameControllerService.create(kickoff, 0);
var clumpFrame = MatchEngineV2FrameControllerService.read(clumpController, kickoff);
var clumpWallClockMs = 0;
for (let tick = 1; tick <= 12; tick += 1) {
  const clumpedSnapshot = {
    ...kickoff,
    second: tick,
    spatial: {
      ...kickoff.spatial,
...
var declumpedDistance = Math.hypot(
