...
var FinanceService = {
   ...
   */
  evaluateReleaseRequest: (player2, club, squad) => {
  ...
  evaluateFASigningBoardDecision: (player2, proposedSalary, proposedBonus, squad, club) => {
    ...
    if (proposedSalary > highestSalary * 2 && highestSalary > 0 && player2.overallRating < 82) {
      return {
        approved: false,
    ...
    const fairSalary = FinanceService.getFairMarketSalary(player2.overallRating);
    ...
    if (overpayRatio > allowedOverpay) {
      return {
        approved: false,
        reason: `ZARZ\u0104D: Ta kwota to absurd! Sugerowana pensja rynkowa dla OVR ${player2.overallRating} to ok. ${fairSalary.toLocaleString()} PLN. Nie pozwolimy na tak\u0105 niegospodarno\u015B\u0107.`
      };
    }
  ...
  evaluateRenewalBoardDecision: (player2, proposedSalary, proposedBonus, squad, club) => {
...
var PlayerFormService = {
  calculate(player2) {
    ...
    score += clamp3(((player2.morale ?? 50) - 50) * 0.1, -5, 5);
  ...
  getTrainingIntensityAdjustment(player2, intensity) {
    const attributes = player2.attributes;
...
var buildRaiseRequest = (player2, club, squadAverage, rank) => {
  const fairSalary = FinanceService.getFairMarketSalary(player2.overallRating);
  const currentSalary = Math.max(5e4, player2.annualSalary || 0);
  const qualityPremium = Math.max(0, player2.overallRating - squadAverage) * 0.025;
  const rolePremium = rank <= 3 ? 0.22 : rank <= 6 ? 0.14 : 0.08;
  const personalityPremium = player2.moralePersonality === "EGOIST" ? 0.14 : player2.moralePersonality === "AMBITIOUS" ? 0.1 : player2.moralePersonality === "LOYAL" ? -0.05 : 0;
  ...
  const years = player2.age <= 23 ? 4 : player2.age <= 28 ? 4 : player2.age <= 32 ? 3 : player2.age <= 34 ? 2 : 1;
  const bonusMultiplier = player2.age >= 33 ? 0.7 : player2.age >= 28 ? 0.58 : player2.age >= 24 ? 0.46 : 0.32;
...
var getLastSeasonMatches = (player2) => {
  const history = player2.seasonHistory || [];
  if (history.length === 0) return getSeasonOutputProfile(player2).matches;
  return history[history.length - 1]?.matchesPlayed ?? 0;
};
var getPromotionRaiseRequest = (player2, club, squadAverage) => {
  const currentSalary = Math.max(5e4, player2.annualSalary || 0);
  const fairSalary = FinanceService.getFairMarketSalary(player2.overallRating);
  const matches = getLastSeasonMatches(player2);
  const underpayPressure = Math.max(0, 1 - currentSalary / Math.max(1, fairSalary));
  const qualityPremium = Math.max(0, player2.overallRating - squadAverage) * 0.015;
  const rolePremium = player2.isUntouchable || player2.squadRole === "KEY_PLAYER" ? 0.08 : player2.squadRole === "STARTER" ? 0.05 : 0.02;
  const regularityPremium = matches >= 30 ? 0.06 : matches >= 24 ? 0.04 : matches >= 18 ? 0.02 : 0;
  const personalityPremium = player2.moralePersonality === "EGOIST" ? 0.07 : player2.moralePersonality === "AMBITIOUS" ? 0.06 : player2.moralePersonality === "CONFIDENT" ? 0.04 : player2.moralePersonality === "LOYAL" ? -0.03 : player2.moralePersonality === "PROFESSIONAL" ? -0.01 : 0;
  const clubStepPremium = club.leagueId === "L_PL_1" ? 0.04 : club.leagueId === "L_PL_2" ? 0.03 : 0.02;
  const seed = stableHash(`${player2.id}_${player2.contractEndDate}_PROMOTION_RAISE`);
  ...
  const years = player2.age <= 23 ? 4 : player2.age <= 28 ? 4 : player2.age <= 32 ? 3 : player2.age <= 34 ? 2 : 1;
  const bonusMultiplier = player2.age >= 33 ? 0.62 : player2.age >= 28 ? 0.52 : player2.age >= 24 ? 0.42 : 0.3;
  return {
    salary,
...
var shouldRequestPromotionRaise = (player2, club, squadAverage, currentDate) => {
  const currentSalary = player2.annualSalary || 0;
  if (currentSalary <= 0) return false;
  if (PlayerMoraleService.isMoraleDemandLocked(player2, currentDate) || PlayerMoraleService.hasActiveMoraleDemand(player2)) return false;
  if (player2.transferPendingClubId || player2.contractRaiseRequest || player2.contractRaiseDemandUntil) return false;
  const matches = getLastSeasonMatches(player2);
  const playedRegularly = matches >= 18 || (player2.squadRole === "STARTER" || player2.squadRole === "KEY_PLAYER" || player2.isUntouchable) && matches >= 12;
  if (!playedRegularly) return false;
  const fairSalary = FinanceService.getFairMarketSalary(player2.overallRating);
  const financialRespectRatio = currentSalary / Math.max(1, fairSalary);
  const hasSportingArgument = player2.overallRating >= squadAverage - 1 || player2.squadRole === "STARTER" || player2.squadRole === "KEY_PLAYER" || player2.isUntouchable;
  return hasSportingArgument && (financialRespectRatio < 0.94 || player2.overallRating >= squadAverage + 3);
};
var estimateProtectedExitPrice = (player2, club, squadAverage) => {
  const marketValue = player2.marketValue ?? Math.max(15e4, Math.round(player2.overallRating * player2.overallRating * 4200));
  const squadPremium = Math.max(0, player2.overallRating - squadAverage) * 0.035;
  const clubPremium = Math.max(0, club.reputation - 7) * 0.025;
  const untouchablePremium = player2.isUntouchable ? 0.28 : 0.12;
  return roundTransferPrice(marketValue * (1.15 + untouchablePremium + squadPremium + clubPremium));
};
var shouldBoardSupportProtectedExit = (player2, club, squadAverage, transferRandomFactor) => {
  const marketValue = player2.marketValue ?? 0;
  const annualSalary = player2.annualSalary ?? 0;
  const saleLooksValuable = marketValue >= Math.max(5e5, annualSalary * 3) || player2.overallRating >= squadAverage + 9;
  ...
  const sportingResistance = ambitionScore >= 3 && player2.overallRating >= squadAverage + 10 ? 3 : 0;
  return greedScore * 2 + financialPressure + confidencePressure + transferRandomFactor - sportingResistance >= 5;
};
var getSeasonOutputProfile = (player2) => {
  const statGroups = [player2.stats, player2.cupStats, player2.euroStats].filter(Boolean);
...
var getOneTimeBonusPerformanceScore = (player2, profile) => {
...
var getOneTimeBonusStatsLine = (player2, profile) => {
...
var hasStandoutSeasonOutput = (player2, profile) => {
  if (profile.matches < 10) return false;
  const excellentRatings = profile.matches >= 14 && (profile.averageRating ?? 0) >= 7.22;
  if (player2.position === "FWD") {
    return profile.goals >= 14 || profile.goalContributions >= 20 || excellentRatings && profile.goalContributions >= 12;
  }
  if (player2.position === "MID") {
    return profile.assists >= 10 || profile.goalContributions >= 16 || excellentRatings && profile.goalContributions >= 8;
  }
  if (player2.position === "DEF") {
    return profile.goalContributions >= 8 || profile.matches >= 16 && (profile.averageRating ?? 0) >= 7.1;
  }
  return (player2.stats.cleanSheets ?? 0) >= 10 || profile.matches >= 16 && (profile.averageRating ?? 0) >= 7.05;
};
var formatSeasonOutputSummary = (profile) => {
  const ratingPart = profile.averageRating !== null ? `, \u015Brednia ocen ${profile.averageRating.toFixed(2).replace(".", ",")}` : "";
  return `${profile.goals} goli, ${profile.assists} asyst${ratingPart}`;
};
var isAvailableForMinutesDemand = (player2) => player2.health.status === "HEALTHY" /* HEALTHY */ && player2.condition >= 75 && (player2.fatigueDebt ?? 0) <= 55;
var getContractDaysLeft = (player2, currentDate) => {
  if (!player2.contractEndDate) return 9999;
  const contractEnd = new Date(player2.contractEndDate);
  if (Number.isNaN(contractEnd.getTime())) return 9999;
  return Math.floor((contractEnd.getTime() - currentDate.getTime()) / DAY_MS);
};
var getAgeTransferStabilityBias = (player2) => {
  const isEliteLatePrime = player2.age >= 26 && player2.overallRating >= 85;
  if (player2.age < 26) return 0;
  if (player2.age <= 28) return isEliteLatePrime ? -1 : -4;
  if (player2.age <= 31) return isEliteLatePrime ? -3 : -8;
  if (player2.age <= 34) return isEliteLatePrime ? -8 : -14;
  return isEliteLatePrime ? -12 : -20;
};
var hasRealisticCareerStepUpside = (player2, personality, hasHighReputationInterest) => {
  if (hasHighReputationInterest) return true;
  if (player2.age <= 24) return true;
  if (player2.age <= 27 && player2.overallRating >= 72) return true;
  if (player2.overallRating >= 78) return true;
  const hasUnrealisticAmbition = personality === "EGOIST" || personality === "AMBITIOUS";
  return hasUnrealisticAmbition && player2.age <= 30 && player2.overallRating >= 72;
};
...
var getMinutesDemandCopy = (player2, approach, recentAverageRating) => {
  ...
  if (approach === "BRAZEN") {
    return {
      subject: `\u017B\u0105danie wi\u0119kszej liczby minut: ${player2.lastName}`,
      body: `Trenerze,
...
${player2.firstName} ${player2.lastName}`
  ...
  if (approach === "ASSERTIVE") {
    return {
      subject: `Rozmowa o wi\u0119kszej liczbie minut: ${player2.lastName}`,
      body: `Trenerze,
...
${player2.firstName} ${player2.lastName}`
  ...
  if (approach === "CALM") {
    return {
      subject: `Pro\u015Bba o wi\u0119cej wyst\u0119p\xF3w: ${player2.lastName}`,
      body: `Trenerze,
...
${player2.firstName} ${player2.lastName}`
  ...
  return {
    subject: `Pro\u015Bba o szans\u0119: ${player2.lastName}`,
    body: `Trenerze,
...
var getDevelopmentExitDemandCopy = (player2, personality, totalMinutes) => {
  ...
  const exitLine = player2.age <= 23 ? "Jestem w wieku, w kt\xF3rym potrzebuj\u0119 regularnych wyst\u0119p\xF3w, a nie samego czekania na \u0142awce." : "Potrzebuj\u0119 regularnej gry, \u017Ceby utrzyma\u0107 rytm i swoj\u0105 pozycj\u0119 sportow\u0105.";
  if (personality === "EGOIST" || personality === "AMBITIOUS") {
    return {
      subject: `Pro\u015Bba o odej\u015Bcie albo wypo\u017Cyczenie: ${player2.lastName}`,
      body: `Trenerze,
...
${player2.firstName} ${player2.lastName}`,
  ...
  if (personality === "LOYAL" || personality === "PROFESSIONAL" || personality === "CALM") {
    return {
      subject: `Pro\u015Bba o rozwi\u0105zanie sytuacji z minutami: ${player2.lastName}`,
      body: `Trenerze,
...
${player2.firstName} ${player2.lastName}`,
  ...
  return {
    subject: `Rozmowa o przysz\u0142o\u015Bci po braku minut: ${player2.lastName}`,
    body: `Trenerze,
...
var getTransferListDemandCopy = (player2, personality, trigger, seasonOutputSummary) => {
  if (trigger === "STANDOUT_SEASON") {
    ...
    return {
      subject: `Pro\u015Bba po mocnym sezonie: ${player2.lastName}`,
      body: `Trenerze,
...
${player2.firstName} ${player2.lastName}`
  ...
  if (trigger === "STRONG_INTEREST") {
    return {
      subject: `Pro\u015Bba o zgod\u0119 na rozmowy: ${player2.lastName}`,
      body: `Trenerze,
...
${player2.firstName} ${player2.lastName}`
  ...
  if (trigger === "HIGHER_REPUTATION") {
    return {
      subject: `Rozmowa o kolejnym kroku w karierze: ${player2.lastName}`,
      body: `Trenerze,
...
${player2.firstName} ${player2.lastName}`
  ...
  if (player2.isUntouchable) {
    if (personality === "EGOIST" || personality === "AMBITIOUS" || personality === "CONFIDENT") {
      return {
        subject: `Rozmowa o mojej przysz\u0142o\u015Bci: ${player2.lastName}`,
        body: `Trenerze,
...
${player2.firstName} ${player2.lastName}`
    ...
    return {
      subject: `Pro\u015Bba o rozmow\u0119 o przysz\u0142o\u015Bci: ${player2.lastName}`,
      body: `Trenerze,
...
${player2.firstName} ${player2.lastName}`
  ...
  return {
    subject: `Pro\u015Bba o list\u0119 transferow\u0105: ${player2.lastName}`,
    body: `Trenerze,
...
var hasBrokenContractPromise = (player2) => !!player2.transferContractPromise?.broken;
...
var hasClinchedSeasonAchievementMorale = (player2, achievement) => {
  const reason = getClinchedSeasonAchievementReason(achievement);
  return (player2.moraleHistory ?? []).some((entry) => entry.reason === reason);
};
var PlayerMoraleService = {
  clamp: (morale) => Math.max(0, Math.min(100, Math.round(morale))),
  getInitialMorale: (player2) => {
    const seed = stableHash(player2.id);
    const mentality = player2.attributes.mentality ?? 50;
    const ageBias = player2.age <= 21 ? 0.04 : player2.age >= 31 ? 0.02 : 0;
  ...
  getInitialPersonality: (player2) => {
    const attrs = player2.attributes;
    ...
    const index = Math.floor(seededRng(stableHash(player2.id), 7) * PERSONALITIES.length);
    return PERSONALITIES[index] ?? "CALM";
  },
  getInitialMindset: (player2) => {
    const morale = player2.morale ?? PlayerMoraleService.getInitialMorale(player2);
    const personality = player2.moralePersonality ?? PlayerMoraleService.getInitialPersonality(player2);
    ...
    const hasRole = player2.squadRole === "STARTER" || player2.squadRole === "KEY_PLAYER";
    const youngDevelopmentNeed = player2.age <= 23 ? 5 : 0;
    const ageStability = player2.age >= 35 ? 16 : player2.age >= 32 ? 11 : player2.age >= 29 ? 7 : player2.age >= 26 ? 3 : 0;
    return {
      ...
      squadBelonging: PlayerMoraleService.clamp(morale + (personality === "LOYAL" ? 10 : 0) - (player2.isOnTransferList ? 18 : 0)),
      ...
      developmentSatisfaction: PlayerMoraleService.clamp(morale - youngDevelopmentNeed + (player2.trainingFocus ? 4 : 0)),
      transferOpenness: PlayerMoraleService.clamp(45 - morale + ambitionPressure - ageStability + (player2.isOnTransferList ? 35 : 0) + (player2.interestedClubs?.length ?? 0) * 5),
      conflictLevel: PlayerMoraleService.clamp(55 - morale + Math.max(0, ambitionPressure)),
      lastUpdatedAt: void 0,
      history: []
    };
  },
  normalizeMindset: (player2) => {
    const initial = PlayerMoraleService.getInitialMindset(player2);
    const existing = player2.playerMindset;
  ...
  withMindsetChange: (player2, deltas, reason, date) => {
    const current = PlayerMoraleService.normalizeMindset(player2);
    const next = { ...current };
    let changed = false;
    ...
    if (!changed) return { ...player2, playerMindset: current };
    const entry = {
      id: `MINDSET_${player2.id}_${date.getTime()}_${stableHash(reason)}`,
      date: toDateKey(date),
      reason,
      deltas
    };
  ...
  ensurePlayerState: (player2) => ({
    ...player2,
    form: typeof player2.form === "number" ? player2.form : PlayerFormService.calculate(player2).score,
    morale: player2.morale ?? PlayerMoraleService.getInitialMorale(player2),
    moralePersonality: player2.moralePersonality ?? PlayerMoraleService.getInitialPersonality(player2),
    moraleHistory: player2.moraleHistory ?? [],
    playerMindset: PlayerMoraleService.normalizeMindset(player2),
    lastIndividualTalkDate: player2.lastIndividualTalkDate ?? null,
    promisedMinutesUntil: player2.promisedMinutesUntil ?? null,
    promisedMinutesBaseline: player2.promisedMinutesBaseline ?? null,
    promisedRoleNextMatchFixtureId: player2.promisedRoleNextMatchFixtureId ?? null,
    lastMoraleDemandDate: player2.lastMoraleDemandDate ?? null,
    minutesDemandUntil: player2.minutesDemandUntil ?? null,
    minutesDemandBaseline: player2.minutesDemandBaseline ?? null,
    unresolvedMinutesDemandDate: player2.unresolvedMinutesDemandDate ?? null,
    unresolvedMinutesDemandBaseline: player2.unresolvedMinutesDemandBaseline ?? null,
    developmentExitDemandUntil: player2.developmentExitDemandUntil ?? null,
    developmentExitDemandBaseline: player2.developmentExitDemandBaseline ?? null,
    lastTemptingOfferConflictDate: player2.lastTemptingOfferConflictDate ?? null,
    roleDemandUntil: player2.roleDemandUntil ?? null,
    requestedSquadRole: player2.requestedSquadRole ?? null,
    squadRoleMindsetLockUntil: player2.squadRoleMindsetLockUntil ?? null,
    transferListDemandUntil: player2.transferListDemandUntil ?? null,
    oneTimeBonusPromise: player2.oneTimeBonusPromise ?? null,
    oneTimeBonusAwardedSeason: player2.oneTimeBonusAwardedSeason ?? null,
    contractRaiseDemandUntil: player2.contractRaiseDemandUntil ?? null,
    contractRaiseRequest: player2.contractRaiseRequest ?? null,
    contractRaiseReminderUntil: player2.contractRaiseReminderUntil ?? null,
    contractRaiseTeamMoraleDelta: player2.contractRaiseTeamMoraleDelta ?? null,
    contractRaiseTeamMoraleReason: player2.contractRaiseTeamMoraleReason ?? null,
    reserveProtestUntil: player2.reserveProtestUntil ?? null,
    moraleDemandLockoutUntil: player2.moraleDemandLockoutUntil ?? null,
    // ── Transfer Request Dialog (PlayerTransferRequestDialogService) ──────────
    transferContractPromise: player2.transferContractPromise ?? null,
    transferAllowAfterSeason: player2.transferAllowAfterSeason ?? false,
    transferAllowAfterSeasonDeadline: player2.transferAllowAfterSeasonDeadline ?? null,
    transferRequestPendingResponse: player2.transferRequestPendingResponse ?? null
  }),
  ...
  isMoraleDemandLocked: (player2, currentDate) => {
    if (!player2.moraleDemandLockoutUntil) return false;
    const lockoutUntil = new Date(player2.moraleDemandLockoutUntil);
    return !Number.isNaN(lockoutUntil.getTime()) && dateOnly(currentDate).getTime() < dateOnly(lockoutUntil).getTime();
  },
  hasActiveMoraleDemand: (player2) => !!player2.minutesDemandUntil || !!player2.roleDemandUntil || !!player2.transferListDemandUntil || !!player2.developmentExitDemandUntil || !!player2.contractRaiseDemandUntil || !!player2.reserveProtestUntil || !!player2.boardAppealDeadline,
  applyClinchedSeasonAchievementMorale: (player2, achievement, currentDate) => {
    let withMorale = PlayerMoraleService.ensurePlayerState(player2);
  ...
  applyPresidentTeamBonusMorale: (player2, totalBonusAmount, squadSize, currentDate) => {
    let withMorale = PlayerMoraleService.ensurePlayerState(player2);
  ...
  applyContractSigningMindflowReset: (player2, currentDate) => ({
    ...player2,
    playerMindset: PlayerMoraleService.withMindsetChange(
      PlayerMoraleService.ensurePlayerState(player2),
      {
        coachTrust: 8,
        clubHappiness: 6,
        roleClarity: 4,
  ...
  applySeasonOutcomeMindflow: (player2, input) => {
  ...
  withMoraleChange: (player2, delta, reason, date) => {
    const withMorale = PlayerMoraleService.ensurePlayerState(player2);
  ...
  canTalk: (player2, currentDate) => {
    if (!player2.lastIndividualTalkDate) return true;
    const last = new Date(player2.lastIndividualTalkDate);
    if (Number.isNaN(last.getTime())) return true;
    return dayDiff(last, currentDate) >= 7;
  },
  getNextTalkDate: (player2) => {
    if (!player2.lastIndividualTalkDate) return null;
    const last = new Date(player2.lastIndividualTalkDate);
  ...
  calculateTalkResult: (player2, talkType, currentDate, seed) => {
    const personality = player2.moralePersonality ?? PlayerMoraleService.getInitialPersonality(player2);
    const morale = player2.morale ?? PlayerMoraleService.getInitialMorale(player2);
    const recentRating = player2.stats.ratingHistory?.at(-1) ?? 6.5;
    const rng = seededRng(seed + stableHash(player2.id) + currentDate.getTime(), talkType.length);
    let base = 3;
    ...
    if (talkType === "PROMISE_MINUTES") {
      base = player2.squadRole === "KEY_PLAYER" ? 2 : 6;
      successChance = 0.68;
    ...
    const backfireRoll = seededRng(seed + stableHash(player2.id), talkType.charCodeAt(0) + 31);
  ...
  applyTrainingMood: (player2, intensity) => {
    const personality = player2.moralePersonality ?? PlayerMoraleService.getInitialPersonality(player2);
    const fatigue = player2.fatigueDebt ?? 0;
    let delta = 0;
    if (intensity === "HEAVY" /* HEAVY */) {
      delta = personality === "PROFESSIONAL" || personality === "AMBITIOUS" ? 1 : -1;
      if (fatigue > 45) delta -= 2;
      if (player2.condition < 65) delta -= 1;
    } else if (intensity === "LIGHT" /* LIGHT */) {
      delta = fatigue > 35 || player2.condition < 70 ? 2 : 0;
      if (personality === "AMBITIOUS" && fatigue < 20) delta -= 1;
    }
    return delta;
  },
  getMatchMultiplier: (player2) => {
    const morale = player2.morale ?? 50;
  ...
  getMatchContributionMultiplier: (player2) => {
    const morale = player2.morale ?? 50;
  ...
  getLineupReadinessMultiplier: (player2) => {
    const morale = player2.morale ?? 50;
  ...
  getEffectiveOverall: (player2) => Math.round(player2.overallRating * PlayerMoraleService.getLineupReadinessMultiplier(player2)),
  applyNaturalDrift: (player2) => {
    const morale = player2.morale ?? PlayerMoraleService.getInitialMorale(player2);
    const drift = morale > 60 ? -1 : morale < 40 ? 1 : 0;
    return { ...player2, morale: PlayerMoraleService.clamp(morale + drift) };
  },
  getMindsetMoraleFeedback: (player2) => {
    const mindset = PlayerMoraleService.normalizeMindset(player2);
    const morale = player2.morale ?? 50;
  ...
  getTotalMinutesPlayed: (player2) => (player2.stats?.minutesPlayed ?? 0) + (player2.reserveStats?.matches ?? 0) * 90,
  reviewMinutePromise: (player2, currentDate) => {
    const withMorale = PlayerMoraleService.ensurePlayerState(player2);
  ...
  processPeriodicReview: (players, currentDate) => {
    const reviewedPlayers = players.map((player2) => {
      const demandReview = PlayerMoraleService.reviewPlayerDemands(player2, currentDate);
    ...
    const teamMoraleEvents = reviewedPlayers.filter((player2) => (player2.contractRaiseTeamMoraleDelta ?? 0) < 0).map((player2) => ({
      playerId: player2.id,
      delta: player2.contractRaiseTeamMoraleDelta ?? 0,
      reason: player2.contractRaiseTeamMoraleReason ?? "Napi\u0119cie w szatni po odrzuconej podwy\u017Cce lidera"
    }));
    if (teamMoraleEvents.length === 0) return reviewedPlayers;
    return reviewedPlayers.map((player2) => {
  ...
  processPlayerDemands: (club, squad, currentDate, existingMessages = [], fixtures, allClubs = []) => {
    ...
    const squadAverage = squad.reduce((sum, player2) => sum + player2.overallRating, 0) / squad.length;
    const rankById = new Map(sortedByQuality.map((player2, index) => [player2.id, index + 1]));
    const byPosition = /* @__PURE__ */ new Map();
    squad.forEach((player2) => {
      byPosition.set(player2.position, [...byPosition.get(player2.position) ?? [], player2]);
    });
    ...
    const hasRecentMail = (player2, requestType) => existingMessages.some(
      (mail) => mail.metadata?.type === "PLAYER_MORALE_REQUEST" && mail.metadata.playerId === player2.id && mail.metadata.requestType === requestType && new Date(mail.date).getTime() >= currentDate.getTime() - 21 * DAY_MS
    );
    ...
    const createdMails = [];
    const nextPlayers = squad.map((player2) => {
      let withMorale = PlayerMoraleService.ensurePlayerState(player2);
      const rank = rankById.get(player2.id) ?? squad.length;
      const positionRank = (byPosition.get(player2.position) ?? []).findIndex((posPlayer) => posPlayer.id === player2.id) + 1 || 99;
...
${withMorale.firstName} ${withMorale.lastName}`,
  ...
  reviewPlayerDemands: (player2, currentDate) => {
    let withMorale = PlayerMoraleService.ensurePlayerState(player2);
  ...
  getOneTimeBonusRequestBlockReason: (player2, club, seasonNumber) => {
    const withMorale = PlayerMoraleService.ensurePlayerState(player2);
  ...
  createOneTimeBonusPromise: (player2, currentDate, seasonNumber) => {
  ...
  processBoardAppeals: (club, squad, currentDate, existingMessages = []) => {
    ...
    const hasBoardAppealMail = (player2) => existingMessages.some(
      (m) => m.metadata?.type === "PLAYER_BOARD_APPEAL" && m.metadata.playerId === player2.id
    );
    const hasBoardDecisionMail = (player2) => existingMessages.some(
      (m) => m.metadata?.type === "BOARD_APPEAL_DECISION" && m.metadata.playerId === player2.id && new Date(m.date).getTime() >= currentDate.getTime() - 60 * DAY_MS
    );
    const nextPlayers = squad.map((player2) => {
      let withMorale = PlayerMoraleService.ensurePlayerState(player2);
...
var calculateLiveOwnGoalChance = ({
  ...
  const activeDefenders = defendingPlayers.filter((player2) => activeIds.includes(player2.id));
  if (activeDefenders.length === 0) return 0;
  const averageDefensiveRead = activeDefenders.reduce((sum, player2) => {
    const attributes = player2.attributes;
  ...
  const moralePenalty = activeDefenders.reduce((sum, player2) => {
    const morale = player2.morale ?? 50;
  ...
  const teamMoraleMultiplier = activeDefenders.reduce(
    (sum, player2) => sum + PlayerMoraleService.getMatchMultiplier(player2),
    0
...
var player = (id, position, attrs = {}, morale = 55) => ({
  id,
  firstName: id,
  lastName: id,
  position,
...
var lineup = ["gk", "d1", "d2", "d3", "d4", "d5", "m1", "m2", "m3", "f1", "f2"];
var positions = ["GK" /* GK */, "DEF" /* DEF */, "DEF" /* DEF */, "DEF" /* DEF */, "DEF" /* DEF */, "DEF" /* DEF */, "MID" /* MID */, "MID" /* MID */, "MID" /* MID */, "FWD" /* FWD */, "FWD" /* FWD */];
var calmPlayers = lineup.map((id, index) => player(id, positions[index]));
var badPlayers = lineup.map((id, index) => player(id, positions[index], { defending: 42, positioning: 42, technique: 42, mentality: 42, workRate: 42 }, 22));
for (const context of ["openPlay", "chaos", "corner"]) {
  const calm = calculateLiveOwnGoalChance({ context, defendingPlayers: calmPlayers, defendingLineup: lineup, defendingFatigue: Object.fromEntries(lineup.map((id) => [id, 92])), weather: { precipitationChance: 0 } });
  const bad = calculateLiveOwnGoalChance({ context, defendingPlayers: badPlayers, defendingLineup: lineup, defendingFatigue: Object.fromEntries(lineup.map((id) => [id, 48])), weather: { precipitationChance: 95 }, pressureMultiplier: 1.45 });
  console.log(`${context}: calm=${(calm * 100).toFixed(2)}%, exhaustedRain=${(bad * 100).toFixed(2)}%`);
}
