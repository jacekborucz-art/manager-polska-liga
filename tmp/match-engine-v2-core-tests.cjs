...
// tests/MatchEngineV2CoreTests.ts
...
var seededRandom = (seed, second2, salt) => {
  let value = stableHash(`${seed}:${second2}:${salt}`) + 1831565813;
...
var CupPenaltyShootoutService = {
  ...
  simulate: (input, fatigue, options = {}) => {
    ...
    const pushAttempt = (attempt) => {
      ...
      const second2 = startSecond + attempt.order * 15;
      ...
      events.push({
        id: attempt.id,
        second: second2,
        minute: Math.floor(second2 / 60) + 1,
        side: attempt.side,
...
var activeGoalkeeperAtSecond = (match, events, side, lineups, second2) => {
  let keeperId = goalkeeperIdForSide(match, side, lineups);
  const team = side === "HOME" ? match.home : match.away;
  const players = new Map(team.players.map((player) => [player.id, player]));
  events.filter((event) => event.type === "SUBSTITUTION" /* SUBSTITUTION */ && event.side === side && event.second <= second2).sort((a, b) => a.second - b.second).forEach((event) => {
...
var CupMatchEngineV2 = {
   ...
   */
  applyManualSubstitution: (live, side, playerOutId2, playerInId2) => {
    if (live.finalResult || playerOutId2 === playerInId2) return false;
    if (live.state.substitutionsUsed[side] >= live.config.maxSubstitutions) return false;
    if (live.state.redCards[playerOutId2] || live.state.redCards[playerInId2]) return false;
    const team = side === "HOME" ? live.input.home : live.input.away;
    const slotIndex = team.lineup.startingXI.findIndex((id) => id === playerOutId2);
    if (slotIndex < 0 || !team.lineup.bench.includes(playerInId2)) return false;
    const alreadyLeftPitch = live.state.events.some(
      (event) => event.type === "SUBSTITUTION" /* SUBSTITUTION */ && event.secondaryPlayerId === playerInId2
    );
    if (alreadyLeftPitch) return false;
    const playerOut = team.players.find((player) => player.id === playerOutId2);
    const playerIn = team.players.find((player) => player.id === playerInId2);
    if (!playerOut || !playerIn) return false;
    team.lineup.startingXI[slotIndex] = playerInId2;
    team.lineup.bench = team.lineup.bench.filter((id) => id !== playerInId2);
    team.lineup.bench.push(playerOutId2);
    live.state.substitutionsUsed[side] += 1;
    live.state.fatigue[playerInId2] = Math.min(
      playerIn.condition,
      live.state.fatigue[playerInId2] ?? playerIn.condition
    );
    live.state.events.push({
      id: `cupv2_manual_substitution_${live.state.second}_${side}_${playerOutId2}_${playerInId2}`,
      second: live.state.second,
      minute: CupMatchClockService.eventMinute(live.state, live.config),
      side,
      type: "SUBSTITUTION" /* SUBSTITUTION */,
      playerId: playerInId2,
      secondaryPlayerId: playerOutId2,
...
var createContext = (side, possession, ballZone, second2) => {
  const phase = settledPhase(side, possession, ballZone);
  return {
    phase,
    previousPhase: phase,
    phaseChangedAtSecond: second2
  };
};
var updateContext = (previous, side, possession, ballZone, second2, possessionChanged) => {
  let phase;
  if (possessionChanged) {
    phase = side === possession ? "TRANSITION_ATTACK" : "TRANSITION_DEFEND";
  } else if ((previous.phase === "TRANSITION_ATTACK" || previous.phase === "TRANSITION_DEFEND") && second2 - previous.phaseChangedAtSecond < TRANSITION_DURATION_SECONDS) {
...
var resolve = ({
  ...
  const elapsed = Math.max(0.2, second2 - previous.lastUpdatedSecond);
  if (latestCue?.atSecond === second2 && travellingKind(latestCue)) {
    ...
    return {
      ...
      lastUpdatedSecond: second2,
      phase: "TRAVELLING"
    };
  }
  if (latestCue?.atSecond === second2 && latestCue.kind === "RESTART") {
    return {
      x: latestCue.end.x,
      y: latestCue.end.y,
      z: 0,
  ...
  if (latestCue?.atSecond === second2 && controlledKind(latestCue)) {
  ...
  for (let iteration = 0; iteration < 14; iteration += 1) {
    for (let firstIndex = 0; firstIndex < active.length; firstIndex += 1) {
      for (let secondIndex = firstIndex + 1; secondIndex < active.length; secondIndex += 1) {
        const first2 = active[firstIndex];
        const second2 = active[secondIndex];
        let dx = second2.position.x - first2.position.x;
        let dy = second2.position.y - first2.position.y;
        let distance = Math.hypot(dx, dy);
        if (distance >= minimumDistance) continue;
        if (distance < 1e-3) {
          const direction = first2.playerId.localeCompare(second2.playerId) <= 0 ? 1 : -1;
          dx = direction;
          dy = 0;
          distance = 1;
        }
        ...
        const firstCorrection = {
          x: clamp4(first2.position.x - normalX * correction, 1, PITCH_WIDTH2 - 1),
          y: clamp4(first2.position.y - normalY * correction, 1, PITCH_LENGTH2 - 1)
        };
        const secondCorrection = {
          x: clamp4(second2.position.x + normalX * correction, 1, PITCH_WIDTH2 - 1),
          y: clamp4(second2.position.y + normalY * correction, 1, PITCH_LENGTH2 - 1)
        };
        first2.position = first2.returningToMovementZone ? firstCorrection : clampToMovementZone2(firstCorrection, first2);
        second2.position = second2.returningToMovementZone ? secondCorrection : clampToMovementZone2(secondCorrection, second2);
      }
    }
  }
};
...
var pressingPlayersForSide = (spatial, live, side, ball) => {
  ...
  return new Set(
    Object.values(spatial.players).filter((player) => player.side === side && player.isOnPitch && player.role !== "GK" /* GK */).sort(
      (first2, second2) => Math.hypot(first2.position.x - ball.x, first2.position.y - ball.y) - Math.hypot(second2.position.x - ball.x, second2.position.y - ball.y)
...
var recordCommand = (runtime, command, accepted, reason) => {
  const entry = {
...
var refreshSpatialDecisionContext = (runtime) => {
...
var MatchEngineV2 = {
   ...
   */
  advanceTo: (runtime, requestedSecond) => {
    ...
    while (runtime.core.state.second < alignedSecond && runtime.core.state.phase !== "FINISHED") {
  ...
  applyCommand: (runtime, command) => {
    ...
    if (command.type === "TOUCHLINE_INSTRUCTION") {
      ...
      MatchEngineV2CoachService.refreshEffects(runtime.core, runtime.coachState);
      return recordCommand(runtime, command, applied2, applied2 ? void 0 : "COACH_COMMAND_TOO_EARLY");
    }
    ...
    return recordCommand(runtime, command, applied, applied ? void 0 : "ILLEGAL_SUBSTITUTION");
  },
  snapshot,
  /** Finalization never advances time; it only exposes an already finished result. */
  finalize: (runtime) => CupMatchEngineV2.finalizeLiveMatch(runtime.core)
};

// tests/MatchEngineV2CoreTests.ts
var makeLeagueInput = (seed) => {
...
var kickoff = MatchEngineV2.createMatch(makeLeagueInput(101));
var kickoffSnapshot = MatchEngineV2.snapshot(kickoff);
import_strict.default.equal(kickoffSnapshot.second, 0);
import_strict.default.equal(kickoffSnapshot.result.events.length, 0);
import_strict.default.equal(kickoffSnapshot.result.winner, void 0);
import_strict.default.equal(MatchEngineV2.finalize(kickoff), null);
var kickoffPlayers = Object.values(kickoffSnapshot.spatial.players);
import_strict.default.equal(kickoffPlayers.filter((player) => player.isOnPitch).length, 22);
kickoffPlayers.forEach((player) => {
  import_strict.default.ok(player.position.x >= 0 && player.position.x <= kickoffSnapshot.spatial.pitchWidth);
  import_strict.default.ok(player.position.y >= 0 && player.position.y <= kickoffSnapshot.spatial.pitchLength);
  import_strict.default.ok(player.anchor.x >= player.movementZone.minX && player.anchor.x <= player.movementZone.maxX);
  import_strict.default.ok(player.anchor.y >= player.movementZone.minY && player.anchor.y <= player.movementZone.maxY);
});
var kickoffSide = kickoff.core.state.firstHalfKickOffSide;
var kickoffTeam = kickoffSide === "HOME" ? kickoff.core.input.home : kickoff.core.input.away;
var kickoffMidfielderId = kickoffTeam.lineup.startingXI.find(
  (playerId) => kickoffTeam.players.find((player) => player.id === playerId)?.position === "MID"
);
import_strict.default.ok(kickoffMidfielderId);
import_strict.default.deepEqual(kickoffSnapshot.spatial.players[kickoffMidfielderId].position, { x: 34, y: 52.5 });
var homeGoalkeeper = kickoffPlayers.find((player) => player.side === "HOME" && player.role === "GK");
var awayGoalkeeper = kickoffPlayers.find((player) => player.side === "AWAY" && player.role === "GK");
import_strict.default.ok(homeGoalkeeper && homeGoalkeeper.position.y <= 8, "Bramkarz gospodarzy musi zaczyna\u0107 przy w\u0142asnej bramce.");
import_strict.default.ok(awayGoalkeeper && awayGoalkeeper.position.y >= 97, "Bramkarz go\u015Bci musi zaczyna\u0107 przy w\u0142asnej bramce.");
var kickoffOutfieldAnchors = kickoffPlayers.filter((player) => player.role !== "GK").map((player) => player.anchor);
import_strict.default.ok(
  Math.max(...kickoffOutfieldAnchors.map((point2) => point2.y)) - Math.min(...kickoffOutfieldAnchors.map((point2) => point2.y)) >= 58,
  "Ustawienie obu dru\u017Cyn powinno wykorzystywa\u0107 wi\u0119kszo\u015B\u0107 d\u0142ugo\u015Bci boiska."
);
import_strict.default.ok(
  Math.max(...kickoffOutfieldAnchors.map((point2) => point2.x)) - Math.min(...kickoffOutfieldAnchors.map((point2) => point2.x)) >= 43,
  "Ustawienie obu dru\u017Cyn powinno wykorzystywa\u0107 wi\u0119kszo\u015B\u0107 szeroko\u015Bci boiska."
);
var first = MatchEngineV2.createMatch(makeLeagueInput(202));
var second = MatchEngineV2.createMatch(makeLeagueInput(202));
var firstAt20 = MatchEngineV2.advanceTo(first, 20 * 60);
var secondAt20 = MatchEngineV2.advanceTo(second, 20 * 60);
import_strict.default.deepEqual(firstAt20.result.events, secondAt20.result.events);
import_strict.default.equal(firstAt20.result.homeScore, secondAt20.result.homeScore);
import_strict.default.equal(firstAt20.result.awayScore, secondAt20.result.awayScore);
import_strict.default.ok(firstAt20.spatial.ball.ownerId);
import_strict.default.equal(firstAt20.spatial.players[firstAt20.spatial.ball.ownerId]?.isOnPitch, true);
import_strict.default.ok(firstAt20.spatial.ball.x >= 0 && firstAt20.spatial.ball.x <= firstAt20.spatial.pitchWidth);
import_strict.default.ok(firstAt20.spatial.ball.y >= 0 && firstAt20.spatial.ball.y <= firstAt20.spatial.pitchLength);
import_strict.default.equal(firstAt20.spatial.lastEventIndex, firstAt20.result.events.length);
import_strict.default.equal(firstAt20.spatial.visualCues.length, firstAt20.result.events.length);
firstAt20.spatial.visualCues.forEach((cue) => {
  import_strict.default.ok(cue.start.x >= 0 && cue.start.x <= firstAt20.spatial.pitchWidth);
  import_strict.default.ok(cue.start.y >= 0 && cue.start.y <= firstAt20.spatial.pitchLength);
  import_strict.default.ok(cue.end.x >= 0 && cue.end.x <= firstAt20.spatial.pitchWidth);
  import_strict.default.ok(cue.end.y >= 0 && cue.end.y <= firstAt20.spatial.pitchLength);
});
var activeSpatialPlayers = Object.values(firstAt20.spatial.players).filter((player) => player.isOnPitch);
...
var homeOutfieldMovementVectors = activeSpatialPlayers.filter((player) => player.side === "HOME" && player.role !== "GK").map((player) => `${(player.target.x - player.anchor.x).toFixed(2)}:${(player.target.y - player.anchor.y).toFixed(2)}`);
import_strict.default.ok(new Set(homeOutfieldMovementVectors).size >= 6, "Zawodnicy powinni otrzymywa\u0107 indywidualne cele ruchu.");
var activeHomeGoalkeeper = activeSpatialPlayers.find((player) => player.side === "HOME" && player.role === "GK");
var activeAwayGoalkeeper = activeSpatialPlayers.find((player) => player.side === "AWAY" && player.role === "GK");
import_strict.default.ok(activeHomeGoalkeeper && activeHomeGoalkeeper.position.y <= 12, "Bramkarz gospodarzy wyszed\u0142 zbyt wysoko.");
import_strict.default.ok(activeAwayGoalkeeper && activeAwayGoalkeeper.position.y >= 93, "Bramkarz go\u015Bci wyszed\u0142 zbyt wysoko.");
import_strict.default.ok(activeSpatialPlayers.some((player) => player.movementIntent === "SUPPORT" || player.movementIntent === "RUN_BEHIND"));
import_strict.default.ok(activeSpatialPlayers.some((player) => player.movementIntent === "PRESS" || player.movementIntent === "RECOVER"));
for (let firstIndex = 0; firstIndex < activeSpatialPlayers.length; firstIndex += 1) {
  for (let secondIndex = firstIndex + 1; secondIndex < activeSpatialPlayers.length; secondIndex += 1) {
    ...
    import_strict.default.ok(renderedDistance >= 35.5, `Player icons overlap: ${firstPlayer.playerId} / ${secondPlayer.playerId}`);
  }
}
var detachedPlayerId = Object.keys(firstAt20.spatial.players)[0];
var authoritativeX = first.spatial.players[detachedPlayerId].position.x;
firstAt20.spatial.players[detachedPlayerId].position.x = -999;
import_strict.default.equal(first.spatial.players[detachedPlayerId].position.x, authoritativeX);
var detachedReport = MatchEngineV2.snapshot(first);
var authoritativeEventCount = first.core.state.events.length;
...
import_strict.default.equal(first.core.state.events.length, authoritativeEventCount);
var historyBeforeCommand = [...firstAt20.result.events];
import_strict.default.equal(MatchEngineV2.applyCommand(first, {
...
var firstAt23 = MatchEngineV2.advanceTo(first, 23 * 60);
import_strict.default.equal(firstAt23.coachState.HOME.activeInstruction?.id, "SPEED_UP");
import_strict.default.equal(firstAt23.coachState.HOME.activeShout?.id, "MOTIVATE");
import_strict.default.notEqual(firstAt23.result.finalState.coachEffects.HOME.initiativeModifier, 0);
import_strict.default.equal(firstAt23.coachPresentation.HOME.instruction?.label, "PRZYSPIESZCIE GR\u0118");
import_strict.default.equal(firstAt23.coachPresentation.HOME.instruction?.status, "ACTIVE");
import_strict.default.equal(firstAt23.coachPresentation.HOME.shout?.label, "ZMOTYWUJ");
import_strict.default.ok(firstAt23.coachPresentation.HOME.summary.includes("Polecenie aktywne"));
import_strict.default.equal(MatchEngineV2.applyCommand(second, {
  type: "UPDATE_INSTRUCTIONS",
...
var secondAt23 = MatchEngineV2.advanceTo(second, 23 * 60);
import_strict.default.deepEqual(secondAt23.result.events, firstAt23.result.events);
import_strict.default.deepEqual(secondAt23.coachState, firstAt23.coachState);
var firstAt30 = MatchEngineV2.advanceTo(first, 30 * 60);
...
import_strict.default.equal(first.commandLog.at(-1)?.reason, "COMMAND_CLOCK_MISMATCH");
var beforeBackwardRequest = MatchEngineV2.snapshot(first);
var afterBackwardRequest = MatchEngineV2.advanceTo(first, 10 * 60);
import_strict.default.equal(afterBackwardRequest.second, beforeBackwardRequest.second);
import_strict.default.deepEqual(afterBackwardRequest.result.events, beforeBackwardRequest.result.events);
var playerOutId = first.core.input.home.lineup.startingXI.find((id) => Boolean(id));
var playerInId = first.core.input.home.lineup.bench[0];
import_strict.default.ok(playerOutId && playerInId);
import_strict.default.equal(MatchEngineV2.applyCommand(first, {
  type: "SUBSTITUTION",
...
var substitutionSnapshot = MatchEngineV2.snapshot(first);
import_strict.default.equal(substitutionSnapshot.spatial.players[playerOutId].isOnPitch, false);
import_strict.default.equal(substitutionSnapshot.spatial.players[playerInId].isOnPitch, true);
import_strict.default.equal(
  Object.values(substitutionSnapshot.spatial.players).filter((player) => player.isOnPitch).length,
  22
);
var secondActivePlayer = first.core.input.home.lineup.startingXI.find(
  (id) => Boolean(id) && id !== playerInId
);
import_strict.default.ok(secondActivePlayer);
import_strict.default.equal(MatchEngineV2.applyCommand(first, {
  type: "SUBSTITUTION",
  atSecond: first.core.state.second,
  side: "HOME",
...
var finalSnapshot = MatchEngineV2.advanceTo(first, 120 * 60);
import_strict.default.equal(finalSnapshot.isFinished, true);
import_strict.default.equal(finalSnapshot.phase, "FINISHED");
import_strict.default.equal(finalSnapshot.result.decidedByPenalties, false);
import_strict.default.equal(finalSnapshot.result.penaltyScore, void 0);
import_strict.default.ok(MatchEngineV2.finalize(first));
import_strict.default.ok(finalSnapshot.result.finalState.firstHalfAddedTimeSeconds >= 60);
import_strict.default.ok(finalSnapshot.result.finalState.secondHalfAddedTimeSeconds >= 60);
import_strict.default.equal(
...
var aiInput = makeLeagueInput(282);
aiInput.coaching = {
  aiSides: ["AWAY"],
  coachAttributes: {
...
var aiFirst = MatchEngineV2.createMatch(aiInput);
var aiSecond = MatchEngineV2.createMatch(aiInput);
var aiFirstAt25 = MatchEngineV2.advanceTo(aiFirst, 25 * 60);
var aiSecondAt25 = MatchEngineV2.advanceTo(aiSecond, 25 * 60);
import_strict.default.ok(aiFirstAt25.coachState.AWAY.instructionMemory.issueCount > 0);
import_strict.default.ok(aiFirstAt25.coachState.AWAY.shoutMemory.issueCount > 0);
import_strict.default.deepEqual(aiSecondAt25.result.events, aiFirstAt25.result.events);
import_strict.default.deepEqual(aiSecondAt25.coachState.AWAY, aiFirstAt25.coachState.AWAY);
import_strict.default.equal(MatchEngineV2.applyCommand(aiFirst, {
  type: "COACH_SHOUT",
  atSecond: aiFirstAt25.second,
  side: "AWAY",
  shoutId: "PRAISE"
}), false);
import_strict.default.equal(aiFirst.commandLog.at(-1)?.reason, "SIDE_CONTROLLED_BY_AI");
var redCardSecond = aiFirst.core.state.second;
...
var afterRedCardReaction = MatchEngineV2.advanceTo(aiFirst, redCardSecond + aiFirst.core.config.tickSeconds);
import_strict.default.equal(afterRedCardReaction.coachState.AWAY.lastDecisionReason, "RED_CARD");
import_strict.default.equal(afterRedCardReaction.coachPresentation.AWAY.decisionReasonLabel, "Reakcja po czerwonej kartce");
var redCardReactionMinute = afterRedCardReaction.coachState.AWAY.lastReactiveDecisionMinute;
MatchEngineV2.advanceTo(aiFirst, aiFirst.core.state.second + 3 * 60);
var injurySubSecond = aiFirst.core.state.second;
aiFirst.core.state.events.push({
...
var afterForcedChange = MatchEngineV2.advanceTo(aiFirst, injurySubSecond + aiFirst.core.config.tickSeconds);
import_strict.default.equal(afterForcedChange.coachState.AWAY.lastDecisionReason, "FORCED_SUBSTITUTION");
import_strict.default.ok(afterForcedChange.coachState.AWAY.lastReactiveDecisionMinute > redCardReactionMinute);
...
var dominanceSecond = aiFirst.core.state.second;
var afterDominanceReaction = MatchEngineV2.advanceTo(aiFirst, dominanceSecond + aiFirst.core.config.tickSeconds);
import_strict.default.equal(afterDominanceReaction.coachState.AWAY.lastDecisionReason, "OPPONENT_DOMINANCE");
var dominanceReactionMinute = afterDominanceReaction.coachState.AWAY.lastDominanceReactionMinute;
MatchEngineV2.advanceTo(aiFirst, aiFirst.core.state.second + 5 * 60);
import_strict.default.equal(aiFirst.coachState.AWAY.lastDominanceReactionMinute, dominanceReactionMinute);
var invalidInput = makeLeagueInput(303);
...
import_strict.default.throws(() => MatchEngineV2.createMatch(invalidInput), /duplicate starter/);
