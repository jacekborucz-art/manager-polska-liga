...
// tests/MatchEngineV2TeamShapeTests.ts
...
// tests/MatchEngineV2TeamShapeTests.ts
var sample = CupSampleMatchFactory.makeInput(442, "EQUAL");
...
var initial = MatchEngineV2.snapshot(runtime).spatial;
import_strict.default.equal(Object.values(initial.players).filter((player) => player.isOnPitch).length, 22);
import_strict.default.notEqual(initial.teamContexts[runtime.core.state.possession].phase, "DEFENSIVE_SHAPE");
import_strict.default.equal(
  initial.teamContexts[runtime.core.state.possession === "HOME" ? "AWAY" : "HOME"].phase,
  "DEFENSIVE_SHAPE"
);
var homePlayers = Object.values(initial.players).filter((player) => player.side === "HOME" && player.isOnPitch);
var targetSet = (phase) => homePlayers.map((player) => ({
  player,
...
var buildUpTargets = targetSet("BUILD_UP");
var attackTargets = targetSet("ATTACK");
var finalThirdTargets = targetSet("FINAL_THIRD");
var averageOutfieldY = (targets) => {
  const outfield = targets.filter((item) => item.player.role !== "GK" /* GK */);
  return outfield.reduce((sum, item) => sum + item.target.point.y, 0) / outfield.length;
};
import_strict.default.ok(averageOutfieldY(buildUpTargets) < averageOutfieldY(attackTargets));
import_strict.default.ok(averageOutfieldY(attackTargets) < averageOutfieldY(finalThirdTargets));
finalThirdTargets.forEach(({ player, target }) => {
  import_strict.default.ok(target.point.x >= player.movementZone.minX && target.point.x <= player.movementZone.maxX);
  import_strict.default.ok(target.point.y >= player.movementZone.minY && target.point.y <= player.movementZone.maxY);
});
var homeGoalkeeperTarget = finalThirdTargets.find((item) => item.player.role === "GK" /* GK */)?.target.point;
import_strict.default.ok(homeGoalkeeperTarget);
import_strict.default.ok(homeGoalkeeperTarget.y <= 11, "Bramkarz musi pozosta\u0107 w bezpiecznej strefie przy w\u0142asnej bramce.");
var averageLineY = (targets, role) => {
  const line = targets.filter((item) => item.player.role === role);
  return line.reduce((sum, item) => sum + item.target.point.y, 0) / line.length;
};
import_strict.default.ok(averageLineY(attackTargets, "DEF") < averageLineY(attackTargets, "MID"));
import_strict.default.ok(averageLineY(attackTargets, "MID") < averageLineY(attackTargets, "FWD"));
import_strict.default.deepEqual(targetSet("ATTACK"), attackTargets);
var originalPossession = runtime.core.state.possession;
var newPossession = originalPossession === "HOME" ? "AWAY" : "HOME";
...
import_strict.default.equal(runtime.spatial.teamContexts[newPossession].phase, "TRANSITION_ATTACK");
import_strict.default.equal(runtime.spatial.teamContexts[originalPossession].phase, "TRANSITION_DEFEND");
import_strict.default.ok(
  Object.values(runtime.spatial.players).filter(
    (player) => player.side === originalPossession && player.isOnPitch && player.movementIntent === "PRESS"
  ).length <= 2,
  "Po stracie do pi\u0142ki mo\u017Ce jednocze\u015Bnie doskoczy\u0107 najwy\u017Cej dw\xF3ch graczy."
);
runtime.core.state.second = 5 + MatchEngineV2TeamPhaseService.transitionDurationSeconds + 1;
MatchEngineV2SpatialService.synchronize(runtime.spatial, runtime.core);
import_strict.default.equal(runtime.spatial.teamContexts[newPossession].phase, "ATTACK");
import_strict.default.equal(runtime.spatial.teamContexts[originalPossession].phase, "DEFENSIVE_SHAPE");
Object.values(runtime.spatial.players).filter((player) => player.isOnPitch).forEach((player) => {
  import_strict.default.ok(player.position.x >= 0 && player.position.x <= runtime.spatial.pitchWidth);
  import_strict.default.ok(player.position.y >= 0 && player.position.y <= runtime.spatial.pitchLength);
  if (player.role !== "GK" /* GK */) {
