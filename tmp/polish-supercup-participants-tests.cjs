...
// tests/PolishSuperCupParticipantsTests.ts
...
// services/SuperCupService.ts
var INITIAL_SUPER_CUP_PARTICIPANTS = {
  2025: {
...
var SuperCupService = {
   ...
   */
  generateFixture: (year, clubs, championId, cupWinnerId) => {
    const historicalParticipants = !championId && !cupWinnerId ? INITIAL_SUPER_CUP_PARTICIPANTS[year] : void 0;
    const topClubs = [...clubs].filter((club) => club.leagueId === "L_PL_1").sort(
    ...
    const homeId = championId ?? historicalParticipants?.championId ?? topClubs[0]?.id ?? "PL_LEGIA_WARSZAWA";
    const potentialAwayId = cupWinnerId ?? historicalParticipants?.cupWinnerId ?? topClubs[1]?.id ?? "PL_LECH_POZNAN";
    const awayId = homeId === potentialAwayId ? topClubs.find((club) => club.id !== homeId)?.id ?? (homeId === "PL_LEGIA_WARSZAWA" ? "PL_LECH_POZNAN" : "PL_LEGIA_WARSZAWA") : potentialAwayId;
    return {
      id: `SUPER_CUP_${year}_${homeId}_${awayId}`,
      leagueId: "SUPER_CUP" /* SUPER_CUP */,
      homeTeamId: homeId,
      awayTeamId: awayId,
      date: new Date(year, 6, 12),
      // Sztywna data: 12 Lipca
...
var datapackClubs = DatapackClubService.applyCareerStartStructure(
  STATIC_CLUBS,
  ...
  stats: {
    ...club.stats,
    points: club.id === "PL_LEGIA_WARSZAWA" ? 100 : club.id === "PL_JAGIELLONIA_BIALYSTOK" ? 90 : 0
  }
}));
var initial2026Fixture = SuperCupService.generateFixture(2026, datapackClubs);
import_strict.default.equal(initial2026Fixture.homeTeamId, "PL_LECH_POZNAN");
import_strict.default.equal(initial2026Fixture.awayTeamId, "PL_GORNIK_ZABRZE");
...
var initial2025Fixture = SuperCupService.generateFixture(2025, datapackClubs);
import_strict.default.equal(initial2025Fixture.homeTeamId, "PL_LEGIA_WARSZAWA");
import_strict.default.equal(initial2025Fixture.awayTeamId, "PL_LECH_POZNAN");
var simulatedSeasonFixture = SuperCupService.generateFixture(
  2026,
  datapackClubs,
  "PL_RAKOW_CZESTOCHOWA",
  "PL_JAGIELLONIA_BIALYSTOK"
);
import_strict.default.equal(simulatedSeasonFixture.homeTeamId, "PL_RAKOW_CZESTOCHOWA");
import_strict.default.equal(simulatedSeasonFixture.awayTeamId, "PL_JAGIELLONIA_BIALYSTOK");
