...
// tests/LiveMatchMathAuditTests.ts
var import_strict = __toESM(require("node:assert/strict"), 1);

// services/match/live/LiveMatchFatigue.ts
var getAverageLiveFatigue = (lineup2, fatigueMap) => {
  const ids = lineup2.filter((id) => id !== null);
  if (ids.length === 0) return 100;
  return ids.reduce((acc, id) => acc + (fatigueMap[id] ?? 100), 0) / ids.length;
};
var calculateLiveFatiguePenalty = (averageFatigue) => {
  if (averageFatigue >= 94) return 0;
  const depth = (94 - averageFatigue) / 94;
  return -(Math.pow(depth, 1.25) * 0.42);
};
var calculateLiveRotationPenalty = ({
  minute,
  lineup: lineup2,
  fatigueMap,
  ownSubs,
...
var calculateLiveMatchFatigueImpact = (inputs) => {
...
// services/match/live/LiveMatchInitiative.ts
var clamp = (value, min, max) => Math.min(max, Math.max(min, value));
var getLegacyQualityGapCurve = (gap) => {
  const absGap = Math.abs(gap);
  if (absGap <= 2) return 0;
  const normalized = Math.min(1, (absGap - 2) / 18);
  return Math.sign(gap) * Math.pow(normalized, 1.35);
};
var calculateLiveMatchInitiative = (inputs) => {
  const midfield = Math.abs(inputs.midfieldControlDiff) <= 2 ? 0 : clamp(inputs.midfieldControlDiff * 14e-4, -0.026, 0.026);
  const quality = getLegacyQualityGapCurve(inputs.homeQualityGapLive) * 0.055;
  const shotDominance = inputs.minute < 25 || Math.abs(inputs.shotGapLive) < 8 ? 0 : -Math.sign(inputs.shotGapLive) * Math.min(0.055, (Math.abs(inputs.shotGapLive) - 7) * 6e-3) * (Math.sign(inputs.shotGapLive) === Math.sign(inputs.homeQualityGapLive) && Math.abs(inputs.homeQualityGapLive) > 8 ? 0.45 : 1);
  const modifiers = {
    fatigue: (inputs.homeFatPenalty - inputs.awayFatPenalty) * 3,
  ...
  const homeAttackChance = clamp(
    0.5 + inputs.momentum / 340 + modifiers.fatigue + modifiers.pressure + modifiers.form + modifiers.playerForm + modifiers.midfield + modifiers.quality + modifiers.shotDominance + modifiers.goalkeeperCrisis + modifiers.redCards,
    0.28,
    0.72
  );
  return {
    homeAttackChance,
    modifiers,
    profileReadinessGap: inputs.homeProfileReadiness === void 0 || inputs.awayProfileReadiness === void 0 ? null : inputs.homeProfileReadiness - inputs.awayProfileReadiness
  };
};
...
var getPolishExperienceFactor = (player) => {
  ...
  switch (player.position) {
    case "DEF" /* DEF */:
      return 0.94 + clamp3(careerMatches / 260, 0, 1) * 0.2;
    case "GK" /* GK */:
      return 0.92 + clamp3(careerMatches / 240, 0, 1) * 0.24;
    default:
      return 0.94 + clamp3(careerMatches / 260, 0, 1) * 0.08;
  }
};
...
var getPolishPerformanceFactor = (player) => {
  ...
  const sampleFactor = clamp3(minutesPlayed / 900, 0, 1);
  ...
  switch (player.position) {
    case "FWD" /* FWD */: {
      ...
      const goalsBoost = clamp3(goals / 20, 0, 1) * 0.2 + clamp3(goalsPer90 / 0.75, 0, 1) * 0.18;
      const assistsBoost = clamp3(assists / 10, 0, 1) * 0.07 + clamp3(assistsPer90 / 0.35, 0, 1) * 0.05;
      const ratingBoost = clamp3(ratingDelta * 0.1, -0.08, 0.1);
      return 1 + clamp3(sampleFactor * (goalsBoost + assistsBoost + ratingBoost), -0.1, 0.52);
    }
    case "MID" /* MID */: {
      const goalsPer90 = goals / fullMatches;
      const assistsPer90 = assists / fullMatches;
      const assistsBoost = clamp3(assists / 14, 0, 1) * 0.18 + clamp3(assistsPer90 / 0.45, 0, 1) * 0.15;
      const goalsBoost = clamp3(goals / 12, 0, 1) * 0.08 + clamp3(goalsPer90 / 0.35, 0, 1) * 0.06;
      const ratingBoost = clamp3(ratingDelta * 0.11, -0.08, 0.12);
      return 1 + clamp3(sampleFactor * (assistsBoost + goalsBoost + ratingBoost), -0.1, 0.46);
    }
    case "DEF" /* DEF */: {
      const matchFactor = clamp3(matchesPlayed / 30, 0, 1) * 0.1;
      const experienceBoost = clamp3(getCareerMatches(player) / 260, 0, 1) * 0.12;
      const ratingBoost = averageRating === null ? 0 : clamp3((averageRating - 6.6) * 0.18, -0.1, 0.22) * clamp3(matchesPlayed / 10, 0, 1);
      return 1 + clamp3(matchFactor + experienceBoost + ratingBoost, -0.1, 0.42);
    }
    case "GK" /* GK */: {
      const matchFactor = clamp3(matchesPlayed / 30, 0, 1) * 0.1;
      const experienceBoost = clamp3(getCareerMatches(player) / 240, 0, 1) * 0.14;
      const ratingBoost = averageRating === null ? 0 : clamp3((averageRating - 6.6) * 0.22, -0.1, 0.24) * clamp3(matchesPlayed / 8, 0, 1);
      return 1 + clamp3(matchFactor + experienceBoost + ratingBoost, -0.12, 0.46);
    }
    default:
      return 1;
  }
...
var calculatePolishMarketValue = (player, reputation, tier) => {
  ...
  const reputationFactor = 0.88 + clamp3(reputation, 1, 10) * 0.025;
...
var getEuropeanCommercialIndex = (club) => {
  ...
  return clamp3(countryFactor * reputationFactor * stadiumFactor * competitionFactor / 1.45, 0.45, 2.6);
};
...
var getInternationalMarketProfile = (country) => {
  ...
  const marketFactor = clamp3(0.5 + Math.sqrt(financeFactor / 1.45) * 0.55, 0.45, 1.1);
  const capScale = clamp3(marketFactor / 0.9, 0.55, 1.22);
  return {
    marketFactor,
...
var calculateInternationalMarketValue = (player, reputation, tier, country) => {
  ...
  const reputationFactor = 0.9 + clamp3(reputation, 1, 20) * 0.015;
...
var FinanceService = {
  ...
  calculatePolishLeagueSalaryCeiling: (tier, reputation) => {
    ...
    const reputationFactor = clamp3((Math.max(1, Math.min(10, reputation)) - 4) / 6, 0, 1);
  ...
  calculateMatchdayExpenses: (club, isHome, attendance) => {
    ...
    if (isEuropeanCommercialClub(club)) {
      ...
      if (isHome) {
        ...
        return Math.round(clamp3(rawCost2, minFloor, maxCap));
      }
  ...
  calculateMonthlyOperationalCosts: (club) => {
    ...
    if (isEuropeanCommercialClub(club)) {
      ...
      return Math.round(clamp3(rawCost2, 5e4, 8e7) / 1e3) * 1e3;
    }
    const tier = Math.min(4, Math.max(1, parseInt(club.leagueId.split("_")[2] || "4")));
    ...
    return Math.round(clamp3(rawCost, tierMin, tierMax) / 1e3) * 1e3;
  },
  ...
  calculateTicketPriceForClub: (club) => {
    ...
    return Math.round(clamp3(maxPrice, 45, 420));
  },
  ...
  calculateSeasonTicketPackageForClub: (club) => {
    ...
    const marketIndex = getEuropeanCommercialIndex(club);
    const seasonTicketShare = clamp3(0.14 + marketIndex * 0.1 + club.reputation / 20 * 0.18, 0.16, 0.65);
    ...
    const seasonDiscount = clamp3(0.68 + marketIndex * 0.05, 0.7, 0.82);
    const seasonTicketPrice = Math.round(clamp3(singleMatchPrice * 19 * seasonDiscount, 900, 8500));
    return {
...
// services/match/live/LiveMatchShotPressure.ts
var clamp5 = (value, min, max) => Math.max(min, Math.min(max, value));
var getActiveLineupIds = (lineup2) => {
  const ids = Array.isArray(lineup2) ? lineup2 : lineup2.startingXI;
  return ids.filter((id) => id !== null);
};
var getLiveEmergencyKeeperRead = (player) => {
  if (!player) return 0;
  return player.attributes.goalkeeping * 0.35 + player.attributes.positioning * 0.25 + player.attributes.mentality * 0.22 + player.attributes.strength * 0.18;
};
var calculateLiveNoGoalkeeperShotBonus = (defendingLineup, slotZeroPlayer) => {
  if (defendingLineup[0] === null) return 0.055;
  if (slotZeroPlayer?.position === "GK" /* GK */) return 0;
  return 0.031 + clamp5((62 - getLiveEmergencyKeeperRead(slotZeroPlayer)) / 45, 0, 1) * 0.017;
};
var calculateLiveTopStrikerShotBonus = (attackingPlayers, attackingLineup) => {
  const attackingIds = getActiveLineupIds(attackingLineup);
  const topStriker = attackingPlayers.filter((player) => attackingIds.includes(player.id) && player.position === "FWD" /* FWD */).sort((a, b) => b.attributes.finishing - a.attributes.finishing)[0];
  return topStriker ? Math.max(0, (topStriker.attributes.finishing * PlayerMoraleService.getMatchMultiplier(topStriker) - 55) / (77 - 55)) * 0.012 : 0;
};
var calculateLiveMoraleShotMultiplier = ({
  attackingPlayers,
  attackingLineup,
  moraleNoise
}) => {
  ...
  getActiveLineupIds(attackingLineup).forEach((id) => {
    ...
    const randomNoise = 1 + (moraleNoise() * 0.1 - 0.05);
    moraleTeamPenalty += effectivePenalty * randomNoise;
  });
  return Math.max(0.15, 1 - moraleTeamPenalty);
};
var calculateLiveRelativeFreshnessShotSwing = (attackingAverageFatigue, defendingAverageFatigue) => {
  return clamp5((attackingAverageFatigue - defendingAverageFatigue) / 100 * 0.18, -0.026, 0.026);
};
var calculateLiveIndividualFatigueShotImpact = ({
  minute,
  attackingLineup,
  defendingLineup,
  attackingFatigue,
  ...
  const noRotationShotPenalty = minute >= 60 && attackingSubsUsed <= 1 ? Math.min(
    0.035,
    (2 - attackingSubsUsed) * 6e-3 + tiredAttackers * 4e-3 + Math.max(0, defendingSubsUsed - attackingSubsUsed) * 4e-3
  ) * Math.min(1, (minute - 60) / 30) : 0;
  const lateFatigueShotDrag = minute >= 60 ? Math.min(0.052, noRotationShotPenalty * 0.75 + criticalFatPenalty * 0.35) : 0;
...
var adjustLiveShotThresholdForRainTechnique = ({
  shotThreshold,
  weather,
  ...
  const rainPenalty = techniqueGap > 10 ? 0.01 : techniqueGap > 6 ? 7e-3 : 4e-3;
  const rainIntensity = Math.min(1, (weather.precipitationChance - 40) / 60);
...
// services/match/live/LiveMatchDiscipline.ts
var calculateLiveFoulThreshold = ({
...
var calculateLivePenaltyIncidentChance = ({
...
// services/match/live/LiveMatchOutcome.ts
var isLiveShotOnTargetEvent = (eventType) => {
  return eventType !== "SHOT" /* SHOT */;
};
var resolveLiveMissedShotOutcome = ({
  failRoll,
  shotAccuracyRoll,
  dangerLabel,
  shotOnTargetBoost
  ...
  if (failRoll < 0.08) failType = "SHOT_POST" /* SHOT_POST */;
  else if (failRoll < 0.16) failType = "SHOT_BAR" /* SHOT_BAR */;
  else if (failRoll < 0.26) failType = "ONE_ON_ONE_SAVE" /* ONE_ON_ONE_SAVE */;
  else if (failRoll < 0.36) failType = "ONE_ON_ONE_MISS" /* ONE_ON_ONE_MISS */;
  else if (failRoll < 0.44) failType = "SAVE" /* SAVE */;
  else if (failRoll < 0.54) failType = "WINGER_STOPPED" /* WINGER_STOPPED */;
  else if (failRoll > 0.85) failType = "SHOT" /* SHOT */;
  if (dangerLabel === "big" && failType === "SHOT" /* SHOT */) failType = "ONE_ON_ONE_MISS" /* ONE_ON_ONE_MISS */;
  if (dangerLabel === "clear" && failType === "WINGER_STOPPED" /* WINGER_STOPPED */) failType = "SHOT_ON_TARGET" /* SHOT_ON_TARGET */;
  if (dangerLabel === "chaotic" && failType === "SHOT_ON_TARGET" /* SHOT_ON_TARGET */) failType = "SHOT" /* SHOT */;
  if (shotOnTargetBoost > 0 && failType === "SHOT" /* SHOT */ && shotAccuracyRoll < shotOnTargetBoost * 3) {
    failType = "SHOT_ON_TARGET" /* SHOT_ON_TARGET */;
  } else if (shotOnTargetBoost < 0 && failType !== "SHOT" /* SHOT */ && shotAccuracyRoll < Math.abs(shotOnTargetBoost) * 2) {
    failType = "SHOT" /* SHOT */;
  }
  return failType;
};
...
// tests/LiveMatchMathAuditTests.ts
var nearlyEqual = (a, b, epsilon = 1e-7) => Math.abs(a - b) <= epsilon;
var makeAttributes = (base, overrides = {}) => ({
  strength: base,
  stamina: base,
  pace: base,
  defending: base,
...
var makePlayer = (id, position, morale, overrides = {}) => ({
  id,
  firstName: id,
  lastName: id,
  position,
...
var auditPlayers = [
  makePlayer("gk", "GK" /* GK */, 55, { goalkeeping: 72, positioning: 70 }),
  makePlayer("d1", "DEF" /* DEF */, 55, { goalkeeping: 20, defending: 66 }),
  makePlayer("d2", "DEF" /* DEF */, 55, { defending: 66 }),
  makePlayer("m1", "MID" /* MID */, 55, { technique: 64, passing: 66 }),
  makePlayer("m2", "MID" /* MID */, 55, { technique: 64, passing: 66 }),
  makePlayer("m3", "MID" /* MID */, 55, { technique: 64, passing: 66 }),
  makePlayer("m4", "MID" /* MID */, 55, { technique: 64, passing: 66 }),
  makePlayer("f1", "FWD" /* FWD */, 85, { finishing: 76, mentality: 68 }),
  makePlayer("f2", "FWD" /* FWD */, 55, { finishing: 66 }),
  makePlayer("f3", "FWD" /* FWD */, 35, { finishing: 58 }),
  makePlayer("f4", "FWD" /* FWD */, 18, { finishing: 55, mentality: 50 })
];
var neutralInitiativeInputs = {
  momentum: 0,
  minute: 30,
  homeFatPenalty: 0,
  awayFatPenalty: 0,
...
var neutralInitiative = calculateLiveMatchInitiative(neutralInitiativeInputs);
import_strict.default.equal(neutralInitiative.homeAttackChance, 0.5);
import_strict.default.equal(neutralInitiative.profileReadinessGap, null);
var previousMomentumChance = 0;
for (const momentum of [-160, -80, 0, 80, 160]) {
  ...
  (0, import_strict.default)(chance >= 0.28 && chance <= 0.72);
  (0, import_strict.default)(chance >= previousMomentumChance);
  previousMomentumChance = chance;
}
import_strict.default.equal(getLegacyQualityGapCurve(2), 0);
(0, import_strict.default)(nearlyEqual(getLegacyQualityGapCurve(12), -getLegacyQualityGapCurve(-12)));
(0, import_strict.default)(calculateLiveMatchInitiative({ ...neutralInitiativeInputs, homeQualityGapLive: 14 }).homeAttackChance > 0.5);
(0, import_strict.default)(calculateLiveMatchInitiative({ ...neutralInitiativeInputs, homeQualityGapLive: -14 }).homeAttackChance < 0.5);
(0, import_strict.default)(calculateLiveMatchInitiative({ ...neutralInitiativeInputs, homeSentOffCount: 1 }).homeAttackChance < 0.5);
(0, import_strict.default)(calculateLiveMatchInitiative({ ...neutralInitiativeInputs, awaySentOffCount: 1 }).homeAttackChance > 0.5);
import_strict.default.equal(calculateLiveFatiguePenalty(100), 0);
import_strict.default.equal(calculateLiveFatiguePenalty(94), 0);
(0, import_strict.default)(calculateLiveFatiguePenalty(80) < calculateLiveFatiguePenalty(88));
(0, import_strict.default)(calculateLiveFatiguePenalty(65) < calculateLiveFatiguePenalty(80));
var freshFatigue = Object.fromEntries(lineup.startingXI.filter(Boolean).map((id) => [id, 96]));
var tiredFatigue = Object.fromEntries(lineup.startingXI.filter(Boolean).map((id) => [id, 78]));
var exhaustedFatigue = Object.fromEntries(lineup.startingXI.filter(Boolean).map((id) => [id, 66]));
import_strict.default.equal(calculateLiveMatchFatigueImpact({ minute: 45, lineup: lineup.startingXI, fatigueMap: freshFatigue, ownSubs: 0, opponentSubs: 0 }).rotationPenalty, 0);
(0, import_strict.default)(calculateLiveMatchFatigueImpact({ minute: 75, lineup: lineup.startingXI, fatigueMap: tiredFatigue, ownSubs: 0, opponentSubs: 2 }).rotationPenalty < 0);
import_strict.default.equal(calculateLiveMatchFatigueImpact({ minute: 75, lineup: lineup.startingXI, fatigueMap: tiredFatigue, ownSubs: 2, opponentSubs: 0 }).rotationPenalty, 0);
var homeFreshAwayTired = calculateLiveMatchInitiative({
  ...neutralInitiativeInputs,
  homeFatPenalty: calculateLiveFatiguePenalty(96),
  awayFatPenalty: calculateLiveFatiguePenalty(78)
}).homeAttackChance;
var homeTiredAwayFresh = calculateLiveMatchInitiative({
  ...neutralInitiativeInputs,
  homeFatPenalty: calculateLiveFatiguePenalty(78),
  awayFatPenalty: calculateLiveFatiguePenalty(96)
}).homeAttackChance;
(0, import_strict.default)(homeFreshAwayTired > 0.5);
(0, import_strict.default)(homeTiredAwayFresh < 0.5);
(0, import_strict.default)(homeFreshAwayTired > homeTiredAwayFresh);
var realGoalkeeper = auditPlayers.find((player) => player.id === "gk");
var fieldEmergencyKeeper = auditPlayers.find((player) => player.id === "d1");
import_strict.default.equal(calculateLiveNoGoalkeeperShotBonus(["gk"], realGoalkeeper), 0);
(0, import_strict.default)(calculateLiveNoGoalkeeperShotBonus(["d1"], fieldEmergencyKeeper) > calculateLiveNoGoalkeeperShotBonus(["gk"], realGoalkeeper));
(0, import_strict.default)(calculateLiveNoGoalkeeperShotBonus([null], null) > calculateLiveNoGoalkeeperShotBonus(["d1"], fieldEmergencyKeeper));
var highStrikerBonus = calculateLiveTopStrikerShotBonus(auditPlayers, lineup.startingXI);
var lowStrikerBonus = calculateLiveTopStrikerShotBonus(
  [makePlayer("weak", "FWD" /* FWD */, 50, { finishing: 52 })],
  ["weak"]
);
(0, import_strict.default)(highStrikerBonus > lowStrikerBonus);
var highMoraleMultiplier = calculateLiveMoraleShotMultiplier({
  attackingPlayers: [makePlayer("calm", "FWD" /* FWD */, 85, { mentality: 60 })],
  attackingLineup: ["calm"],
  moraleNoise: () => 0.5
});
var lowMoraleMultiplier = calculateLiveMoraleShotMultiplier({
  attackingPlayers: [makePlayer("nervous", "FWD" /* FWD */, 18, { mentality: 60 })],
  attackingLineup: ["nervous"],
  moraleNoise: () => 0.5
});
(0, import_strict.default)(highMoraleMultiplier > lowMoraleMultiplier);
(0, import_strict.default)(lowMoraleMultiplier >= 0.15);
import_strict.default.equal(calculateLiveRelativeFreshnessShotSwing(100, 100), 0);
(0, import_strict.default)(calculateLiveRelativeFreshnessShotSwing(95, 75) > 0);
(0, import_strict.default)(calculateLiveRelativeFreshnessShotSwing(75, 95) < 0);
var tiredShotFatigue = calculateLiveIndividualFatigueShotImpact({
  minute: 75,
...
var freshShotFatigue = calculateLiveIndividualFatigueShotImpact({
  minute: 75,
  attackingLineup: lineup.startingXI,
  defendingLineup: lineup.startingXI,
...
(0, import_strict.default)(tiredShotFatigue.criticalFatPenalty > freshShotFatigue.criticalFatPenalty);
(0, import_strict.default)(tiredShotFatigue.freshDefBonus > freshShotFatigue.freshDefBonus);
(0, import_strict.default)(tiredShotFatigue.noRotationShotPenalty > freshShotFatigue.noRotationShotPenalty);
var rain = {
  tempC: 12,
...
var weakTechniqueRain = adjustLiveShotThresholdForRainTechnique({
  shotThreshold: 0.12,
  weather: rain,
  attackingTechniqueAverage: 55,
  defendingTechniqueAverage: 68
});
var strongTechniqueRain = adjustLiveShotThresholdForRainTechnique({
  shotThreshold: 0.12,
  weather: rain,
  attackingTechniqueAverage: 70,
  defendingTechniqueAverage: 60
});
(0, import_strict.default)(weakTechniqueRain.threshold < 0.12);
import_strict.default.equal(strongTechniqueRain.threshold, 0.12);
import_strict.default.equal(calculateLiveFoulThreshold({ intensityFoulMultiplier: 1, pressureCardMultiplier: 1, rivalryMultiplier: 1 }), 0.043);
(0, import_strict.default)(calculateLiveFoulThreshold({ intensityFoulMultiplier: 1.5, pressureCardMultiplier: 1.3, rivalryMultiplier: 1.2 }) > 0.043);
import_strict.default.equal(calculateLivePenaltyIncidentChance({ intensityPenaltyMultiplier: 1, pressurePenaltyMultiplier: 1 }), 0.0956);
(0, import_strict.default)(calculateLivePenaltyIncidentChance({ intensityPenaltyMultiplier: 1.4, pressurePenaltyMultiplier: 1.2 }) > 0.0956);
var nonGoalEvents = Array.from(
  { length: 1e3 },
  (_, index) => resolveLiveMissedShotOutcome({
    failRoll: (index + 0.5) / 1e3,
    shotAccuracyRoll: 0.99,
    dangerLabel: "normal",
    shotOnTargetBoost: 0
  })
);
var onTargetShare = nonGoalEvents.filter(isLiveShotOnTargetEvent).length / nonGoalEvents.length;
(0, import_strict.default)(onTargetShare >= 0.84 && onTargetShare <= 0.86);
(0, import_strict.default)(nonGoalEvents.includes("SHOT_POST" /* SHOT_POST */));
(0, import_strict.default)(nonGoalEvents.includes("SHOT_BAR" /* SHOT_BAR */));
(0, import_strict.default)(nonGoalEvents.includes("ONE_ON_ONE_SAVE" /* ONE_ON_ONE_SAVE */));
(0, import_strict.default)(nonGoalEvents.includes("ONE_ON_ONE_MISS" /* ONE_ON_ONE_MISS */));
(0, import_strict.default)(nonGoalEvents.includes("SAVE" /* SAVE */));
(0, import_strict.default)(nonGoalEvents.includes("WINGER_STOPPED" /* WINGER_STOPPED */));
(0, import_strict.default)(nonGoalEvents.includes("SHOT" /* SHOT */));
