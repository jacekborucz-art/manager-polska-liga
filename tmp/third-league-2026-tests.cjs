...
// services/LeagueScheduleGenerator.ts
var LeagueScheduleGenerator = {
  generate: (clubs2, seasonTemplate, leagueTier2, leagueId, scheduleSeed) => {
    if (clubs2.length !== 18) {
      throw new Error(`League Generation Error: Expected 18 active clubs for Tier ${leagueTier2}, got ${clubs2.length}`);
    }
    const leagueSlots = seasonTemplate.slots.filter((s) => s.competition === "LEAGUE" /* LEAGUE */).sort((a, b) => a.start.getTime() - b.start.getTime());
    ...
    const ids = clubs2.map((c) => c.id);
    const n = ids.length;
    const numRounds = n - 1;
    const matchesPerRound = n / 2;
    let rngState = (scheduleSeed ?? 0) >>> 0;
    const random = scheduleSeed === void 0 ? Math.random : () => {
    ...
    for (let i = 0; i < 34; i++) {
      ...
      const fixtures = basePairings.map((pair, idx) => {
        ...
        return {
          ...
          awayTeamId: awayId,
          date: slot.start,
          // All matches start at slot start date (simplified)
...
var PolishThirdLeagueSeasonTransitionService = {
  resolve(clubs2, players, sessionSeed, seasonNumber, projectedLeagueByClubId, overrides = {}, fourthLeagueState) {
    ...
    const secondLeagueTable2 = table(clubs2, "L_PL_3");
    const incumbents2 = secondLeagueTable2.slice(12, 14);
    ...
    const directRelegated = [...new Map(
      [...secondLeagueTable2.slice(14, 18), ...secondLeagueTable2.filter((club) => forcedReserveRelegationIds.has(club.id))].map((club) => [club.id, club])
    ).values()];
    ...
    firstStage.forEach((firstMatch, pairIndex) => {
      ...
      const incumbent = incumbents2[pairIndex];
...
// services/LeagueStatsService.ts
var EMPTY_STATS2 = {
  goals: 0,
  assists: 0,
  yellowCards: 0,
...
var getStatsForLeagueId = (player, leagueId) => {
  if (leagueId === "L_CL" || leagueId === "L_EL" || leagueId === "L_CONF") {
    return player.euroStats ?? EMPTY_STATS2;
  }
  if (leagueId && player.competitionStats) {
    return player.competitionStats[leagueId] ?? EMPTY_STATS2;
  }
  if (leagueId?.startsWith("L_PL_4_G")) {
    return EMPTY_STATS2;
  }
  return player.stats ?? EMPTY_STATS2;
};
var LeagueStatsService = {
  getStatsForLeagueId,
  /**
   * Helper to get all players from valid clubs in a league
   */
  getPlayersForLeague: (leagueId, clubs2, playersMap) => {
    const leagueClubs = clubs2.filter((c) => c.leagueId === leagueId && c.isDefaultActive);
    const rows = [];
   ...
   * 1. Top Scorers
   */
  getTopScorers: (rows, limit = 50, leagueId) => {
    return [...rows].filter((row) => getStatsForLeagueId(row.player, leagueId).goals > 0).sort((a, b) => {
   ...
   * 2. Top Assists
   */
  getTopAssists: (rows, limit = 50, leagueId) => {
    return [...rows].filter((row) => getStatsForLeagueId(row.player, leagueId).assists > 0).sort((a, b) => {
      const aStats = getStatsForLeagueId(a.player, leagueId);
      const bStats = getStatsForLeagueId(b.player, leagueId);
      if (bStats.assists !== aStats.assists) {
        return bStats.assists - aStats.assists;
   ...
   */
  getYellowCardsList: (rows, limit = 50, leagueId) => {
    return [...rows].filter((row) => getStatsForLeagueId(row.player, leagueId).yellowCards > 0).sort((a, b) => {
      const aStats = getStatsForLeagueId(a.player, leagueId);
      const bStats = getStatsForLeagueId(b.player, leagueId);
   ...
   */
  getRedCardsList: (rows, limit = 50, leagueId) => {
    return [...rows].filter((row) => (getStatsForLeagueId(row.player, leagueId).redCards || 0) > 0).sort((a, b) => (getStatsForLeagueId(b.player, leagueId).redCards || 0) - (getStatsForLeagueId(a.player, leagueId).redCards || 0)).slice(0, limit);
  },
  /**
   * 5. Injuries
   */
  getInjuryList: (rows, limit = 50) => {
    return [...rows].filter((row) => row.player.health.status === "INJURED" /* INJURED */).sort((a, b) => {
      const daysA = a.player.health.injury?.daysRemaining || 999;
      const daysB = b.player.health.injury?.daysRemaining || 999;
      return daysA - daysB;
    }).slice(0, limit);
  },
  getAppearancesList: (rows, limit = 50, leagueId) => [...rows].filter((row) => getStatsForLeagueId(row.player, leagueId).matchesPlayed > 0).sort(
    (a, b) => getStatsForLeagueId(b.player, leagueId).matchesPlayed - getStatsForLeagueId(a.player, leagueId).matchesPlayed || getStatsForLeagueId(b.player, leagueId).minutesPlayed - getStatsForLeagueId(a.player, leagueId).minutesPlayed || a.player.lastName.localeCompare(b.player.lastName)
  ).slice(0, limit),
  getMinutesList: (rows, limit = 50, leagueId) => [...rows].filter((row) => getStatsForLeagueId(row.player, leagueId).minutesPlayed > 0).sort(
    (a, b) => getStatsForLeagueId(b.player, leagueId).minutesPlayed - getStatsForLeagueId(a.player, leagueId).minutesPlayed || a.player.lastName.localeCompare(b.player.lastName)
  ).slice(0, limit),
  getCleanSheetsList: (rows, limit = 50, leagueId) => [...rows].filter((row) => getStatsForLeagueId(row.player, leagueId).cleanSheets > 0).sort(
    (a, b) => getStatsForLeagueId(b.player, leagueId).cleanSheets - getStatsForLeagueId(a.player, leagueId).cleanSheets || getStatsForLeagueId(a.player, leagueId).matchesPlayed - getStatsForLeagueId(b.player, leagueId).matchesPlayed
  ).slice(0, limit),
  getAverageRatingList: (rows, limit = 50, leagueId) => [...rows].filter((row) => getStatsForLeagueId(row.player, leagueId).ratingHistory.length > 0).sort((a, b) => {
    const average2 = (row) => {
      const ratings = getStatsForLeagueId(row.player, leagueId).ratingHistory;
      return ratings.reduce((sum, rating) => sum + rating, 0) / ratings.length;
    };
    return average2(b) - average2(a) || a.player.lastName.localeCompare(b.player.lastName);
  }).slice(0, limit)
};

// tests/ThirdLeague2026Tests.ts
var clubs = PolishLeagueSeasonService.buildClubsForCareerStart(STATIC_CLUBS, 2026);
var membershipIds = THIRD_LEAGUE_GROUP_IDS.flatMap(
  (groupId) => clubs.filter((club) => club.leagueId === groupId).map((club) => club.id)
);
import_strict.default.equal(membershipIds.length, 72, "The four III-liga groups must contain exactly 72 clubs.");
import_strict.default.equal(new Set(membershipIds).size, 72, "A club may not appear in two III-liga groups.");
ReserveTeamLeagueService.getParentReservePairs().forEach(({ reserveClubId, parentClubId }) => {
  ...
  import_strict.default.ok(reserveClub, `Configured reserve club ${reserveClubId} must exist in the Polish database.`);
  import_strict.default.ok(parentClub, `Configured parent club ${parentClubId} must exist in the Polish database.`);
  import_strict.default.ok(parentClub.logoFile, `Parent club ${parentClubId} must provide a logo file.`);
...
THIRD_LEAGUE_GROUP_IDS.forEach((groupId, groupIndex) => {
  ...
  import_strict.default.equal(groupClubs.length, 18, `${groupId} must contain 18 clubs.`);
  const schedule = LeagueScheduleGenerator.generate(groupClubs, template, 4, groupId, 202600 + groupIndex);
  import_strict.default.equal(schedule.matchdays.length, 34, `${groupId} must contain 34 rounds.`);
  import_strict.default.ok(schedule.matchdays.every((matchday) => matchday.fixtures.length === 9), "Every round must contain nine matches.");
  const fixtures = schedule.matchdays.flatMap((matchday) => matchday.fixtures);
  import_strict.default.equal(fixtures.length, 306, `${groupId} must contain 306 fixtures.`);
  const pairCounts = /* @__PURE__ */ new Map();
  fixtures.forEach((fixture) => {
    const pairKey = [fixture.homeTeamId, fixture.awayTeamId].sort().join("|");
    pairCounts.set(pairKey, (pairCounts.get(pairKey) ?? 0) + 1);
  });
  import_strict.default.equal(pairCounts.size, 153, "Every unique pair must occur once in the pair map.");
  import_strict.default.ok([...pairCounts.values()].every((count) => count === 2), "Every pair must play home and away.");
});
var rankedClubs = clubs.map((club) => {
  const league = club.leagueId;
  const groupIndex = THIRD_LEAGUE_GROUP_IDS.indexOf(league);
...
var resolution = PolishThirdLeagueSeasonTransitionService.resolve(rankedClubs, {}, 123456, 1);
import_strict.default.equal(resolution.playoffMatches.length, 6, "The playoff must contain two single matches and two two-legged ties.");
import_strict.default.ok(resolution.promotedToSecondLeagueIds.length >= 4 && resolution.promotedToSecondLeagueIds.length <= 6);
var expectedRunnerUpIds = THIRD_LEAGUE_GROUP_IDS.map(
  (groupId) => rankedClubs.filter((club) => club.leagueId === groupId).sort(
    (a, b) => b.stats.points - a.stats.points || b.stats.goalDifference - a.stats.goalDifference || b.stats.goalsFor - a.stats.goalsFor || a.id.localeCompare(b.id)
  )[1].id
).sort();
var expectedChampionIds = THIRD_LEAGUE_GROUP_IDS.map(
  (groupId) => rankedClubs.filter((club) => club.leagueId === groupId).sort(
    (a, b) => b.stats.points - a.stats.points || b.stats.goalDifference - a.stats.goalDifference || b.stats.goalsFor - a.stats.goalsFor || a.id.localeCompare(b.id)
  )[0].id
);
...
var firstStageParticipantIds = resolution.playoffMatches.filter((match) => match.stage === "RUNNERS_UP").flatMap((match) => [match.homeClubId, match.awayClubId]).sort();
...
var otherSeedParticipantIds = PolishThirdLeagueSeasonTransitionService.resolve(rankedClubs, {}, 987654, 1).playoffMatches.filter((match) => match.stage === "RUNNERS_UP").flatMap((match) => [match.homeClubId, match.awayClubId]).sort();
import_strict.default.deepEqual(otherSeedParticipantIds, expectedRunnerUpIds, "Changing the draw seed may change pairs, but never participants.");
var finalFirstLegs = resolution.playoffMatches.filter((match) => match.stage === "SECOND_LEAGUE_FIRST_LEG");
var firstStageWinnerIds = new Set(
...
THIRD_LEAGUE_GROUP_IDS.forEach((groupId) => {
  import_strict.default.equal(resolution.promotedFromFeederByGroup[groupId].length, 5, `${groupId} must receive five feeder clubs.`);
  const nextCount = rankedClubs.filter(
    (club) => (resolution.nextLeagueByClubId.get(club.id) ?? club.leagueId) === groupId
  ).length;
  import_strict.default.equal(nextCount, 18, `${groupId} must be restored to 18 clubs after all movements.`);
});
var secondLeagueTable = rankedClubs.filter((club) => club.leagueId === "L_PL_3").sort(
  (a, b) => b.stats.points - a.stats.points || b.stats.goalDifference - a.stats.goalDifference || b.stats.goalsFor - a.stats.goalsFor || a.id.localeCompare(b.id)
);
var challengers = resolution.playoffMatches.filter((match) => match.stage === "RUNNERS_UP").map((match) => match.winnerClubId);
var incumbents = secondLeagueTable.slice(12, 14);
...
var interactiveResolution = PolishThirdLeagueSeasonTransitionService.resolve(
  rankedClubs,
  {},
  123456,
  1,
...
import_strict.default.equal(interactiveResolution.promotedToSecondLeagueIds.length, 4);
import_strict.default.equal(
...
for (let outcomeMask = 0; outcomeMask < 4; outcomeMask++) {
  ...
  THIRD_LEAGUE_GROUP_IDS.forEach((groupId) => {
    const nextCount = rankedClubs.filter(
      (club) => (exhaustiveResolution.nextLeagueByClubId.get(club.id) ?? club.leagueId) === groupId
    ).length;
    import_strict.default.equal(nextCount, 18, `${groupId} must contain 18 clubs for playoff outcome mask ${outcomeMask}.`);
  });
}
var reserveChampionClubs = rankedClubs.map((club) => {
...
var reserveConflictProjection = /* @__PURE__ */ new Map([["PL_WIDZEW_LODZ", "L_PL_3"]]);
var reserveChampionResolution = PolishThirdLeagueSeasonTransitionService.resolve(
...
import_strict.default.ok(!reserveChampionResolution.promotedToSecondLeagueIds.includes("PL_WIDZEW_LODZ_II"));
var groupOneAfterBlockedPromotion = reserveChampionClubs.filter(
  (club) => (reserveChampionResolution.nextLeagueByClubId.get(club.id) ?? club.leagueId) === "L_PL_4_G1"
).length;
import_strict.default.equal(groupOneAfterBlockedPromotion, 18, "A blocked reserve promotion must not leave its group with 19 clubs.");
var forcedReserveProjection = /* @__PURE__ */ new Map([["PL_SLASK_WROCLAW", "L_PL_3"]]);
var forcedReserveResolution = PolishThirdLeagueSeasonTransitionService.resolve(
  rankedClubs,
  {},
  123456,
  1,
...
THIRD_LEAGUE_GROUP_IDS.forEach((groupId) => {
  const nextCount = rankedClubs.filter(
    (club) => (forcedReserveResolution.nextLeagueByClubId.get(club.id) ?? club.leagueId) === groupId
  ).length;
  import_strict.default.equal(nextCount, 18, `${groupId} must stay balanced after a forced reserve relegation.`);
});
var statsClub = clubs.find((club) => club.leagueId === "L_PL_4_G1");
var statsSquad = SquadGeneratorService.generateSquadForClub(statsClub.id, statsClub);
var statsPlayerId = statsSquad[0].id;
var statsMap = { [statsClub.id]: statsSquad };
statsMap = PlayerStatsService.processMatchDayEndForClub(statsMap, statsClub.id, [statsPlayerId], "L_PL_4_G1", { [statsPlayerId]: 37 });
statsMap = PlayerStatsService.applyGoal(statsMap, statsPlayerId, void 0, "L_PL_4_G1");
statsMap = PlayerStatsService.applyCard(statsMap, statsPlayerId, "YELLOW_CARD" /* YELLOW_CARD */, "L_PL_4_G1");
var statsPlayer = statsMap[statsClub.id][0];
import_strict.default.equal(LeagueStatsService.getStatsForLeagueId(statsPlayer, "L_PL_4_G1").matchesPlayed, 1);
import_strict.default.equal(LeagueStatsService.getStatsForLeagueId(statsPlayer, "L_PL_4_G1").minutesPlayed, 37);
import_strict.default.equal(LeagueStatsService.getStatsForLeagueId(statsPlayer, "L_PL_4_G1").goals, 1);
import_strict.default.equal(LeagueStatsService.getStatsForLeagueId(statsPlayer, "L_PL_4_G1").yellowCards, 1);
import_strict.default.equal(LeagueStatsService.getStatsForLeagueId(statsPlayer, "L_PL_4_G2").goals, 0, "A transfer must not leak goals into the destination group.");
