...
var PlayerAttributesGenerator = {
  ...
  generateAttributes: (position, leagueTier2, clubReputation, age, isEuropean = false, talentConfig, regionProfile) => {
    const configTable = isEuropean ? EUROPEAN_TIER_CONFIG : TIER_CONFIG;
    const config = talentConfig ?? (configTable[leagueTier2] || configTable[4]);
...
var getRandomPolishClubForeignRegion = (leagueTier2) => {
  const brazilWeight = leagueTier2 <= 2 ? 5 : 0;
...
var injectRegionalStarProspects = (squad, leagueTier2, clubReputation, profile) => {
  if (!profile || Math.random() >= profile.firstTalentChance) return;
  const candidateIndexes = squad.map((player, index) => ({ player, index })).filter(({ player }) => !profile.preferredRegions || profile.preferredRegions.includes(player.nationality)).map(({ index }) => index);
...
var SquadGeneratorService = {
  generateSquadForClub: (clubId, clubOverride) => {
    ...
    let clubRep = 1;
    if (clubInfo) {
      if (clubInfo.leagueId === "L_PL_1") leagueTier2 = 1;
      else if (clubInfo.leagueId === "L_PL_2") leagueTier2 = 2;
      else if (clubInfo.leagueId === "L_PL_3") leagueTier2 = 3;
      else leagueTier2 = 4;
    ...
    const squadBase = slots.map((slot, index) => {
      let namePair;
      let fullName;
      let region = slot.isForeign ? getRandomPolishClubForeignRegion(leagueTier2) : "POLAND" /* POLAND */;
      let attempts = 0;
      do {
      ...
      const genData = PlayerAttributesGenerator.generateAttributes(slot.pos, leagueTier2, clubRep, age);
      return {
    ...
    if (leagueTier2 >= 3) {
      ...
      const injectTalent = (squad) => {
        ...
        const genData = PlayerAttributesGenerator.generateAttributes(p.position, leagueTier2, clubRep, talentAge, false, TALENT_CONFIG);
    ...
    const finalSquad = squadBase.map((p) => ({
      ...
      marketValue: FinanceService.calculateMarketValue(p, clubRep, leagueTier2, clubInfo?.country)
    }));
  ...
  generateReservesSquad: (clubId, clubName, leagueTier2, clubRep, clubBudget = 5e6) => {
    const usedNames = /* @__PURE__ */ new Set();
    ...
    const resTier = Math.min(leagueTier2 + 1, 4);
  ...
  generateYouthPlayersForClub: (club, currentSquad, seasonYear, targetSquadSize = 22) => {
    ...
    const avgSalary = currentSquad.length > 0 ? currentSquad.reduce((sum, p) => sum + (p.annualSalary || 0), 0) / currentSquad.length : FinanceService.getWagePool(club.budget) / 22;
    ...
    const localRegion = isPolish ? "POLAND" /* POLAND */ : youthCountryToRegion[club.country ?? ""] ?? "SSA" /* SSA */;
    ...
    return positions.map((pos, i) => {
      ...
      return {
        ...
        annualSalary,
        marketValue: FinanceService.calculateMarketValue(
          { overallRating: genData.overall, age, clubId: club.id },
          reputation,
          tier,
          club.country
        ),
...
// services/DatapackClubService.ts
var POLISH_LEAGUE_PREFIX = "L_PL_";
var isPolishClub = (club) => club.id.startsWith("PL_") || club.country === "POL" || club.leagueId?.startsWith(POLISH_LEAGUE_PREFIX) === true;
var omitPolishLeagueStructure = (club) => {
...
var DatapackClubService = {
  isPolishClub,
  prepareClubForExport(club) {
    return omitPolishLeagueStructure(club);
  },
  applyCareerStartStructure(baseClubs, importedClubs, startYear) {
    ...
    const customClubs = importedClubs.filter((club) => !baseIds.has(club.id)).map(
      (club) => isPolishClub(club) ? {
  ...
  ensureSquads(clubs, sourcePlayers, excludedClubIds = /* @__PURE__ */ new Set()) {
    ...
    return {
      ...
      clubs: clubs.map((club) => ({
        ...club,
        rosterIds: Array.isArray(players[club.id]) ? players[club.id].map((player) => player.id) : club.rosterIds
      }))
    };
  }
};
...
// tests/DatapackClubTests.ts
var legia = STATIC_CLUBS.find((club) => club.id === "PL_LEGIA_WARSZAWA");
var importedLegia = {
  ...legia,
  name: "Legia Testowa",
...
var regionalCustomClub = {
  ...customClub,
  id: "PL_CUSTOM_KLUB_MAZOWIECKI",
  name: "Datapack Mazowsze",
  shortName: "DMA",
  polishVoivodeship: "mazowieckie"
};
var preparedClubs = DatapackClubService.applyCareerStartStructure(
  STATIC_CLUBS,
  [importedLegia, customClub],
  2025
);
var preparedLegia = preparedClubs.find((club) => club.id === legia.id);
import_node_assert.strict.equal(preparedLegia.name, "Legia Testowa", "datapack musi zachowa\u0107 edytowan\u0105 nazw\u0119 klubu");
import_node_assert.strict.deepEqual(preparedLegia.colorsHex, ["#123456", "#abcdef"], "datapack musi zachowa\u0107 kolory klubu");
import_node_assert.strict.equal(preparedLegia.leagueId, "L_PL_1", "datapack nie mo\u017Ce nadpisa\u0107 oficjalnej ligi klubu");
import_node_assert.strict.equal(preparedLegia.tier, 1, "poziom klubu musi wynika\u0107 z konfiguracji sezonu");
import_node_assert.strict.equal(preparedLegia.isDefaultActive, true, "klub z oficjalnej ligi musi by\u0107 aktywny");
var preparedCustomClub = preparedClubs.find((club) => club.id === customClub.id);
import_node_assert.strict.equal(preparedCustomClub.leagueId, "L_PL_4", "nowy polski klub z datapacka musi trafi\u0107 do puli ligowej");
import_node_assert.strict.equal(preparedCustomClub.tier, 4, "nowy polski klub nie mo\u017Ce sam przypisa\u0107 si\u0119 do grywalnej ligi");
var preparedClubs2026 = DatapackClubService.applyCareerStartStructure(
  STATIC_CLUBS,
...
import_node_assert.strict.equal(
  ...
  "L_PL_5",
  "niestandardowy klub z datapacka bez wojew\xF3dztwa musi pozosta\u0107 w zapasowej puli regionalnej"
);
import_node_assert.strict.equal(
  preparedClubs2026.find((club) => club.id === regionalCustomClub.id)?.leagueId,
  FOURTH_LEAGUE_FEEDER_BY_VOIVODESHIP.mazowieckie,
  "klub z datapacka musi trafi\u0107 do puli swojego wojew\xF3dztwa"
);
import_node_assert.strict.equal(
  preparedClubs2026.filter((club) => club.leagueId === FOURTH_LEAGUE_FEEDER_BY_VOIVODESHIP.mazowieckie).length,
  18,
  "datapack nie mo\u017Ce zmieni\u0107 rozmiaru wojew\xF3dzkiej puli awansowej"
);
["L_PL_1", "L_PL_2", "L_PL_3"].forEach((leagueId) => {
  import_node_assert.strict.equal(
    preparedClubs.filter((club) => club.leagueId === leagueId).length,
    18,
    `${leagueId} musi zawiera\u0107 dok\u0142adnie 18 klub\xF3w po imporcie datapacka`
  );
});
var exportedPolishClub = DatapackClubService.prepareClubForExport(importedLegia);
import_node_assert.strict.equal("leagueId" in exportedPolishClub, false, "eksport nie mo\u017Ce zapisywa\u0107 ligi polskiego klubu");
import_node_assert.strict.equal("tier" in exportedPolishClub, false, "eksport nie mo\u017Ce zapisywa\u0107 poziomu ligi polskiego klubu");
import_node_assert.strict.equal("isDefaultActive" in exportedPolishClub, false, "eksport nie mo\u017Ce zapisywa\u0107 aktywno\u015Bci ligowej polskiego klubu");
var squadResult = DatapackClubService.ensureSquads([preparedCustomClub], {});
import_node_assert.strict.ok(squadResult.players[customClub.id].length >= 26, "nowy klub bez kadry musi otrzyma\u0107 zawodnik\xF3w");
import_node_assert.strict.equal(squadResult.generatedClubIds[0], customClub.id);
