...
// tests/FourthLeague2026Tests.ts
...
var FinanceService = {
  ...
  compareMultipleOffers: (offers, clubs2) => {
    return [...offers].sort((a, b) => {
      const clubA = clubs2.find((c) => c.id === a.clubId);
      const clubB = clubs2.find((c) => c.id === b.clubId);
...
var loadClubsForTier = (tier, leagueId, limit) => {
  ...
  if (tier < 4) {
    const activeCount = clubs2.filter((c) => c.isDefaultActive).length;
    if (activeCount < limit) {
      const missing = limit - activeCount;
      for (let i = 0; i < missing; i++) {
        clubs2.push(generatePlaceholderClub(leagueId, i + 1, tier));
      }
    }
  }
  return clubs2;
...
var makePlayerStats = (leagueId, clubs2) => clubs2.flatMap((club) => Array.from({ length: 11 }, (_, index) => {
...
var createSchedule = (leagueId, clubIds, template2, seed) => {
  ...
  const leagueSlots = template2.slots.filter((slot) => String(slot.competition) === "LEAGUE").sort((left, right) => left.start.getTime() - right.start.getTime());
  ...
  return Array.from({ length: roundCount }, (_, roundIndex) => {
    ...
    return base.map((pair, matchIndex) => ({
      id: `IV_${template2.seasonStartYear}_${leagueId}_R${roundIndex + 1}_M${matchIndex + 1}`,
      leagueId,
      round: roundIndex + 1,
...
var PolishFourthLeagueService = {
   ...
   */
  ensureRegionalFeederPools(clubs2, seasonStartYear) {
    let result = clubs2.map((club) => ({ ...club }));
  ...
  createSeason(clubs2, template2, seed) {
    const resetClubs = clubs2.map((club) => this.isFourthLeagueId(club.leagueId) ? { ...club, tier: 5, isDefaultActive: false, stats: EMPTY_STATS() } : club);
    const fixtures = {};
    const playerStats = {};
    FOURTH_LEAGUE_IDS.forEach((leagueId, index) => {
      ...
      fixtures[leagueId] = createSchedule(leagueId, leagueClubs.map((club) => club.id), template2, seed + 500 + index);
      playerStats[leagueId] = makePlayerStats(leagueId, leagueClubs);
    });
    return { state: { seasonStartYear: template2.seasonStartYear, fixtures, playerStats }, clubs: resetClubs };
  },
  rebalanceForNextSeason(clubs2, seasonStartYear, seed) {
    let result = this.ensureRegionalFeederPools(clubs2, seasonStartYear);
  ...
  processDate(state, clubs2, date, seed) {
    if (!state) return { state, clubs: clubs2, played: 0 };
    const cutoff = date.getTime();
    const clubById = new Map(clubs2.map((club) => [club.id, club]));
    ...
    if (played === 0) return { state, clubs: clubs2, played: 0 };
    return {
      state: { ...state, fixtures: nextFixtures, playerStats: nextPlayerStats },
      clubs: clubs2.map((club) => clubById.get(club.id) ?? club),
      played
    };
  },
  getTable(clubs2, leagueId) {
    return clubs2.filter((club) => club.leagueId === leagueId).sort(
  ...
  canReserveEnterThirdLeague(club, clubs2) {
    const parentName = reserveParentName(club.name);
    if (!parentName) return true;
    const normalizedParentName = parentName.toLocaleLowerCase("pl-PL");
    const parent = clubs2.find((candidate) => {
  ...
  canReserveEnterFourthLeague(club, clubs2) {
    const parentName = reserveParentName(club.name);
    if (!parentName) return true;
    const normalizedParentName = parentName.toLocaleLowerCase("pl-PL");
    const parent = clubs2.find((candidate) => {
...
// resources/FixedSeasonDates.ts
var FIXED_SEASON_EVENTS = [
  // --- LIPIEC (Przygotowania i Start) ------------------------------------------------------------------------------------------------------------
  { day: 1, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "Przygotowania do nowego sezonu", priority: 100 },
  { day: 1, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "TRANSFER_WINDOW" /* TRANSFER_WINDOW */, label: "Otwarcie letniego okna transferowego", priority: 60 },
  // Losowanie Ligi Europy - 1 runda kwalifikacyjna
  { day: 2, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_R1Q_DRAW" /* EL_R1Q_DRAW */, label: "LOSOWANIE LE: RUNDA 1 PREELIMINACYJNA", priority: 88 },
  //LIGA MISTRZOW - losowanie 1 rundy preeliminacyjnej 
  { day: 3, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "CHAMPIONS_LEAGUE_DRAW" /* CHAMPIONS_LEAGUE_DRAW */, label: "LOSOWANIE LM: RUNDA 1 PREELIMINACYJNA", priority: 90 },
  { day: 4, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "BOARD" /* BOARD */, label: "ZEBRANIE ZARZ\u0104DU KLUBU", priority: 80 },
  //LIGA EUROPY - 1 runda kwalifikacyjna 1 mecz
  { day: 5, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_R1Q" /* EL_R1Q */, label: "LE: R1Q - 1. mecz", priority: 82 },
  // 6 LIPCA - losowanie 1 rundy kwalifikacyjnej Ligi Konferencji
  { day: 6, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_R1Q_DRAW" /* CONF_R1Q_DRAW */, label: "LOSOWANIE LK: RUNDA 1 PREELIMINACYJNA", priority: 87 },
  //SPARINGI
  { day: 8, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 9, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  //Liga Europy - 1 runda kwalifikacyjna rewanż
  { day: 10, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_R1Q_RETURN" /* EL_R1Q_RETURN */, label: "LE: R1Q - Rewan\u017C", priority: 82 },
  // LIGA MISTRZOW 1 mecz 1 rudna 
  { day: 11, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_R1Q" /* CL_R1Q */, label: "LM: R1Q - 1. mecz", priority: 85 },
  //SUPERPUCHAR POLSKI ///
  { day: 12, month: 6, type: "WEEKEND" /* WEEKEND */, comp: "SUPER_CUP" /* SUPER_CUP */, label: "Mecz o Superpuchar Polski", priority: 95 },
  //LOSOWANIE PUCHARU POLSKI 1/64
  { day: 13, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "LOSOWANIE PUCHARU POLSKI 1/64", priority: 40 },
  // 14 LIPCA - Pierwszy mecz 1 runda Ligi Konferencji
  { day: 14, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_R1Q" /* CONF_R1Q */, label: "LK: R1Q - 1. mecz", priority: 84 },
  //Liga mistrzow rewanż 1 rundy 
  { day: 15, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_R1Q_RETURN" /* CL_R1Q_RETURN */, label: "LM: R1Q - Rewan\u017C", priority: 85 },
  ///LOSOWANIE LIGI MISTRZOW 2 RUNDY PREELIMINACYJNEJ
  { day: 16, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_R2Q_DRAW" /* CL_R2Q_DRAW */, label: "LOSOWANIE LM: RUNDA 2 PREELIMINACYJNA", priority: 90 },
  // 17 LIPCA - Rewanż 1 rundy Ligi Konferencji
  { day: 17, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_R1Q_RETURN" /* CONF_R1Q_RETURN */, label: "LK: R1Q - Rewan\u017C", priority: 84 },
  { day: 18, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "TRANSFER_WINDOW" /* TRANSFER_WINDOW */, label: "Letnie przygotowania (Dzie\u0144 5)", priority: 20 },
  // LOSOWANIE LIGI EUROPY 2 RUNDY PREELIMINACYJNEJ
  { day: 19, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_R2Q_DRAW" /* EL_R2Q_DRAW */, label: "LOSOWANIE LE: RUNDA 2 PREELIMINACYJNA", priority: 88 },
  //MECZE 1/64 PUCHARU POLSKI ///
  { day: 20, month: 6, type: "WEEKEND" /* WEEKEND */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "Puchar Polski: 1/64", priority: 50 },
  //21 LIPCA - losowanie 2 rundy kwalifikacyjnej Ligi Konferencji
  { day: 21, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_R2Q_DRAW" /* CONF_R2Q_DRAW */, label: "LOSOWANIE LK: RUNDA 2 PREELIMINACYJNA", priority: 87 },
  { day: 24, month: 6, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 1", priority: 10 },
  /// -- LIGA MISTRZOW 2 runda preeliminacyjna 1 mecz
  { day: 27, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_R2Q" /* CL_R2Q */, label: "LM: R2Q - 1. mecz", priority: 85 },
  // 28 LIPCA - 1 mecz  2rundy kwalifikacyjnej Ligi Konferencji
  { day: 28, month: 6, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_R2Q" /* CONF_R2Q */, label: "LK: R2Q - 1. mecz", priority: 84 },
  { day: 30, month: 6, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 2", priority: 10 },
  // --- SIERPIEŃ --------------------------------------------------------------------------------------------------------------------------
  { day: 5, month: 7, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 3", priority: 10 },
  // LE 2 runda kwalifikacyjna 1 mecz
  { day: 8, month: 7, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_R2Q" /* EL_R2Q */, label: "LE: R2Q - 1. mecz", priority: 82 },
  { day: 11, month: 7, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 4", priority: 10 },
  ///LOSOWANIE PP 1/32
  { day: 12, month: 7, type: "MIDWEEK" /* MIDWEEK */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "LOSOWANIE PUCHARU POLSKI 1/32", priority: 40 },
  //-- LIGA MISTRZOW 2 RUNDA REWANZ 
  { day: 14, month: 7, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_R2Q_RETURN" /* CL_R2Q_RETURN */, label: "LM: R2Q - Rewan\u017C", priority: 85 },
  // -- LIGA EUROPY 2 RUNDA REWANZ
  { day: 15, month: 7, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_R2Q_RETURN" /* EL_R2Q_RETURN */, label: "LE: R2Q - Rewan\u017C", priority: 82 },
  //16 SIERPNIA - Rewanż 2 rundy kwalifikacyjnej Ligi Konferencji
  { day: 16, month: 7, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_R2Q_RETURN" /* CONF_R2Q_RETURN */, label: "LK: R2Q - Rewan\u017C", priority: 84 },
  // LOSOWANIE FAZ GRUPOWYCH PUCHAROW EUROPEJSKICH LIGA EUROPY
  { day: 17, month: 7, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_GROUP_DRAW" /* EL_GROUP_DRAW */, label: "LOSOWANIE LE: FAZA GRUPOWA", priority: 88 },
  // LOSOWANIE FAZ GRUPOWYCH PUCHAROW EUROPEJSKICH LIGA MISTRZÓW
  { day: 18, month: 7, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_GROUP_DRAW" /* CL_GROUP_DRAW */, label: "LOSOWANIE LM: FAZA GRUPOWA", priority: 90 },
  { day: 19, month: 7, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_GROUP_DRAW" /* CONF_GROUP_DRAW */, label: "LOSOWANIE LK: FAZA GRUPOWA", priority: 87 },
  { day: 20, month: 7, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 5", priority: 10 },
  //23 SIERPNIA - SuperPuchar Europy
  { day: 23, month: 7, type: "MIDWEEK" /* MIDWEEK */, comp: "UEFA_SUPER_CUP" /* UEFA_SUPER_CUP */, label: "SUPERPUCHAR EUROPY", priority: 95 },
  { day: 25, month: 7, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 6", priority: 10 },
  { day: 30, month: 7, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 7", priority: 10 },
  // --- WRZESIEŃ -----------------------------------------------------------------------------------------------------------------------
  // --- REPREZENTAcjA ---
  { day: 4, month: 8, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  { day: 7, month: 8, type: "WEEKEND" /* WEEKEND */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  // 8 WRZESNIA - zamkniecie okna transferowego letniego
  { day: 10, month: 8, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 8", priority: 10 },
  { day: 14, month: 8, type: "WEEKEND" /* WEEKEND */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "Puchar Polski: 1/32", priority: 50 },
  { day: 15, month: 8, type: "MIDWEEK" /* MIDWEEK */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "LOSOWANIE PUCHARU POLSKI 1/16", priority: 40 },
  // --- PUCHARY EUROPEJSKIE --- 18 września LIGA MISTRZÓW---- faza grupowa 1 kolejka
  { day: 18, month: 8, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_GROUP_STAGE" /* CL_GROUP_STAGE */, label: "LM: Faza Grupowa - Kolejka 1", priority: 85 },
  // --- PUCHARY EUROPEJSKIE --- 19 września LIGA EUROPY ---- faza grupowa 1 kolejka
  { day: 19, month: 8, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_GROUP_STAGE" /* EL_GROUP_STAGE */, label: "LE: Faza Grupowa - Kolejka 1", priority: 82 },
  { day: 20, month: 8, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_GROUP_STAGE" /* CONF_GROUP_STAGE */, label: "LK: Faza Grupowa - Kolejka 1", priority: 84 },
  { day: 22, month: 8, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 9", priority: 10 },
  { day: 26, month: 8, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 10", priority: 10 },
  { day: 30, month: 8, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 11", priority: 10 },
  // --- WRZESIEŃ (Zamknięcie letniego okna) --------------------------------------------------------------------------------------------
  { day: 8, month: 8, type: "WEEKEND" /* WEEKEND */, comp: "TRANSFER_WINDOW" /* TRANSFER_WINDOW */, label: "Zamkni\u0119cie letniego okna transferowego", priority: 60 },
  { day: 3, month: 9, type: "WEEKEND" /* WEEKEND */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "Puchar Polski: 1/16", priority: 50 },
  { day: 5, month: 9, type: "MIDWEEK" /* MIDWEEK */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "LOSOWANIE PUCHARU POLSKI 1/8", priority: 40 },
  { day: 6, month: 9, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 12", priority: 10 },
  // --- REPREZENTAcjA ---
  { day: 8, month: 9, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  { day: 11, month: 9, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  { day: 14, month: 9, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 13", priority: 10 },
  // --- PUCHARY EUROPEJSKIE --- 17 pazdziernika LIGA MISTRZÓW----faza grupowa 2 kolejka 
  { day: 17, month: 9, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_GROUP_STAGE" /* CL_GROUP_STAGE */, label: "LM: Faza Grupowa - Kolejka 2", priority: 85 },
  // --- PUCHARY EUROPEJSKIE --- 18 pazdziernika LIGA EUROPY ----faza grupowa 2 kolejka
  { day: 18, month: 9, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_GROUP_STAGE" /* EL_GROUP_STAGE */, label: "LE: Faza Grupowa - Kolejka 2", priority: 82 },
  { day: 19, month: 9, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_GROUP_STAGE" /* CONF_GROUP_STAGE */, label: "LK: Faza Grupowa - Kolejka 2", priority: 84 },
  { day: 21, month: 9, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 14", priority: 10 },
  // --- PUCHARY EUROPEJSKIE --- 25 pazdziernika LIGA MISTRZÓW----faza grupowa 3 kolejka
  { day: 25, month: 9, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_GROUP_STAGE" /* CL_GROUP_STAGE */, label: "LM: Faza Grupowa - Kolejka 3", priority: 85 },
  // --- PUCHARY EUROPEJSKIE --- 26 pazdziernika LIGA EUROPY ----faza grupowa 3 kolejka
  { day: 26, month: 9, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_GROUP_STAGE" /* EL_GROUP_STAGE */, label: "LE: Faza Grupowa - Kolejka 3", priority: 82 },
  { day: 27, month: 9, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_GROUP_STAGE" /* CONF_GROUP_STAGE */, label: "LK: Faza Grupowa - Kolejka 3", priority: 84 },
  { day: 30, month: 9, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 15", priority: 10 },
  // --- LISTOPAD ------------------------------------------------------------------------------------------------------------
  { day: 3, month: 10, type: "WEEKEND" /* WEEKEND */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "Puchar Polski: 1/8", priority: 50 },
  // --- REPREZENTAcjA ---
  { day: 14, month: 10, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  { day: 17, month: 10, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  { day: 19, month: 10, type: "MIDWEEK" /* MIDWEEK */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "LOSOWANIE PUCHARU POLSKI 1/4", priority: 40 },
  { day: 21, month: 10, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 16", priority: 10 },
  // --- PUCHARY EUROPEJSKIE --- 25 listopad LIGA MISTRZÓW----faza grupowa 4 kolejka
  { day: 25, month: 10, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_GROUP_STAGE" /* CL_GROUP_STAGE */, label: "LM: Faza Grupowa - Kolejka 4", priority: 85 },
  // --- PUCHARY EUROPEJSKIE --- 26 listopad LIGA EUROPY ----faza grupowa 4 kolejka
  { day: 26, month: 10, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_GROUP_STAGE" /* EL_GROUP_STAGE */, label: "LE: Faza Grupowa - Kolejka 4", priority: 82 },
  { day: 27, month: 10, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_GROUP_STAGE" /* CONF_GROUP_STAGE */, label: "LK: Faza Grupowa - Kolejka 4", priority: 84 },
  //29 LISTOPADA - MIEDZYNARODOWE ROZGRYWKI LUB INNE WYDARZENIA ZWIĄZANE Z REPREZENTACJĄ
  { day: 29, month: 10, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  // --- GRUDZIEŃ ----------------------------------------------------------------------------------------------------------------------------
  { day: 1, month: 11, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 17", priority: 10 },
  // --- PUCHARY EUROPEJSKIE --- 4 grudnia LIGA MISTRZÓW----faza grupowa 5 kolejka
  { day: 4, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_GROUP_STAGE" /* CL_GROUP_STAGE */, label: "LM: Faza Grupowa - Kolejka 5", priority: 85 },
  // --- PUCHARY EUROPEJSKIE --- 5 grudnia LIGA EUROPY ----faza grupowa 5 kolejka
  { day: 5, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_GROUP_STAGE" /* EL_GROUP_STAGE */, label: "LE: Faza Grupowa - Kolejka 5", priority: 82 },
  { day: 6, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_GROUP_STAGE" /* CONF_GROUP_STAGE */, label: "LK: Faza Grupowa - Kolejka 5", priority: 84 },
  { day: 10, month: 11, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 18", priority: 10 },
  { day: 11, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "WINTER_CAMP_INVITE" /* WINTER_CAMP_INVITE */, label: "Propozycja obozu zimowego od Zarz\u0105du", priority: 75 },
  // --- PUCHARY EUROPEJSKIE --- 14 grudnia LIGA MISTRZÓW----faza grupowa 6 kolejka
  { day: 14, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_GROUP_STAGE" /* CL_GROUP_STAGE */, label: "LM: Faza Grupowa - Kolejka 6", priority: 85 },
  // --- PUCHARY EUROPEJSKIE --- 15 grudnia LIGA EUROPY ----faza grupowa 6 kolejka
  { day: 15, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_GROUP_STAGE" /* EL_GROUP_STAGE */, label: "LE: Faza Grupowa - Kolejka 6", priority: 82 },
  { day: 16, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_GROUP_STAGE" /* CONF_GROUP_STAGE */, label: "LK: Faza Grupowa - Kolejka 6", priority: 84 },
  { day: 17, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "BOARD" /* BOARD */, label: "Zimowa sesja Zarz\u0105du", priority: 80 },
  { day: 18, month: 11, type: "WEEKEND" /* WEEKEND */, comp: "BREAK" /* BREAK */, label: "Pocz\u0105tek Przerwy Zimowej", priority: 100 },
  { day: 19, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_R16_DRAW" /* CL_R16_DRAW */, label: "LOSOWANIE LM: 1/8 FINA\u0141U", priority: 90 },
  { day: 20, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_R16_DRAW" /* EL_R16_DRAW */, label: "LOSOWANIE LE: 1/8 FINA\u0141U", priority: 88 },
  // 21 grudzen LIGA KONFERENCJI ----losowanie 1/8 finału
  { day: 21, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_R16_DRAW" /* CONF_R16_DRAW */, label: "LOSOWANIE LK: 1/8 FINA\u0141U", priority: 87 },
  { day: 22, month: 11, type: "MIDWEEK" /* MIDWEEK */, comp: "WINTER_CAMP_PROGRAM" /* WINTER_CAMP_PROGRAM */, label: "Rozmowa z asystentem: plan obozu zimowego", priority: 75 },
  // --- STYCZEŃ ---------------------------------------------------------------------------------------------------------------------------------------
  //SPARINGI
  { day: 2, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 3, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 4, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 5, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 6, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 7, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 8, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 9, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 10, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 11, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 12, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "TRANSFER_WINDOW" /* TRANSFER_WINDOW */, label: "Otwarcie zimowego okna transferowego", priority: 60 },
  //SPARINGI
  { day: 13, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 14, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 15, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 15, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "WINTER_CAMP_END" /* WINTER_CAMP_END */, label: "Zako\u0144czenie obozu zimowego", priority: 76 },
  { day: 16, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 17, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 18, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  // --- PUCHARY EUROPEJSKIE --- 19 styczen LIGA MISTRZÓW----1/8 finału
  { day: 19, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_R16" /* CL_R16 */, label: "LM: 1/8 Fina\u0142u - 1. mecz", priority: 85 },
  { day: 20, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_R16" /* EL_R16 */, label: "LE: 1/8 Fina\u0142u - 1. mecz", priority: 82 },
  //  21 styczen LIGA KONFERENCJI ---- 1 mecz - 1/8 finału 
  { day: 21, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_R16" /* CONF_R16 */, label: "LK: 1/8 Fina\u0142u - 1. mecz", priority: 84 },
  //SPARINGI
  { day: 23, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  { day: 24, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "FRIENDLY" /* FRIENDLY */, label: "SPARING", priority: 20 },
  // --- PUCHARY EUROPEJSKIE --- 25 styczen LIGA MISTRZÓW----1.8 finału REWANZ
  { day: 25, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_R16_RETURN" /* CL_R16_RETURN */, label: "LM: 1/8 Fina\u0142u - Rewan\u017C", priority: 85 },
  { day: 26, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_R16_RETURN" /* EL_R16_RETURN */, label: "LE: 1/8 Fina\u0142u - Rewan\u017C", priority: 82 },
  // ---- 27 styczeń LOSOWANIE 1/4 FINAŁU LIGI MISTRZÓW 
  { day: 27, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_QF_DRAW" /* CL_QF_DRAW */, label: "LOSOWANIE LM: 1/4 FINA\u0141U", priority: 90 },
  // ---- 28 styczeń LOSOWANIE 1/4 FINAŁU LIGI EUROPY
  { day: 28, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_QF_DRAW" /* EL_QF_DRAW */, label: "LOSOWANIE LE: 1/4 FINA\u0141U", priority: 88 },
  // 29 styczen LIGA KONFERENCJI ----1.8 finału/rewanż 
  { day: 29, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_R16_RETURN" /* CONF_R16_RETURN */, label: "LK: 1/8 Fina\u0142u - Rewan\u017C", priority: 84 },
  { day: 30, month: 0, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_QF_DRAW" /* CONF_QF_DRAW */, label: "LOSOWANIE LK: 1/4 FINA\u0141U", priority: 86 },
  { day: 31, month: 0, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 19", priority: 10 },
  // --- LUTY ------------------------------------------------------------------------------------------------------------------------------------------
  { day: 7, month: 1, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 20", priority: 10 },
  { day: 12, month: 1, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 21", priority: 10 },
  //13 luty - zamkniecie zimowego okna transferowego
  // --- PUCHARY EUROPEJSKIE --- 16 LUTY LIGA MISTRZÓW----cwierfinaly 1/4
  { day: 16, month: 1, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_QF" /* CL_QF */, label: "LM: 1/4 Fina\u0142u - 1. mecz", priority: 85 },
  // --- PUCHARY EUROPEJSKIE --- 17 LUTY LIGA EUROPY ----cwierfinaly 1 mecz 1/4
  { day: 17, month: 1, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_QF" /* EL_QF */, label: "LE: 1/4 Fina\u0142u - 1. mecz", priority: 82 },
  { day: 18, month: 1, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_QF" /* CONF_QF */, label: "LK: 1/4 Fina\u0142u - 1. mecz", priority: 83 },
  { day: 22, month: 1, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 22", priority: 10 },
  { day: 13, month: 1, type: "WEEKEND" /* WEEKEND */, comp: "TRANSFER_WINDOW" /* TRANSFER_WINDOW */, label: "Zamkni\u0119cie zimowego okna transferowego", priority: 60 },
  { day: 28, month: 1, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 23", priority: 10 },
  // --- MARZEC ------------------------------------------------------------------------------------------------------------------------------------------
  { day: 1, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "BOARD" /* BOARD */, label: "ZEBRANIE ZARZ\u0104DU KLUBU", priority: 80 },
  // --- PUCHARY EUROPEJSKIE --- 2 MARZEC LIGA MISTRZÓW----cwierfinaly/rewanże 1/4
  { day: 2, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_QF_RETURN" /* CL_QF_RETURN */, label: "LM: 1/4 Fina\u0142u - Rewan\u017C", priority: 85 },
  // --- PUCHARY EUROPEJSKIE --- 3 MARZEC LIGA EUROPY ----cwierfinaly/rewanż 1/4
  { day: 3, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_QF_RETURN" /* EL_QF_RETURN */, label: "LE: 1/4 Fina\u0142u - Rewan\u017C", priority: 82 },
  { day: 4, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_QF_RETURN" /* CONF_QF_RETURN */, label: "LK: 1/4 Fina\u0142u - Rewan\u017C", priority: 83 },
  //=== 6 MARZEC LOSOWANIE PÓŁFINAŁÓW LIGI MISTRZÓW
  { day: 6, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_SF_DRAW" /* CL_SF_DRAW */, label: "LOSOWANIE LM: 1/2 FINA\u0141U", priority: 90 },
  { day: 7, month: 2, type: "WEEKEND" /* WEEKEND */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "Puchar Polski: 1/4", priority: 50 },
  // 8 marzec - losowanie połfinałów Ligi europy 
  { day: 8, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_SF_DRAW" /* EL_SF_DRAW */, label: "LOSOWANIE LE: 1/2 FINA\u0141U", priority: 88 },
  { day: 9, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "LOSOWANIE PUCHARU POLSKI 1/2", priority: 40 },
  { day: 10, month: 2, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 24", priority: 10 },
  { day: 11, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_SF_DRAW" /* CONF_SF_DRAW */, label: "LOSOWANIE LK: 1/2 FINA\u0141U", priority: 88 },
  // --- REPREZENTAcjA ---
  { day: 17, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  { day: 20, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  { day: 24, month: 2, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 25", priority: 10 },
  // --- PUCHARY EUROPEJSKIE --- 26 Marzec LIGA MISTRZÓW----polfinaly 1/2
  { day: 26, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_SF" /* CL_SF */, label: "LM: 1/2 Fina\u0142u - 1. mecz", priority: 85 },
  // 27 MARZEC LIGA EUROPY ----polfinaly - 1 mecz 1/2
  { day: 27, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_SF" /* EL_SF */, label: "LE: 1/2 Fina\u0142u - 1. mecz", priority: 82 },
  { day: 28, month: 2, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_SF" /* CONF_SF */, label: "LK: 1/2 Fina\u0142u - 1. mecz", priority: 83 },
  { day: 30, month: 2, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 26", priority: 10 },
  // --- KWIECIEŃ --------------------------------------------------------------------------------------------------------------------------
  { day: 4, month: 3, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 27", priority: 10 },
  { day: 8, month: 3, type: "WEEKEND" /* WEEKEND */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "Puchar Polski: 1/2", priority: 55 },
  { day: 9, month: 3, type: "MIDWEEK" /* MIDWEEK */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "Og\u0142oszenie finalist\xF3w Pucharu Polski", priority: 70 },
  { day: 12, month: 3, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 28", priority: 10 },
  // --- PUCHARY EUROPEJSKIE --- 15 Kwiecen LIGA MISTRZÓW----polfinaly/rewanże 1/2
  { day: 15, month: 3, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_SF_RETURN" /* CL_SF_RETURN */, label: "LM: 1/2 Fina\u0142u - Rewan\u017C", priority: 85 },
  //  16 Kwiecien LIGA EUROPY ----polfinaly rewzn 
  { day: 16, month: 3, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_SF_RETURN" /* EL_SF_RETURN */, label: "LE: 1/2 Fina\u0142u - Rewan\u017C", priority: 82 },
  { day: 17, month: 3, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_SF_RETURN" /* CONF_SF_RETURN */, label: "LK: 1/2 Fina\u0142u - Rewan\u017C", priority: 83 },
  // --- 18 kwietnia ogloszenienie finalistow ligii mistrzów 
  { day: 18, month: 3, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_FINAL_DRAW" /* CL_FINAL_DRAW */, label: "OG\u0141OSZENIE FINALIST\xD3W LM", priority: 90 },
  // 19 kwietnia ogłoszenie finalistów ligi europy
  { day: 19, month: 3, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_FINAL_DRAW" /* EL_FINAL_DRAW */, label: "OG\u0141OSZENIE FINALIST\xD3W LE", priority: 88 },
  { day: 20, month: 3, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_FINAL_DRAW" /* CONF_FINAL_DRAW */, label: "OG\u0141OSZENIE FINALIST\xD3W LK", priority: 88 },
  { day: 21, month: 3, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 29", priority: 10 },
  { day: 27, month: 3, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 30", priority: 10 },
  // --- MAJ ------------------------------------------------------------------------------------------------------------------------------------------
  { day: 2, month: 4, type: "WEEKEND" /* WEEKEND */, comp: "POLISH_CUP" /* POLISH_CUP */, label: "Fina\u0142 Pucharu Polski", priority: 95 },
  { day: 6, month: 4, type: "MIDWEEK" /* MIDWEEK */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 31", priority: 15 },
  { day: 11, month: 4, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 32", priority: 10 },
  { day: 15, month: 4, type: "MIDWEEK" /* MIDWEEK */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 33", priority: 15 },
  { day: 19, month: 4, type: "MIDWEEK" /* MIDWEEK */, comp: "SUMMER_CAMP_INVITE" /* SUMMER_CAMP_INVITE */, label: "Propozycja obozu letniego od Zarz\u0105du", priority: 75 },
  // --- FINAL PUCHARY EUROPEJSKIE --- 20 Maj LIGA EUROPY ----
  { day: 20, month: 4, type: "MIDWEEK" /* MIDWEEK */, comp: "EL_FINAL" /* EL_FINAL */, label: "LE: Fina\u0142", priority: 100 },
  { day: 23, month: 4, type: "WEEKEND" /* WEEKEND */, comp: "LEAGUE" /* LEAGUE */, label: "Liga: Kolejka 34", priority: 90 },
  { day: 24, month: 4, type: "MIDWEEK" /* MIDWEEK */, comp: "PLAYOFF_DRAW_CEREMONY" /* PLAYOFF_DRAW_CEREMONY */, label: "Og\u0142oszenie par bara\u017Cowych", priority: 95 },
  ...
  { day: 26, month: 4, type: "MIDWEEK" /* MIDWEEK */, comp: "RELEGATION_PLAYOFF_1" /* RELEGATION_PLAYOFF_1 */, label: "Bara\u017Ce o utrzymanie: 1. mecze", priority: 92 },
  { day: 27, month: 4, type: "MIDWEEK" /* MIDWEEK */, comp: "CONF_FINAL" /* CONF_FINAL */, label: "LK: Fina\u0142", priority: 100 },
  ...
  { day: 29, month: 4, type: "MIDWEEK" /* MIDWEEK */, comp: "RELEGATION_PLAYOFF_2" /* RELEGATION_PLAYOFF_2 */, label: "Bara\u017Ce o utrzymanie: Rewan\u017Ce", priority: 92 },
  // --- FINAL PUCHARY EUROPEJSKIE --- 30 Maj LIGA MISTRZÓW----
  { day: 30, month: 4, type: "MIDWEEK" /* MIDWEEK */, comp: "CL_FINAL" /* CL_FINAL */, label: "LM: Fina\u0142", priority: 100 },
  //31 MAJ - PIERWSZE MECZE PLAY OFF AWANSOW DO EKSTRAKLASY I I LIGI 
  { day: 31, month: 4, type: "MIDWEEK" /* MIDWEEK */, comp: "PROMOTION_PLAYOFF_31_MAY" /* PROMOTION_PLAYOFF_31_MAY */, label: "Bara\u017Ce awansowe: 1. mecze", priority: 93 },
  // --- CZERWIEC --------------------------------------------------------------------------------------------------------------------------------------------------
  { day: 1, month: 5, type: "MIDWEEK" /* MIDWEEK */, comp: "BOARD" /* BOARD */, label: "ZEBRANIE ZARZ\u0104DU KLUBU/URLOPY", priority: 80 },
  // --- REPREZENTAcjA ---
  // 4 CZERWCA - FINAŁY PLAY OFF AWANSOW DO EKSTRAKLASY I I LIGI
  { day: 4, month: 5, type: "MIDWEEK" /* MIDWEEK */, comp: "PROMOTION_PLAYOFF_4_JUNE" /* PROMOTION_PLAYOFF_4_JUNE */, label: "Bara\u017Ce awansowe: Fina\u0142y", priority: 93 },
  { day: 7, month: 5, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  { day: 11, month: 5, type: "MIDWEEK" /* MIDWEEK */, comp: "BREAK" /* BREAK */, label: "REPREZENTACJA", priority: 30 },
  { day: 5, month: 5, type: "MIDWEEK" /* MIDWEEK */, comp: "SUMMER_CAMP_PROGRAM" /* SUMMER_CAMP_PROGRAM */, label: "Rozmowa z asystentem: plan obozu letniego", priority: 75 },
  { day: 28, month: 5, type: "MIDWEEK" /* MIDWEEK */, comp: "SUMMER_CAMP_END" /* SUMMER_CAMP_END */, label: "Zako\u0144czenie obozu letniego", priority: 76 },
  { day: 29, month: 5, type: "MIDWEEK" /* MIDWEEK */, comp: "TRANSFER_WINDOW" /* TRANSFER_WINDOW */, label: "PODSUMOWANIE SEZONU", priority: 60 },
  { day: 30, month: 5, type: "MIDWEEK" /* MIDWEEK */, comp: "OFF_SEASON" /* OFF_SEASON */, label: "ZAKO\u0143CZENIE SEZONU", priority: 100 }
];

// services/SeasonTemplateGenerator.ts
var SeasonTemplateGenerator = {
  generate: (seasonStartYear) => {
    const careerStartDate = new Date(seasonStartYear, 6, 1);
    const slots = FIXED_SEASON_EVENTS.map((event, index) => {
      ...
      return {
        ...
        // W nowym systemie operujemy na konkretnych dniach
        slotType: event.type,
...
var getLeagueId = (clubId, clubs2, projectedLeagueByClubId) => projectedLeagueByClubId?.get(clubId) ?? clubs2.find((club) => club.id === clubId)?.leagueId;
var ReserveTeamLeagueService = {
  createLeagueProjection(clubs2, changes = []) {
    const projection = new Map(clubs2.map((club) => [club.id, club.leagueId]));
   ...
   */
  getPlayableReserveClubId(parentClubId, clubs2) {
    const reserveClubId = this.getReserveClubId(parentClubId);
    if (!reserveClubId) return null;
    const reserveClub = clubs2.find((club) => club.id === reserveClubId);
  ...
  canEnterLeague(clubId, targetLeagueId, clubs2, projectedLeagueByClubId) {
    const parentClubId = this.getParentClubId(clubId);
    if (!parentClubId) return true;
    if (targetLeagueId === "L_PL_1") return false;
    const parentLeagueId = getLeagueId(parentClubId, clubs2, projectedLeagueByClubId);
    return PolishThirdLeagueService.getPolishTier(parentLeagueId) !== PolishThirdLeagueService.getPolishTier(targetLeagueId);
  },
  selectPromotionPlaces(standings, targetLeagueId, clubs2, directPlaceCount = 2, playoffPlaceCount = 4, projectedLeagueByClubId) {
    const eligible = this.getEligibleCandidates(standings, targetLeagueId, clubs2, projectedLeagueByClubId);
    return {
      direct: eligible.slice(0, directPlaceCount),
      playoffs: eligible.slice(directPlaceCount, directPlaceCount + playoffPlaceCount)
    };
  },
  getEligibleCandidates(standings, targetLeagueId, clubs2, projectedLeagueByClubId) {
    return standings.map((club, index) => ({ club, tablePosition: index + 1 })).filter((candidate) => this.canEnterLeague(candidate.club.id, targetLeagueId, clubs2, projectedLeagueByClubId));
  },
  findSameLeagueConflicts(clubs2, projectedLeagueByClubId) {
    return Object.entries(RESERVE_PARENT_CLUB_BY_ID).flatMap(([reserveClubId, parentClubId]) => {
      const reserveLeagueId = getLeagueId(reserveClubId, clubs2, projectedLeagueByClubId);
      const parentLeagueId = getLeagueId(parentClubId, clubs2, projectedLeagueByClubId);
  ...
  resolvePlayoffWinner(result, targetLeagueId, clubs2, excludedClubIds = /* @__PURE__ */ new Set(), projectedLeagueByClubId) {
    if (!result) return null;
    const loserId = result.winnerId === result.homeId ? result.awayId : result.homeId;
    return [result.winnerId, loserId].find(
      (clubId) => !excludedClubIds.has(clubId) && this.canEnterLeague(clubId, targetLeagueId, clubs2, projectedLeagueByClubId)
    ) ?? null;
  }
};

// services/PolishThirdLeagueSeasonTransitionService.ts
var hash2 = (value) => {
  let result = 2166136261;
  for (let index = 0; index < value.length; index++) {
    result ^= value.charCodeAt(index);
...
var rngFor2 = (seed, key) => {
  let state = (seed ^ hash2(key)) >>> 0;
  return () => {
    state = state * 1664525 + 1013904223 >>> 0;
    return state / 4294967296;
  };
};
var table = (clubs2, leagueId) => clubs2.filter((club) => club.leagueId === leagueId && club.isDefaultActive).sort(
  (a, b) => b.stats.points - a.stats.points || b.stats.goalDifference - a.stats.goalDifference || b.stats.goalsFor - a.stats.goalsFor || a.id.localeCompare(b.id)
);
var squadStrength = (club, players) => {
  const bestEighteen = [...players[club.id] ?? []].filter((player) => player.health.status !== "INJURED").sort((a, b) => b.overallRating - a.overallRating).slice(0, 18);
  const squadAverage = bestEighteen.length ? bestEighteen.reduce((sum, player) => sum + player.overallRating, 0) / bestEighteen.length : 35 + club.reputation * 2;
  return squadAverage + club.reputation * 1.5 + Math.min(4, club.stats.points / 20);
};
var simulateScore = (home, away, players, seed, key) => {
  const rng = rngFor2(seed, key);
  ...
  const awayExpected = Math.max(0.2, Math.min(3, 1.15 - difference / 18));
  const goals = (expected) => {
    const limit = Math.exp(-expected);
    let product = 1;
    let count = 0;
    do {
...
var resolveSingleMatch = (home, away, players, seed, key, stage = "RUNNERS_UP") => {
  ...
  if (!winnerClubId) {
    const rng = rngFor2(seed, `${key}|PENALTIES`);
...
var emptyGroupRecord = () => ({
  L_PL_4_G1: [],
  L_PL_4_G2: [],
  L_PL_4_G3: [],
  L_PL_4_G4: []
});
var shuffledForFourthLeaguePlayoff = (clubs2, sessionSeed, seasonNumber, groupId) => [...clubs2].sort(
  (left, right) => hash2(`${sessionSeed}|${seasonNumber}|${groupId}|IV_RUNNER|${left.id}`) - hash2(`${sessionSeed}|${seasonNumber}|${groupId}|IV_RUNNER|${right.id}`)
);
var PolishThirdLeagueSeasonTransitionService = {
  resolve(clubs2, players, sessionSeed, seasonNumber, projectedLeagueByClubId, overrides = {}, fourthLeagueState) {
    const clubById = new Map(clubs2.map((club) => [club.id, club]));
    const groupTables = Object.fromEntries(
      THIRD_LEAGUE_GROUP_IDS.map((groupId) => [groupId, table(clubs2, groupId)])
    );
    ...
    const promotedChampionIds = champions.filter((club) => ReserveTeamLeagueService.canEnterLeague(club.id, "L_PL_3", clubs2, projectedLeagueByClubId)).map((club) => club.id);
    const promotedToSecondLeagueIds = [...promotedChampionIds];
    const shuffledRunnersUp = [...runnersUp].sort(
      (a, b) => hash2(`${sessionSeed}|${seasonNumber}|${a.id}`) - hash2(`${sessionSeed}|${seasonNumber}|${b.id}`)
    );
    const firstStage = [0, 1].map((pairIndex) => {
      const left = shuffledRunnersUp[pairIndex * 2];
      const right = shuffledRunnersUp[pairIndex * 2 + 1];
      const hostRng = rngFor2(sessionSeed, `III_PLAYOFF_HOST|${seasonNumber}|${pairIndex}`);
    ...
    const secondLeagueTable = table(clubs2, "L_PL_3");
    const incumbents = secondLeagueTable.slice(12, 14);
    const forcedReserveRelegationIds = new Set(
      projectedLeagueByClubId ? ReserveTeamLeagueService.findSameLeagueConflicts(clubs2, projectedLeagueByClubId).filter(
        (conflict) => PolishThirdLeagueService.getPolishTier(conflict.leagueId) === 3 && clubById.get(conflict.reserveClubId)?.leagueId === "L_PL_3"
      ).map((conflict) => conflict.reserveClubId) : []
    );
    const directRelegated = [...new Map(
      [...secondLeagueTable.slice(14, 18), ...secondLeagueTable.filter((club) => forcedReserveRelegationIds.has(club.id))].map((club) => [club.id, club])
    ).values()];
    ...
    const playoffMatches = [...firstStage];
    const playoffPromoted = /* @__PURE__ */ new Set();
    firstStage.forEach((firstMatch, pairIndex) => {
      ...
      const validInteractiveWinnerId = interactiveWinnerId === challenger.id || interactiveWinnerId === incumbent.id ? interactiveWinnerId : void 0;
      let winnerClubId = forcedReserveRelegationIds.has(incumbent.id) ? challenger.id : validInteractiveWinnerId ?? (challengerAggregate > incumbentAggregate ? challenger.id : incumbentAggregate > challengerAggregate ? incumbent.id : "");
      if (!winnerClubId) {
        const rng = rngFor2(sessionSeed, `III_PLAYOFF_FINAL_PENALTIES_${seasonNumber}_${pairIndex}`);
      ...
      if (winnerClubId === challenger.id && ReserveTeamLeagueService.canEnterLeague(challenger.id, "L_PL_3", clubs2, projectedLeagueByClubId)) {
    ...
    const supplementaryCandidates = THIRD_LEAGUE_GROUP_IDS.flatMap((groupId) => groupTables[groupId].map((club, index) => ({ club, tablePosition: index + 1 }))).filter(
      (candidate) => !promotedSet.has(candidate.club.id) && ReserveTeamLeagueService.canEnterLeague(candidate.club.id, "L_PL_3", clubs2, projectedLeagueByClubId)
    ).sort(
      (a, b) => a.tablePosition - b.tablePosition || b.club.stats.points - a.club.stats.points || b.club.stats.goalDifference - a.club.stats.goalDifference || b.club.stats.goalsFor - a.club.stats.goalsFor || a.club.id.localeCompare(b.club.id)
    );
    ...
    if (fourthLeagueState) {
      THIRD_LEAGUE_GROUP_IDS.forEach((groupId) => {
        const regionalLeagueIds = FOURTH_LEAGUE_IDS.filter(
          (leagueId) => PolishThirdLeagueService.getGroupForVoivodeship(
            PolishFourthLeagueService.getVoivodeshipForLeague(leagueId)
          ) === groupId
        );
        const regionalQualifiers = regionalLeagueIds.map((leagueId) => {
          const ranking = PolishFourthLeagueService.getTable(clubs2, leagueId).filter(
            (club) => PolishFourthLeagueService.canReserveEnterThirdLeague(club, clubs2) && ReserveTeamLeagueService.canEnterLeague(club.id, groupId, clubs2, projectedLeagueByClubId)
          );
          if (ranking.length < 2) {
        ...
        const champions2 = regionalQualifiers.map((candidate) => candidate.champion);
        const runnersUp2 = shuffledForFourthLeaguePlayoff(
        ...
        const semiFinals = [0, 1].map((pairIndex) => {
          const left = runnersUp2[pairIndex * 2];
          const right = runnersUp2[pairIndex * 2 + 1];
          const hostRng = rngFor2(sessionSeed, `IV_PLAYOFF_HOST|${seasonNumber}|${groupId}|${pairIndex}`);
          const [home, away] = hostRng() < 0.5 ? [left, right] : [right, left];
        ...
        const finalists = semiFinals.map((match) => clubById.get(match.winnerClubId ?? "")).filter((club) => !!club);
        if (finalists.length !== 2) throw new Error(`${groupId} IV-liga playoff did not produce two finalists.`);
        const finalHostRng = rngFor2(sessionSeed, `IV_PLAYOFF_FINAL_HOST|${seasonNumber}|${groupId}`);
        ...
        promotedFromFeederByGroup[groupId] = [
          ...champions2.map((club) => club.id),
          finalMatch.winnerClubId
        ];
      });
    } else {
      THIRD_LEAGUE_GROUP_IDS.forEach((groupId) => {
        const candidates = clubs2.filter(
          (club) => (club.leagueId === "L_PL_5" || PolishFourthLeagueService.isFourthLeagueId(club.leagueId)) && club.polishVoivodeship && PolishThirdLeagueService.getGroupForVoivodeship(club.polishVoivodeship) === groupId
        ).sort((a, b) => {
          const score = (club) => club.reputation * 100 + squadStrength(club, players) + hash2(`${sessionSeed}|${seasonNumber}|${groupId}|${club.id}`) % 100 / 100;
    ...
    const nextLeagueByClubId = /* @__PURE__ */ new Map();
    promotedToSecondLeagueIds.forEach((clubId) => nextLeagueByClubId.set(clubId, "L_PL_3"));
    Object.entries(relegatedFromSecondLeagueByGroup).forEach(
      ([groupId, clubIds]) => clubIds.forEach((clubId) => nextLeagueByClubId.set(clubId, groupId))
    );
    relegatedToFeederIds.forEach((clubId) => {
      const club = clubById.get(clubId);
      const targetLeagueId = club?.polishVoivodeship && fourthLeagueState ? PolishFourthLeagueService.getLeagueForVoivodeship(club.polishVoivodeship) : "L_PL_5";
      nextLeagueByClubId.set(clubId, targetLeagueId);
    });
    Object.entries(promotedFromFeederByGroup).forEach(
      ([groupId, clubIds]) => clubIds.forEach((clubId) => nextLeagueByClubId.set(clubId, groupId))
    );
    return {
...
// tests/FourthLeague2026Tests.ts
var template = SeasonTemplateGenerator.generate(2026);
var clubs = PolishLeagueSeasonService.buildClubsForCareerStart(STATIC_CLUBS, 2026);
FOURTH_LEAGUE_FEEDER_IDS.forEach((poolId) => {
  const poolClubs = clubs.filter((club) => club.leagueId === poolId);
  import_strict.default.equal(poolClubs.length, 18, `${poolId} must start with exactly 18 promotion candidates`);
  import_strict.default.ok(poolClubs.every((club) => club.tier === 6 && club.isDefaultActive === false));
});
var initial = PolishFourthLeagueService.createSeason(clubs, template, 812734);
clubs = initial.clubs;
var initialFourthLeagueSizes = new Map(FOURTH_LEAGUE_IDS.map((leagueId) => [
  leagueId,
  clubs.filter((club) => club.leagueId === leagueId).length
]));
var sharedLeagueSlots = template.slots.filter((slot) => String(slot.competition) === "LEAGUE").sort((left, right) => left.start.getTime() - right.start.getTime());
FOURTH_LEAGUE_IDS.forEach((leagueId) => {
  const leagueClubs = clubs.filter((club) => club.leagueId === leagueId);
  import_strict.default.ok([14, 16, 18].includes(leagueClubs.length), `${leagueId} has an invalid size`);
  const fixtures = initial.state.fixtures[leagueId];
  import_strict.default.equal(fixtures.length, leagueClubs.length * (leagueClubs.length - 1));
  import_strict.default.equal(new Set(fixtures.map((fixture) => fixture.round)).size, (leagueClubs.length - 1) * 2);
  ...
  if (leagueClubs.length < 18) {
    ...
    import_strict.default.equal(Math.min(...fixtureDates), sharedLeagueSlots[expectedFirstSlot].start.getTime(), `${leagueId} must use its later start date`);
    import_strict.default.equal(Math.max(...fixtureDates), sharedLeagueSlots[32].start.getTime(), `${leagueId} must finish on 15 May`);
  }
});
var completed = PolishFourthLeagueService.processDate(
  initial.state,
  clubs,
  new Date(2027, 5, 30),
  812734
...
var repeated = PolishFourthLeagueService.processDate(
  completed.state,
  completed.clubs,
  new Date(2027, 5, 30),
  812734
);
import_strict.default.equal(repeated.played, 0, "finished IV-liga fixtures must be idempotent");
FOURTH_LEAGUE_IDS.forEach((leagueId) => {
  const tableGoals = completed.clubs.filter((club) => club.leagueId === leagueId).reduce((sum, club) => sum + club.stats.goalsFor, 0);
  const playerGoals = completed.state.playerStats[leagueId].reduce((sum, row) => sum + row.goals, 0);
  import_strict.default.equal(playerGoals, tableGoals, `${leagueId} scorer totals do not match match goals`);
});
var resolution = PolishThirdLeagueSeasonTransitionService.resolve(
  completed.clubs,
  {},
  812734,
  1,
...
THIRD_LEAGUE_GROUP_IDS.forEach((groupId) => {
  import_strict.default.equal(resolution.promotedFromFeederByGroup[groupId].length, 5, `${groupId} must receive five IV-liga clubs`);
});
import_strict.default.equal(
...
var nextClubs = completed.clubs.map((club) => {
  const leagueId = resolution.nextLeagueByClubId.get(club.id) ?? club.leagueId;
  return { ...club, leagueId };
});
THIRD_LEAGUE_GROUP_IDS.forEach((groupId) => {
  import_strict.default.equal(nextClubs.filter((club) => club.leagueId === groupId).length, 18, `${groupId} must return to 18 clubs`);
});
var clubsBeforeFourthLeagueRebalance = nextClubs.map((club) => ({ ...club }));
var poolMembershipBeforeRebalance = new Map(FOURTH_LEAGUE_FEEDER_IDS.map((poolId) => [
  poolId,
...
var repeatedRebalance = PolishFourthLeagueService.rebalanceForNextSeason(clubsBeforeFourthLeagueRebalance, 2027, 812735);
Object.entries(FOURTH_LEAGUE_FEEDER_BY_VOIVODESHIP).forEach(([voivodeship, poolId]) => {
  const leagueId = FOURTH_LEAGUE_BY_VOIVODESHIP[voivodeship];
  const previousPoolIds = poolMembershipBeforeRebalance.get(poolId);
  const promotedIds = nextClubs.filter((club) => club.leagueId === leagueId && previousPoolIds.has(club.id)).map((club) => club.id).sort();
  const repeatedPromotedIds = repeatedRebalance.filter((club) => club.leagueId === leagueId && previousPoolIds.has(club.id)).map((club) => club.id).sort();
  import_strict.default.equal(promotedIds.length, 4, `${voivodeship} must promote four clubs from its own pool`);
  import_strict.default.deepEqual(repeatedPromotedIds, promotedIds, `${voivodeship} pool draw must be deterministic`);
  import_strict.default.equal(nextClubs.filter((club) => club.leagueId === poolId).length, 18, `${poolId} must be restored to 18 clubs`);
  import_strict.default.equal(
...
var nextSeason = PolishFourthLeagueService.createSeason(
  nextClubs,
