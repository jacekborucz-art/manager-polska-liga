...
// tests/CupMatchEngineV2FullFactorTests.ts
...
var selectAction = (player, zone, pattern, instructions, roll) => {
  ...
  const shortPassing2 = instructions.passing === "SHORT";
  ...
  return pickWeighted([
    {
      item: "PASS",
      weight: passQuality * (shortPassing2 ? 1.48 : 1) * (pattern === "BUILD_UP" ? 1.22 : 1)
    },
    {
      item: "DIRECT_PASS",
...
var resolveSetPieceDelivery = ({
  ...
  if (!chance) {
    return {
      ...
      nextZone: kind === "PENALTY" ? "BOX" : "FINAL_THIRD",
      momentumDelta: attacking.side === "HOME" ? 1.4 : -1.4,
      events
    };
  }
...
var smallNoise = (seed, salt) => {
  let hash = 0;
  for (let index = 0; index < seed.length; index += 1) {
    hash = hash * 31 + seed.charCodeAt(index) + salt >>> 0;
  }
  return (hash % 1e3 / 1e3 - 0.5) * 8;
};
var makeInstructions = (overrides = {}) => ({
  ...DEFAULT_INSTRUCTIONS,
  ...overrides
});
var makeTactic = (formation, id) => {
  const attackBias = formation === "4-3-3" ? 66 : formation === "4-2-3-1" ? 60 : formation === "5-4-1" ? 42 : formation === "4-5-1" ? 48 : 54;
  const defenseBias = formation === "5-4-1" ? 72 : formation === "4-5-1" ? 66 : formation === "4-3-3" ? 46 : formation === "4-2-3-1" ? 56 : 58;
  const pressingIntensity = formation === "4-3-3" ? 66 : formation === "5-4-1" ? 42 : formation === "4-5-1" ? 54 : 56;
  return {
    id,
    name: formation,
...
var positionalBoost = (position, key) => {
  if (position === "GK" /* GK */) {
...
var profileBoost = (profile, key, position) => {
  ...
  if (profile === "KEEPER_PLUS") {
    return position === "GK" /* GK */ && ["goalkeeping", "positioning", "mentality", "strength"].includes(key) ? 16 : 0;
  }
...
var makeAttributes = (seed, position, quality, profile) => {
  const attrs = {};
  ...
  if (position !== "GK" /* GK */) attrs.goalkeeping = Math.round(clamp(14 + smallNoise(seed, 99), 5, 28));
  return attrs;
};
var overallFor = (position, attrs) => {
  const weights = {
...
var makeSquad = (prefix, quality, profile, condition, morale) => {
  const positions = [
    "GK" /* GK */,
    "GK" /* GK */,
    "DEF" /* DEF */,
...
var buildLineup = (players, clubId, tactic) => {
...
var eventXg = (result, side, fromSecond) => result.events.filter((event) => event.side === side && event.second >= fromSecond && typeof event.xG === "number").reduce((sum, event) => sum + (event.xG ?? 0), 0);
var avgFinalFatigue = (input, result, side) => {
  const team = side === "HOME" ? input.home : input.away;
  const ids = team.lineup.startingXI.filter((id) => Boolean(id));
  return ids.reduce((sum, id) => sum + (result.finalState.fatigue[id] ?? 0), 0) / Math.max(1, ids.length);
};
var summarize = (label, pairs) => {
  ...
  const totals = pairs.reduce((sum, pair) => {
    ...
    return {
      ...
      substitutions: sum.substitutions + events.filter((event) => event.type === "SUBSTITUTION" /* SUBSTITUTION */).length,
      ...
      homePoints: sum.homePoints + (result.homeScore > result.awayScore ? 3 : result.homeScore === result.awayScore ? 1 : 0),
...
var runSeries = (label, options, count = 32) => {
...
var assertHigher = (actual, baseline, diff, message) => {
  import_strict.default.ok(actual >= baseline + diff, `${message}: ${actual.toFixed(3)} < ${baseline.toFixed(3)} + ${diff}`);
};
var assertLower = (actual, baseline, diff, message) => {
  import_strict.default.ok(actual <= baseline - diff, `${message}: ${actual.toFixed(3)} > ${baseline.toFixed(3)} - ${diff}`);
};
var assertInjuredStarterWeakensTeamProfile = () => {
  ...
  const forwardId = input.home.lineup.startingXI.find(
    (id) => Boolean(id && input.home.players.find((player) => player.id === id)?.position === "FWD" /* FWD */)
  );
  import_strict.default.ok(forwardId, "Test kontuzji wymaga napastnika w wyj\u015Bciowym sk\u0142adzie");
  ...
  assertLower(severeInjury.finishing, lightInjury.finishing, 3, "Ci\u0119\u017Cko kontuzjowany napastnik musi os\u0142abia\u0107 wyko\u0144czenie bardziej ni\u017C lekki uraz");
  assertLower(severeInjury.staminaReserve, lightInjury.staminaReserve, 1, "Ci\u0119\u017Cko kontuzjowany zawodnik musi mocniej obni\u017Ca\u0107 rezerw\u0119 fizyczn\u0105 dru\u017Cyny");
};
assertInjuredStarterWeakensTeamProfile();
var mirror442 = runSeries("mirror_442", {
  homeFormation: "4-4-2",
  awayFormation: "4-4-2"
}, 20);
var attackAttributes = runSeries("attack_attributes", {
  homeProfile: "ATTACK_PLUS",
  awayProfile: "BALANCED"
}, 20);
var defenseAttributes = runSeries("defense_attributes", {
  homeProfile: "BALANCED",
  awayProfile: "DEFENSE_PLUS",
  awayFormation: "5-4-1",
  awayInstructions: { mindset: "DEFENSIVE", marking: "ZONE" }
}, 20);
var keeperAttributes = runSeries("keeper_attributes", {
  homeProfile: "BALANCED",
  awayProfile: "KEEPER_PLUS"
}, 20);
var aggressiveAttributes = runSeries("aggressive_attributes", {
...
var lowStaminaPress = runSeries("low_stamina_press", {
...
var attacking433 = runSeries("attacking_433_vs_541", {
  homeFormation: "4-3-3",
  awayFormation: "5-4-1",
...
var midfield451 = runSeries("midfield_451_vs_442", {
  homeFormation: "4-5-1",
  awayFormation: "4-4-2",
...
var slowDefensive = runSeries("slow_defensive", {
...
var fastOffensive = runSeries("fast_offensive", {
...
var shortPassing = runSeries("short_passing", {
...
var longCounter = runSeries("long_counter", {
...
var noMarking = runSeries("no_marking", {
  homeInstructions: { marking: "NONE" },
  awayInstructions: { marking: "NONE" }
}, 20);
var manMarking = runSeries("man_marking", {
  homeInstructions: { marking: "MAN", intensity: "AGGRESSIVE" },
  awayInstructions: { marking: "MAN", intensity: "AGGRESSIVE" }
}, 20);
var goodConditions = runSeries("good_weather_pitch", {
  weather: CLEAR_WEATHER,
  pitchQuality: 92
}, 24);
var badConditions = runSeries("bad_weather_pitch", {
  weather: STORM_WEATHER,
  pitchQuality: 46
}, 24);
var lenientRef = runSeries("lenient_referee", {
...
var strictRef = runSeries("strict_referee", {
  referee: STRICT_REFEREE,
  homeInstructions: { intensity: "AGGRESSIVE", pressing: "PRESSING", marking: "MAN" },
  awayInstructions: { intensity: "AGGRESSIVE", pressing: "PRESSING", marking: "MAN" }
}, 24);
var lowMotivation = runSeries("low_motivation", {
  homeMorale: 42,
  homeMotivation: 42,
  homeSupport: 48,
  awayMorale: 60,
...
var highMotivation = runSeries("high_motivation", {
  homeMorale: 76,
  homeMotivation: 82,
  homeSupport: 64,
  awayMorale: 60,
...
var noHalfTimeTalk = runSeries("no_half_time_talk", {
  homeMorale: 58,
  homeMotivation: 58,
  awayMorale: 58,
  awayMotivation: 58
}, 24);
var strongHalfTimeTalk = runSeries("strong_half_time_talk", {
  homeMorale: 58,
  homeMotivation: 58,
  awayMorale: 58,
  awayMotivation: 58,
...
import_strict.default.ok(mirror442.totalShots >= 12 && mirror442.totalShots <= 30, `4-4-2 vs 4-4-2 ma nierealn\u0105 liczb\u0119 strza\u0142\xF3w: ${mirror442.totalShots}`);
import_strict.default.ok(mirror442.homePossessionPct >= 43 && mirror442.homePossessionPct <= 57, `4-4-2 vs 4-4-2 nie jest neutralne w posiadaniu: ${mirror442.homePossessionPct}`);
import_strict.default.ok(mirror442.homeWinPct >= 30 && mirror442.homeWinPct <= 70, `4-4-2 vs 4-4-2 nie jest neutralne w zwyci\u0119stwach: ${mirror442.homeWinPct}`);
...
assertLower(defenseAttributes.homeXg, mirror442.homeXg, 0.1, "Lepsza obrona/formacja defensywna rywala musi obni\u017Ca\u0107 xG gospodarzy");
assertLower(keeperAttributes.homeGoals, mirror442.homeGoals, 0.1, "Lepszy bramkarz rywala musi obni\u017Ca\u0107 gole gospodarzy");
...
assertHigher(attacking433.homeShots, attacking433.awayShots, 2, "4-3-3 ofensywne musi generowa\u0107 wi\u0119cej strza\u0142\xF3w ni\u017C 5-4-1 defensywne");
assertHigher(attacking433.homeXg, attacking433.awayXg, 0.22, "4-3-3 ofensywne musi generowa\u0107 wi\u0119ksze xG ni\u017C 5-4-1 defensywne");
import_strict.default.ok(midfield451.homePossessionPct >= 51.5, `4-5-1 z kr\xF3tkim podaniem musi dawa\u0107 przewag\u0119 kontroli \u015Brodka/posiadania: ${midfield451.homePossessionPct}`);
assertHigher(fastOffensive.totalShots, slowDefensive.totalShots, 2, "Szybkie ofensywne nastawienie musi zwi\u0119ksza\u0107 liczb\u0119 strza\u0142\xF3w");
assertHigher(fastOffensive.totalXg, slowDefensive.totalXg, 0.25, "Szybkie ofensywne nastawienie musi zwi\u0119ksza\u0107 xG");
assertLower(fastOffensive.homeFinalFatigue, slowDefensive.homeFinalFatigue, 0.9, "Szybkie ofensywne nastawienie musi mocniej m\u0119czy\u0107 zawodnik\xF3w");
assertHigher(longCounter.directEvents, shortPassing.directEvents, 2, "D\u0142ugie podania musz\u0105 zwi\u0119ksza\u0107 liczb\u0119 akcji bezpo\u015Brednich");
assertHigher(longCounter.counterEvents, shortPassing.counterEvents, 2, "Kontratak musi zwi\u0119ksza\u0107 liczb\u0119 akcji z kontry");
...
assertLower(badConditions.totalOnTarget, goodConditions.totalOnTarget, 0.45, "Z\u0142a pogoda musi obni\u017Ca\u0107 liczb\u0119 celnych strza\u0142\xF3w");
assertHigher(badConditions.injuries, goodConditions.injuries, 0.04, "Z\u0142a pogoda i murawa musz\u0105 zwi\u0119ksza\u0107 ryzyko uraz\xF3w");
assertHigher(strictRef.yellows, lenientRef.yellows, 0.45, "Surowy s\u0119dzia musi zwi\u0119ksza\u0107 liczb\u0119 \u017C\xF3\u0142tych kartek");
...
assertHigher(highMotivation.homePointsPerMatch, lowMotivation.homePointsPerMatch, 0.1, "Motywacja przedmeczowa i stadion musz\u0105 zwi\u0119ksza\u0107 dorobek punktowy gospodarzy");
assertHigher(
