...
var FinanceService = {
   ...
   */
  evaluateReleaseRequest: (player, club2, squad) => {
  ...
  evaluateFASigningBoardDecision: (player, proposedSalary, proposedBonus, squad, club2) => {
  ...
  evaluateRenewalBoardDecision: (player, proposedSalary, proposedBonus, squad, club2) => {
...
var IncomingTransferService = {
  ...
  getLoanSquadNeed(player, buyerSquad) {
    if (!buyerSquad || buyerSquad.length === 0) {
      return { fits: true, needScore: 9, positionGap: 9, squadGap: 9 };
    }
    const squadAverage = IncomingTransferService.getSquadAverageOverall(buyerSquad);
    const samePosition = buyerSquad.filter((squadPlayer) => squadPlayer.position === player.position);
    const positionAverage = samePosition.length > 0 ? IncomingTransferService.getSquadAverageOverall(samePosition) : squadAverage - 3;
    const bestInPosition = samePosition.length > 0 ? Math.max(...samePosition.map((squadPlayer) => squadPlayer.overallRating)) : squadAverage - 4;
    const positionGap = player.overallRating - positionAverage;
    ...
    const isDevelopmentLoan = player.age <= 23 && player.overallRating >= positionAverage - 4 && strongerOrEqualInPosition < matchdayRotationLimit + 2;
    const rotationScore = isInsidePositionRotation ? Math.max(3, matchdayRotationLimit - strongerOrEqualInPosition) : 0;
  ...
  shouldGenerateLoanOffer(player, buyerClub, sellerClub, activeIncomingOffers, seed, currentDate, buyerPlayers) {
  ...
  simulateLoanPlayerDecision(player, buyerClub, sellerClub, buyerSquad, seed) {
    const rng = IncomingTransferService.seededRandom(seed);
    const repDelta = buyerClub.reputation - sellerClub.reputation;
