...
var CupShadowSimulationService = {
  simulateFromMatchContext: (ctx2, options) => {
    ...
    const result = CupMatchEngineV2.simulate(adapted2.input);
    const matchSummary = CupMatchReportAdapter.fromMatchContext(ctx2, adapted2.input, result, {
      userTeamId: options.userSide === "AWAY" ? ctx2.awayClub.id : ctx2.homeClub.id
    });
    const summary = summarizeResult(adapted2.input, result);
    return {
      input: adapted2.input,
      result,
      matchSummary,
      ...
      initialLineup: {
        HOME: {
          ...adapted2.input.home.lineup,
          startingXI: [...adapted2.input.home.lineup.startingXI],
          bench: [...adapted2.input.home.lineup.bench],
          reserves: [...adapted2.input.home.lineup.reserves]
        },
        AWAY: {
          ...adapted2.input.away.lineup,
          startingXI: [...adapted2.input.away.lineup.startingXI],
          bench: [...adapted2.input.away.lineup.bench],
          reserves: [...adapted2.input.away.lineup.reserves]
        }
      }
    };
  },
  ...
  createLiveMatch: (ctx2, options) => {
    ...
    return {
      ...
      options,
      diagnostics: adapted2.diagnostics,
      live: CupMatchEngineV2.createLiveMatch(adapted2.input)
    };
  },
