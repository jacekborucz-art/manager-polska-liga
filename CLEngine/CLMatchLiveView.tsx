...
const getEuropeanDebriefMatchStage = (competition: CompetitionType): DebriefMatchStage => {
  if (competition === CompetitionType.CL_FINAL) return 'CUP_FINAL';
  if (competition === CompetitionType.CL_SF || competition === CompetitionType.CL_SF_RETURN) return 'CUP_SEMIFINAL';
  if (competition === CompetitionType.CL_GROUP_STAGE) return 'LEAGUE';
  return 'CUP';
};
  ...
  useEffect(() => {
   if (ctx && (!matchState || matchState.fixtureId !== ctx.fixture.id)) {
      ...
      const aiLineupBase = LineupService.autoPickLineup(aiClubInit.id, aiPlayersInit, '4-4-2', aiCoachInit, {
        ...
        formAware: true,
        selectionSeed: `${ctx.fixture.id}_${aiClubInit.id}_cl_live_ai_start`
      });
      ...
      const aiPreparedLineup = LineupService.autoPickLineup(aiClubInit.id, aiPlayersInit, aiPreparedTacticId, aiCoachInit, {
        competitionId: ctx.fixture.leagueId as string,
        formAware: true,
        selectionSeed: `${ctx.fixture.id}_${aiClubInit.id}_cl_live_ai_${aiPreparedTacticId}_${preMatchInstr.tempo}_${preMatchInstr.mindset}`,
...
const applyHalftimeRegen = (fatigueMap: Record<string, number>, playersList: Player[]) => {
 ...
 const recoveredHomeFatigue = (prev.period === 1 || prev.period === 3) ? applyHalftimeRegen(prev.homeFatigue, ctx.homePlayers) : prev.homeFatigue;
       ...
       const engineComment = MatchEngineService.generateCommentary(nextMinute, currentSeed, ctx.homeClub.name, ctx.awayClub.name);
        ...
        // Krzywa kary: kondycja 94â†’0 | 85â†’-0.022 | 80â†’-0.039 | 75â†’-0.057 | 70â†’-0.077 | 60â†’-0.118
        // Brak zmian ma byÄ‡ odczuwalny wczeÅ›niej, szczegÃ³lnie gdy rywal wprowadziÅ‚ Å›wieÅ¼e nogi.
          ...
          if (isHome) {
            if (ticketRevenue > 0) {
              financeLogsToAdd.push({
                ...
                description: `Bilety (vs ${ctx.awayClub.name}): ${attendance} widzÃ³w @ ${ticketAvgPrice} PLN`,
...
const summary: MatchSummary = {
  ...
  const handleCLDebriefClose = (effect: DebriefEffect) => {
