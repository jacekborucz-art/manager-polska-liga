...
const PANEL_GLASS = 'relative overflow-hidden rounded-[20px] border border-[rgba(96,165,250,0.42)] bg-slate-950/54 shadow-[inset_0_1px_0_rgba(96,165,250,0.20),0_0_0_1px_rgba(37,99,235,0.12),0_24px_80px_rgba(0,0,0,0.48)] backdrop-blur-2xl';
const CELL_GLASS = 'relative overflow-hidden rounded-[16px] border border-[rgba(96,165,250,0.38)] bg-slate-950/48 shadow-[inset_0_1px_0_rgba(96,165,250,0.18),0_0_0_1px_rgba(59,130,246,0.12),0_14px_40px_rgba(0,0,0,0.28)]';
...
export const MysteryAgentNegotiationView: React.FC = () => {
  ...
  return (
    <div className="agent-offer-shell relative flex min-h-screen items-center justify-center overflow-x-hidden overflow-y-auto bg-slate-950 px-4 py-6 text-slate-50 sm:px-7 lg:px-10">
      <AgentSvgBackground />
      <div className="relative z-10 mx-auto flex w-full max-w-[1460px] flex-col">
        <header className="mb-5 shrink-0 border-b border-blue-200/15 pb-4">
          <div className="flex items-start justify-between gap-5">
            <div className="min-w-0">
              ...
              <h1 className={`${DISPLAY_FONT} mt-2 whitespace-nowrap text-[clamp(2rem,4.7vw,4.2rem)] leading-[0.86] text-white drop-shadow-[0_8px_22px_rgba(0,0,0,0.6)]`}>
                Propozycja agenta
              </h1>
            </div>
            <button
              onClick={() => navigateTo(ViewState.DASHBOARD)}
              className={`${DISPLAY_FONT} group inline-flex h-11 shrink-0 items-center justify-center gap-2 rounded-full border border-emerald-200/70 bg-gradient-to-b from-emerald-300 via-emerald-500 to-emerald-800 px-5 text-xs text-white shadow-[inset_0_2px_0_rgba(255,255,255,0.42),inset_0_-4px_0_rgba(4,120,87,0.72),0_5px_0_rgba(4,80,61,0.95),0_18px_38px_rgba(16,185,129,0.28)] transition hover:from-emerald-200 hover:via-emerald-400 hover:to-emerald-700 hover:shadow-[inset_0_2px_0_rgba(255,255,255,0.48),inset_0_-4px_0_rgba(4,120,87,0.66),0_5px_0_rgba(4,80,61,0.95),0_22px_44px_rgba(16,185,129,0.34)] active:translate-y-[2px] active:shadow-[inset_0_2px_0_rgba(255,255,255,0.32),inset_0_-2px_0_rgba(4,120,87,0.7),0_2px_0_rgba(4,80,61,0.95),0_10px_24px_rgba(16,185,129,0.22)]`}
            >
              <AgentIcon name="close" className="h-4 w-4 text-white transition group-hover:text-white" />
              WYJŚCIE
            </button>
          </div>
        </header>

        <main className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(430px,0.82fr)_minmax(580px,1fr)]">
          <section className={`${PANEL_GLASS} h-full p-4 sm:p-5`}>
            <PanelGloss />
            <div className="relative z-10 flex h-full flex-col">
              <div className="flex items-center gap-4">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[12px] border border-blue-300/50 bg-gradient-to-br from-blue-500/22 to-slate-950/58 text-blue-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.18),0_18px_38px_rgba(37,99,235,0.20)]">
                  <AgentIcon name="profile" className="h-7 w-7" />
                </div>
                <div className="min-w-0">
                  <p className="whitespace-nowrap text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">
                    Klient agenta
                  </p>
                  <h2 className={`${DISPLAY_FONT} mt-1.5 whitespace-nowrap text-[clamp(1.4rem,2.05vw,2rem)] leading-none text-white drop-shadow-[0_6px_18px_rgba(0,0,0,0.45)]`}>
                    {player.firstName} {player.lastName}
                  </h2>
                </div>
              </div>

              <div className="mt-4 grid grid-cols-2 overflow-hidden rounded-[18px] border border-[rgba(96,165,250,0.34)] bg-slate-950/38 shadow-[inset_0_1px_0_rgba(96,165,250,0.12)]">
              ...
              <div className={`${CELL_GLASS} mt-4 p-4`}>
                <div className="relative z-10 flex items-start gap-3">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-amber-300/40 bg-amber-300/8 text-amber-300">
                  ...
                  <div className="min-w-0">
                    <p className={`${DISPLAY_FONT} text-[clamp(0.92rem,1.25vw,1.2rem)] leading-[1.16] text-amber-300 drop-shadow-[0_2px_10px_rgba(251,191,36,0.22)]`}>
                      "{mysteryAgentOffer.lastAgentMessage}"
                    </p>
                    <p className="mt-3 text-[clamp(0.7rem,0.8vw,0.8rem)] leading-relaxed text-slate-300">
          ...
          <section className={`${PANEL_GLASS} h-full p-4 sm:p-5`}>
            ...
            <div className="relative z-10 flex h-full flex-col gap-3">
              ...
              <div className={`${CELL_GLASS} p-3.5`}>
                <div className="relative z-10 flex items-center gap-3">
                  <AgentIcon name="calendar" className="h-5 w-5 shrink-0 text-blue-300" />
                  <p className={`${DISPLAY_FONT} whitespace-nowrap text-sm text-slate-300`}>
                    Długość kontraktu
                  </p>
                </div>
                <div className="relative z-10 mt-3 grid grid-cols-5 gap-2.5">
                  {[1, 2, 3, 4, 5].map(yearOption => (
                    <button
                      key={yearOption}
                      onClick={() => setYears(yearOption)}
                      disabled={isEnded}
                      className={`${DISPLAY_FONT} h-9 rounded-[8px] border text-sm transition ${
                        years === yearOption
                          ? 'border-[#5b8dff] bg-gradient-to-b from-[#315fbd] via-[#234a9b] to-[#132f68] text-white shadow-[inset_0_1px_0_rgba(191,219,254,0.58),0_0_18px_rgba(59,130,246,0.26)]'
                          : 'border-[#202b3d] bg-gradient-to-b from-[#202a3a] via-[#172131] to-[#111827] text-[#a7b6cc] shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_8px_18px_rgba(0,0,0,0.26)] hover:border-[#35527a] hover:from-[#25334a] hover:via-[#1b2940] hover:to-[#131c2b]'
                      }`}
                    >
                      {yearOption}
                    </button>
              ...
              <div className={`${CELL_GLASS} p-3.5`}>
                <div className="relative z-10 flex items-center justify-between gap-5">
                  <div className="flex min-w-0 items-center gap-3">
                    <AgentIcon name="wallet" className="h-6 w-6 shrink-0 text-slate-300" />
                    <span className={`${DISPLAY_FONT} whitespace-nowrap text-sm text-slate-400`}>
                      Łączny pakiet
                    </span>
                  </div>
                  <span className={`${DISPLAY_FONT} whitespace-nowrap text-[clamp(1.25rem,1.72vw,1.68rem)] leading-none text-white`}>
              ...
              <div className="grid grid-cols-3 gap-2.5">
                <button
                  ...
                  disabled={isEnded}
                  className={`${DISPLAY_FONT} inline-flex h-11 min-w-0 items-center justify-center gap-2 rounded-[10px] border border-emerald-200/35 bg-gradient-to-b from-emerald-400/90 to-emerald-700/82 px-3 text-[clamp(0.64rem,0.78vw,0.8rem)] leading-none text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.28),0_12px_28px_rgba(16,185,129,0.18)] transition hover:brightness-110 disabled:border-blue-300/10 disabled:bg-none disabled:bg-slate-800 disabled:text-slate-500 disabled:shadow-none`}
                >
                  <AgentIcon name="send" className="h-4 w-4 shrink-0" />
                  <span className="whitespace-nowrap">WYŚLIJ OFERTĘ</span>
                </button>
                <button
                  onClick={handleBoardRequest}
                  disabled={isEnded || missingBudget <= 0 || mysteryAgentOffer.boardRequestUsed}
                  className={`${DISPLAY_FONT} inline-flex h-11 min-w-0 items-center justify-center gap-2 rounded-[10px] border border-[rgba(96,165,250,0.38)] bg-blue-950/30 px-3 text-[clamp(0.62rem,0.74vw,0.78rem)] leading-none text-blue-100 shadow-[inset_0_1px_0_rgba(96,165,250,0.12)] transition hover:border-[rgba(147,197,253,0.58)] hover:bg-blue-500/20 disabled:text-slate-500 disabled:opacity-70`}
                >
                  <AgentIcon name="board" className="h-4 w-4 shrink-0" />
                  <span className="whitespace-nowrap">PROŚBA DO ZARZĄDU</span>
                </button>
                <button
                  onClick={declineMysteryAgentOffer}
                  disabled={isEnded}
                  className={`${DISPLAY_FONT} inline-flex h-11 min-w-0 items-center justify-center gap-2 rounded-[10px] border border-red-400/45 bg-red-950/45 px-3 text-[clamp(0.58rem,0.68vw,0.74rem)] leading-none text-red-200 shadow-[inset_0_1px_0_rgba(254,202,202,0.10),0_10px_24px_rgba(127,29,29,0.14)] transition hover:bg-red-900/55 disabled:opacity-45`}
                >
                  <AgentIcon name="close" className="h-4 w-4 shrink-0" />
                  <span className="whitespace-nowrap">ZREZYGNUJ Z ROZMÓW</span>
                </button>
              </div>
...
const InfoBox: React.FC<{ icon: AgentIconName; label: string; value: string; accent?: boolean }> = ({ icon, label, value, accent }) => (
  <div className="relative min-w-0 border-[rgba(96,165,250,0.34)] p-4 odd:border-r [&:nth-child(-n+2)]:border-b">
...
const MetricBox: React.FC<{ icon: AgentIconName; label: string; value: string }> = ({ icon, label, value }) => (
  <div className={`${CELL_GLASS} min-h-[76px] p-3.5`}>
    <div className="relative z-10 flex h-full items-center gap-4">
      <AgentIcon name={icon} className="h-6 w-6 shrink-0 text-blue-300" />
      <div className="min-w-0">
        <p className="whitespace-nowrap text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">{label}</p>
        <p className={`${DISPLAY_FONT} mt-1 whitespace-nowrap text-[clamp(1rem,1.15vw,1.25rem)] leading-none text-white`}>
          {value}
        </p>
      </div>
    </div>
...
const MoneyControl: React.FC<{
  ...
  return (
    <div className={`${CELL_GLASS} min-h-[96px] p-3.5`}>
      <div className="relative z-10 flex h-full flex-col justify-between gap-2.5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <AgentIcon name={icon} className="h-6 w-6 shrink-0 text-slate-300" />
            <span className={`${DISPLAY_FONT} min-w-0 whitespace-nowrap text-[clamp(0.72rem,0.78vw,0.84rem)] leading-none text-slate-300`}>
              {label}
            </span>
          </div>
          <div className="flex h-8 w-[128px] shrink-0 items-center justify-end gap-1.5 px-1">
            <input
              type="number"
              value={value}
              min={0}
              ...
              className={`${DISPLAY_FONT} w-[92px] bg-transparent text-right text-[14px] leading-none outline-none ${toneClass}`}
            />
            <span className="whitespace-nowrap text-[9px] font-black uppercase tracking-wide text-slate-400">PLN</span>
          </div>
        </div>
        <input
          type="range"
