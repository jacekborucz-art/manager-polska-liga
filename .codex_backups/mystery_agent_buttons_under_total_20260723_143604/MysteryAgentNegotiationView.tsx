        ...
        <main className="grid flex-1 grid-cols-1 gap-4 lg:grid-cols-[minmax(430px,0.82fr)_minmax(580px,1fr)]">
          ...
          <section className={`${PANEL_GLASS} p-4 sm:p-5`}>
            ...
            <div className="relative z-10 flex h-full min-h-0 flex-col gap-4">
              ...
              <div className="mt-auto grid grid-cols-1 gap-2.5 md:grid-cols-[minmax(0,1fr)_minmax(150px,0.42fr)]">
                <button
                  onClick={handleSubmit}
                  disabled={isEnded}
                  className={`${DISPLAY_FONT} inline-flex h-12 items-center justify-center gap-3 rounded-[11px] border border-emerald-200/35 bg-gradient-to-b from-emerald-400/90 to-emerald-700/82 px-5 text-sm text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.28),0_16px_38px_rgba(16,185,129,0.22)] transition hover:brightness-110 disabled:border-blue-300/10 disabled:bg-none disabled:bg-slate-800 disabled:text-slate-500 disabled:shadow-none`}
                >
                  <AgentIcon name="send" className="h-5 w-5 shrink-0" />
                  Wyślij ofertę
                </button>
                <button
                  onClick={handleBoardRequest}
                  disabled={isEnded || missingBudget <= 0 || mysteryAgentOffer.boardRequestUsed}
                  className={`${DISPLAY_FONT} inline-flex h-12 items-center justify-center gap-2 rounded-[11px] border border-blue-300/16 bg-blue-950/24 px-4 text-sm text-blue-100 shadow-[inset_0_1px_0_rgba(147,197,253,0.10)] transition hover:border-blue-300/34 hover:bg-blue-500/20 disabled:text-slate-500 disabled:opacity-70`}
                >
                  <AgentIcon name="board" className="h-5 w-5 shrink-0" />
                  Do zarządu
                </button>
                <button
                  onClick={declineMysteryAgentOffer}
                  disabled={isEnded}
                  className={`${DISPLAY_FONT} inline-flex h-9 w-full items-center justify-center gap-2 justify-self-center rounded-[10px] border border-red-400/45 bg-red-950/45 px-6 text-xs text-red-200 shadow-[inset_0_1px_0_rgba(254,202,202,0.10),0_10px_26px_rgba(127,29,29,0.16)] transition hover:bg-red-900/55 disabled:opacity-45 md:col-span-2 md:w-[300px]`}
                >
