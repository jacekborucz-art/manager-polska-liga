...
const BRANDED_CLUB_CARDS: ClubCardAssets[] = [
  {
...
function closestImage(kits: KitVariant[], targetHex: string): string {
  let best = kits[0];
  let minDist = Infinity;
  for (const kit of kits) {
    const dist = colorDistance(kit.hex, targetHex);
    if (dist < minDist) { minDist = dist; best = kit; }
  }
  return best.image;
}
 ...
 */
export function getPlayerCardImage(clubId: string, kitHex: string): string {
