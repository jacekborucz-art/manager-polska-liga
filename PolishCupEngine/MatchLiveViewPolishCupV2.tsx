        ...
        setMatchState(prev => {
          ...
          if (isGoal) {
            ...
            const surge = updateAiMentalitySurgeAfterGoal({
              ...
              minute: prev.minute,
              rng: () => seededRng(prev.sessionSeed, prev.minute + (prev.period - 1) * 300, surgeOffset++),
            });
 ...
 const recoveredHomeFatigue = !isFT ? applyHalftimeRegen(prev.homeFatigue, ctx.homePlayers) : prev.homeFatigue;
        ...
        const getInjuredPlayerChanceImpact = (
          ...
          LiveMatchPhysicalMismatchService.getInjuryChanceImpact({
            ...
            mode,
            rng: () => seededRng(currentSeed, rngMinute, rngOffset),
          });
        ...
        const applyAiMentalitySurgeForGoal = (scoringSide: 'HOME' | 'AWAY') => {
          ...
          const surge = updateAiMentalitySurgeAfterGoal({
            ...
            currentBoost: nextAiMentalitySurgeBoost,
            minute: nextMinute,
            rng: () => seededRng(currentSeed, rngMinute, surgeOffset++),
          });
          nextAiMentalitySurgeBoost = surge.boost;
          nextAiMentalitySurgePreviousBoost = surge.previousBoost;
          nextAiMentalitySurgeLastGoalMinute = surge.lastGoalMinute;
       ...
       const engineComment = MatchEngineService.generateCommentary(nextMinute, currentSeed, ctx.homeClub.name, ctx.awayClub.name);
        ...
        if (activeSide !== userSide && counterAttackEnabled && counterShape && opponentPushes) {
          ...
          if (seededRng(currentSeed, rngMinute, 631) < counterChance) {
            activeSide = userSide;
        ...
        const userPushes =
          ...
          userScoreDiff < 0;
        const userOvercommitsIntoAiBlock =
          prev.userInstructions.mindset === 'OFFENSIVE' &&
          userCounterTactic.attackBias >= 60 &&
          (
            aiCounterTacticObj.defenseBias >= 74 ||
            aiCounterTacticObj.attackBias <= 42 ||
            prev.aiActiveShout?.mindset === 'DEFENSIVE'
          ) &&
          (
            aiCounterAttackEnabled ||
            prev.aiActiveShout?.mindset === 'DEFENSIVE' ||
            prev.aiActiveShout?.mindset === 'NEUTRAL'
          );
        const userOvercommitIntoAiBlockFactor = userOvercommitsIntoAiBlock
          ? Math.max(
              0.35,
              Math.min(
                1,
                (userCounterTactic.attackBias - 58) / 32 +
                (aiCounterTacticObj.defenseBias - 68) / 34 +
                (prev.userInstructions.tempo === 'FAST' ? 0.18 : 0) +
                (aiCounterAttackEnabled ? 0.16 : 0)
              )
            )
          : 0;
        ...
        if (!counterAttackTriggered && activeSide === userSide && aiCounterAttackEnabled && aiCounterShape && userPushes) {
          ...
          const aiCounterChance = Math.max(
            0,
            Math.min(
              userOvercommitsIntoAiBlock ? 0.20 : 0.14,
              (0.023 +
                userPressFactor * 0.054 +
                aiShapeFactor * 0.022 +
                userRiskFactor * 0.022 +
                aiScoreFactor +
                userOvercommitIntoAiBlockFactor * 0.036) * aiCounterResponseFactor
            )
          );
          if (seededRng(currentSeed, rngMinute, 641) < aiCounterChance) {
            ...
            aiCounterAttackShotBonus = Math.max(
              0.006,
              Math.min(
                userOvercommitsIntoAiBlock ? 0.050 : 0.028,
                0.011 +
                  userPressFactor * 0.009 +
                  userRiskFactor * 0.005 +
                  aiCounterQualityModifier +
                  userOvercommitIntoAiBlockFactor * 0.016
              )
            );
          }
        }
   ...
   // TUTAJ WSTAW TEN KOD - Logika Nasycenia (Satiety Logic)
        ...
        const userOvercommitLowBlockShotDrag =
          userOvercommitsIntoAiBlock && activeSide === userSide
            ? Math.min(0.034, 0.010 + userOvercommitIntoAiBlockFactor * 0.024)
            : 0;
        const aiLowBlockTransitionBonus =
          userOvercommitsIntoAiBlock && activeSide !== userSide
            ? Math.min(
                0.040,
                0.008 +
                  userOvercommitIntoAiBlockFactor * 0.020 +
                  (aiCounterAttackTriggered ? 0.014 : 0)
              )
            : 0;
        ...
        if (defendingSentOff > 0 && defendingTacticObj.attackBias <= 60) {
          const baseBoost = 0.006 + seededRng(currentSeed, rngMinute, 777) * 0.009;
        ...
        const rotationMismatchAttackBonus = LiveMatchPhysicalMismatchService.getRotationMismatchAttackBonus({
          ...
          defendingFatigue: defendingFatigueMap,
          rng: () => seededRng(currentSeed, rngMinute, 6201),
        });
        ...
        shotThreshold = Math.max(
          ...
          shotThreshold
            ...
            - userOvercommitLowBlockShotDrag
            + aiLowBlockTransitionBonus
            + noGkBonus
            + redCardDefensiveBoost
            + defendingInjuredSpineExposure
        ...
        if (nextMinute >= nextAiNextInstructionMinute || aiFormationChangedThisMinute) {
          ...
          if (decision) {
            ...
            nextAiActiveShout = { id: `ai_${rngMinute}`, ...decisionShout, expiryMinute: -1 };
          ...
          const randomDelayWindow = nextMinute >= 46 && nextMinute <= 75
            ...
            : userOvercommits
              ? 7
              : 11;
          const randomDelay = Math.floor(seededRng(currentSeed, rngMinute, 77) * randomDelayWindow);
        ...
        /**
         * Marking model note:
         * Marking is a defensive instruction, so it is applied from the team currently
         * defending against the active attack. The individual adjustment only exists for
         * the user's players for now because only the user can set per-player live marking.
         */
        ...
        if (activeSide === userSide) {
          ...
          userPhysicalActionSuppression = calculateLiveUserPhysicalActionSuppression({
            ...
            existingPhysicalActionPenalty:
              ...
              attackingInjuredSpineDrag +
              lateFatigueShotDrag,
            rng: () => seededRng(currentSeed, rngMinute, 6700 + userPhysicalRngOffset++),
          });
        }
         ...
         if (isHomeInj) {
          ...
          if (severity === InjurySeverity.SEVERE) {
            ...
            if (isUserTeam) {
              ...
              if (wantsOff || seededRng(currentSeed, rngMinute, 9901) < coachReadsRiskProb) priorityAiTrigger = true;
            }
          }
        };

        /**
         * Marking discipline note:
         * The foul branch treats activeSide as the side committing the defensive action,
         * so the marking discipline multiplier follows activeSide, not the attacking team.
         */
        ...
        if (shouldTriggerLiveFoul({ forceZeroShotChance, eventRoll: rngEvent, foulThreshold: uFoulThreshold })) {
           ...
           const isPenalty = seededRng(currentSeed, rngMinute, 1700) < calculateLivePenaltyIncidentChance({
             intensityPenaltyMultiplier: activeIntensityRisk.penalty * activeMarkingRisk.penaltyMultiplier,
           ...
           if (isPenalty) {
            ...
            const kickerXI = (attackingSide === 'HOME' ? nextHomeLineup.startingXI : nextAwayLineup.startingXI).filter(id => id !== null) as string[];
              ...
              const kicker = (_designatedPK ? kickerTeam.find(p => p.id === _designatedPK && kickerXI.includes(p.id)) : null) ?? GoalAttributionService.pickScorer(kickerTeam, kickerXI, false, () => seededRng(currentSeed, rngMinute, 1800));
              ...
              const refereeGivesPenalty = seededRng(currentSeed, rngMinute, 1710) < penaltyCallChance;

              nextIsPausedForEvent = true;
              if (refereeGivesPenalty) {
                const reason: PenaltyReviewReason = chooseLivePenaltyReviewReason(seededRng(currentSeed, rngMinute, 1712));
                const usesVar = seededRng(currentSeed, rngMinute, 1714) < 0.48;
                ...
                const verdict: PenaltyReviewVerdict = resolveLivePenaltyReviewVerdict({
                  usesVar,
                  varRoll: seededRng(currentSeed, rngMinute, 1716),
                  varOverturnChance,
                });
                const baseCard = reason === 'FOUL'
                  ? DisciplineService.evaluateFoul(env.ref, player, nextPlayerYellowCards[pId] || 0, () => seededRng(currentSeed, rngMinute, 1718))
                  : resolveLiveHandballBaseCard(seededRng(currentSeed, rngMinute, 1720));
      ...
      const scorer = GoalAttributionService.pickScorer(team, xi as string[], false, () => seededRng(currentSeed, rngMinute, 700));
           ...
           const scorerBriefingFatigue = activeSide === userSide
            ...
            const isGoal = GoalAttributionService.checkShotSuccess(
             ...
             defs,
             false,
             () => seededRng(currentSeed, rngMinute, 750),
              false,
           ...
           if (isGoal) {
              ...
              const ownGoal = resolveLiveOwnGoal({
                ...
                rng: () => seededRng(currentSeed, rngMinute, 781 + openPlayOwnGoalRngStep++),
              });
              const goalInfo = { 
   ...
   if (activeSide === 'HOME') { 
              ...
              // Konwencja zapisu: HOME > 0, AWAY < 0 (jeden number w stanie).
              {
                ...
                if (_prevScoringScore < _prevOppScore) {
                  ...
                  const _boostDuration = 5 + Math.floor(seededRng(currentSeed, rngMinute, 9901) * 11);
              ...
              const failType = resolveLiveMissedShotOutcome({
                failRoll: seededRng(currentSeed, rngMinute, 780),
                shotAccuracyRoll: seededRng(currentSeed, rngMinute, 790),
          ...
          const statRng = seededRng(currentSeed, rngMinute, 910);
          ...
          const statPlayerId = statActiveIds[Math.floor(seededRng(currentSeed, rngMinute, 914) * statActiveIds.length)];
          ...
          if (statRng < chaosShotShare) {
            ...
            const isChaosGoal = seededRng(currentSeed, rngMinute, 920) < chaosGoalChance;
            const onTargetChance = 0.24 + seededRng(currentSeed, rngMinute, 916) * 0.16 + (strengthEdge * 0.04);
            const shotType = isChaosGoal || seededRng(currentSeed, rngMinute, 918) < onTargetChance
            ...
            if (isChaosGoal && statPlayer) {
              ...
              const chaosOwnGoal = resolveLiveOwnGoal({
                ...
                rng: () => seededRng(currentSeed, rngMinute, 930 + chaosOwnGoalRngStep++),
              });
              const goalInfo = {
              ...
              applyAiMentalitySurgeForGoal(activeSide);
            } else if (seededRng(currentSeed, rngMinute, 922) < 0.18) {
              newLog = {
            ...
            if (seededRng(currentSeed, rngMinute, 924) < 0.16) {
              newLog = {
       ...
       if (type === MatchEventType.CORNER) {
            ...
            if (seededRng(currentSeed, rngMinute, 3300) < _cornerShotChance) {
              ...
              if (headerScorer) {
                ...
                if (isHeaderGoal) {
                  ...
                  const cornerOwnGoal = resolveLiveOwnGoal({
                    ...
                    rng: () => seededRng(currentSeed, rngMinute, 3510 + cornerOwnGoalRngStep++),
                  });
                  if (activeSide === 'HOME') {
                    nextHomeScore++;
                    newHomeGoals.push({
          ...
          if (type === MatchEventType.FREE_KICK && seededRng(currentSeed, rngMinute, 5100) < 0.18) {
            ...
            if (fkTaker) {
              ...
              if (seededRng(currentSeed, rngMinute, 5200) < fkGoalProb) {
        ...
        const accidentalInjurySides: Array<{
        ...
        }> = [
          {
            ...
            instructionInjuryMod: userSide === 'HOME'
              ? userIntensityRisk.injury * coachCommands.user.injuryMultiplier
              : aiIntensityRisk.injury * coachCommands.ai.injuryMultiplier,
            rollOffset: 4500,
            pickOffset: 4700,
            severityOffset: 4800,
          },
        ...
        accidentalInjurySides.forEach(({ side, pressure, instructionInjuryMod, rollOffset, pickOffset, severityOffset }) => {
          ...
          if (seededRng(currentSeed, rngMinute, rollOffset) >= threshold) return;
          ...
          const victim = healthyOnPitch[Math.floor(seededRng(currentSeed, rngMinute, pickOffset) * healthyOnPitch.length)];
          const isSevere = seededRng(currentSeed, rngMinute, severityOffset) < 0.15;
          processInjury({
            minute: nextMinute,
            teamSide: side,
        ...
        staminaInjurySides.forEach(({ side, lineup, injuries, fatMap, pool }, sideIdx) => {
          lineup.forEach((id, slotIdx) => {
            ...
            if (seededRng(currentSeed, rngMinute, 5000 + sideIdx * 100 + slotIdx) < pStamina) {
              ...
              if (!p) return;
              const isSevere = seededRng(currentSeed, rngMinute, 5200 + sideIdx * 100 + slotIdx) < 0.22;
              processInjury({
                minute: nextMinute,
                teamSide: side,
                type: isSevere ? MatchEventType.INJURY_SEVERE : MatchEventType.INJURY_LIGHT,
     ...
     if (priorityAiTrigger) {
        ...
        if (userIndividualFatigueEntries.length > 0) {
          ...
          const uXIForFat = userSide === 'HOME' ? nextHomeLineup.startingXI : nextAwayLineup.startingXI;
          const uSubsUsed = userSide === 'HOME' ? nextSubsCountHome : nextSubsCountAway;
          const activeIds = new Set(uXIForFat.filter((id): id is string => id !== null));
          userIndividualFatigueEntries.forEach(([id, extra]) => {
            if (!activeIds.has(id)) return;
            const multiplier = extra > 0
              ? getLiveInstructionFatigueMultiplier(
                  nextMinute,
                  uInstr.tempo,
                  uInstr.intensity,
        ...
        if (aiTotalFatExtra !== 0) {
          ...
          const aiFatigueMultiplier = aiTotalFatExtra > 0 && nextAiActiveShout
            ? getLiveInstructionFatigueMultiplier(
                nextMinute,
                nextAiActiveShout.tempo,
                nextAiActiveShout.intensity,
          ...
          aiXIForFat.filter((id): id is string => id !== null).forEach(id => {
            aiFatTarget[id] = Math.min(100, Math.max(0, (aiFatTarget[id] ?? 100) - (aiTotalFatExtra * aiFatigueMultiplier)));
          });
        }
...
const summary: MatchSummary = {
  ...
  const homeClubLogo = getClubLogo(ctx.homeClub.id) ?? (ctx.homeClub.logoFile ? new URL(`../../Graphic/logo/${ctx.homeClub.logoFile}`, import.meta.url).href : null);
  const awayClubLogo = getClubLogo(ctx.awayClub.id) ?? (ctx.awayClub.logoFile ? new URL(`../../Graphic/logo/${ctx.awayClub.logoFile}`, import.meta.url).href : null);
...
<div className="flex-1 relative p-2 overflow-visible">
  ...
  >
    {/* Tło trawy */}
    <div 
      className="absolute inset-0 bg-[#0a4f32]"
    />
    ...
    {awayClubLogo && (
      <img
        src={awayClubLogo}
        alt=""
        className="absolute left-[5%] top-[5%] h-[34%] w-[38%] object-contain opacity-[0.06] pointer-events-none select-none"
        style={{ filter: 'grayscale(1) contrast(1.45) brightness(1.12) drop-shadow(0 0 14px rgba(255,255,255,0.16))' }}
        aria-hidden="true"
      />
    )}
    ...
    {homeClubLogo && (
      <img
        src={homeClubLogo}
        alt=""
        className="absolute bottom-[5%] right-[5%] h-[28%] w-[32%] object-contain opacity-[0.06] pointer-events-none select-none"
        style={{ filter: 'grayscale(1) contrast(1.45) brightness(1.12) drop-shadow(0 0 14px rgba(255,255,255,0.16))' }}
        aria-hidden="true"
      />
    )}
