...
export function LargerStands({ seatColor = '#1a3a6a' }: { seatColor?: string | string[] }) {
  return (
    <group>
      <StraightStand position={[0, 0, -48]}  rotation={[0, 0, 0]}          width={100} rows={16} seatColor={seatColor} />
      <StraightStand position={[0, 0, 48]}   rotation={[0, Math.PI, 0]}    width={100} rows={16} seatColor={seatColor} />
      <StraightStand position={[62, 0, 0]}   rotation={[0, -Math.PI / 2, 0]} width={72} rows={12} seatColor={seatColor} />
      <StraightStand position={[-62, 0, 0]}  rotation={[0, Math.PI / 2, 0]}  width={72} rows={12} seatColor={seatColor} />
      <BowlCorner center={[50, 0, -36]}  startAngle={-Math.PI / 2}    endAngle={0}             rowsStart={16} rowsEnd={12} seatColor={seatColor} />
      <BowlCorner center={[-50, 0, -36]} startAngle={Math.PI}         endAngle={Math.PI * 1.5} rowsStart={12} rowsEnd={16} seatColor={seatColor} />
      <BowlCorner center={[50, 0, 36]}   startAngle={0}               endAngle={Math.PI / 2}   rowsStart={12} rowsEnd={16} seatColor={seatColor} />
      <BowlCorner center={[-50, 0, 36]}  startAngle={Math.PI / 2}     endAngle={Math.PI}       rowsStart={16} rowsEnd={12} seatColor={seatColor} />
    </group>
  );
}
      ...
      {Array.from({ length: segments }).map((_, s) => {
        ...
        return (
          <group key={s} position={[x, 0, z]} rotation={[0, -angle - Math.PI / 2, 0]}>
            ...
            <group>
              {Array.from({ length: Math.ceil(avgRows) }).map((__, i) => (
                <mesh key={i} position={[0, i * 0.5 + 0.25, -i * 0.8]} castShadow receiveShadow>
                  <boxGeometry args={[width, 0.51, 0.81]} />
                  <meshStandardMaterial color="#222" />
