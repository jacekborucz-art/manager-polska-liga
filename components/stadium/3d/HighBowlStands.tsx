...
export function HighBowlStands({ seatColor = '#1a3a6a' }: { seatColor?: string | string[] }) {
  return (
    <group>
      ...
      <StraightStand position={[62, 0, 0]}   rotation={[0, -Math.PI / 2, 0]} width={72}  rows={24} seatColor={seatColor} />
      <StraightStand position={[-62, 0, 0]}  rotation={[0, Math.PI / 2, 0]}  width={72}  rows={24} seatColor={seatColor} />
      <BowlCorner center={[50, 0, -36]}  startAngle={-Math.PI / 2}    endAngle={0}             rows={24} seatColor={seatColor} />
      <BowlCorner center={[-50, 0, -36]} startAngle={Math.PI}         endAngle={Math.PI * 1.5} rows={24} seatColor={seatColor} />
      <BowlCorner center={[50, 0, 36]}   startAngle={0}               endAngle={Math.PI / 2}   rows={24} seatColor={seatColor} />
      <BowlCorner center={[-50, 0, 36]}  startAngle={Math.PI / 2}     endAngle={Math.PI}       rows={24} seatColor={seatColor} />
    </group>
  );
}
      ...
      {Array.from({ length: segments }).map((_, s) => {
        ...
        return (
          <group key={s} position={[x, 0, z]} rotation={[0, -angle - Math.PI / 2, 0]}>
            <mesh position={[0, (rows * 0.5 + 5) / 2, -(rows * 0.8 + 0.5)]} castShadow receiveShadow>
              <boxGeometry args={[width, rows * 0.5 + 5, 1.2]} />
              <meshStandardMaterial color="#2a2a2a" />
            </mesh>
            <group>
