...
type LoanPositionFilter = 'ALL' | PlayerPosition;
...
const _persisted = {
  ...
  } as typeof initialLoanFilters,
  loanPositionFilter: 'ALL' as LoanPositionFilter,
...
const JobMarketContent: React.FC<{ onOpenScouting: () => void }> = ({ onOpenScouting }) => {
  ...
  const loanListedPlayers = useMemo(() => {
    ...
    list = list.filter(p =>
      p.age >= appliedLoanFilters.age.min &&
      p.age <= appliedLoanFilters.age.max &&
      p.overallRating >= appliedLoanFilters.overall.min &&
      p.overallRating <= appliedLoanFilters.overall.max &&
      (loanPositionFilter === 'ALL' || p.position === loanPositionFilter)
    );
    list = list.filter(p => (p.marketValue || 0) >= priceFilter.min && (p.marketValue || 0) <= priceFilter.max);
    if (marketOriginFilter === 'LOCAL') list = list.filter(p => clubById.get(p.clubId)?.leagueId?.startsWith('L_PL_'));
    if (marketOriginFilter === 'FOREIGN') list = list.filter(p => !clubById.get(p.clubId)?.leagueId?.startsWith('L_PL_'));
    return list.sort((a, b) => b.overallRating - a.overallRating);
  }, [visibleMarketPlayers, userTeamId, searchTermPlayers, posFilter, filters, playerNationalityFilter, searchTermTransfer, appliedLoanFilters, loanPositionFilter, priceFilter, marketOriginFilter, clubById]);
