...
export const CLQFDrawView: React.FC = () => {
  const { clubs, fixtures, confirmCLQFDraw, userTeamId, sessionSeed } = useGame();

  const getClub = (id: string) => clubs.find(c => c.id === id);

  // Wyznacz 8 zwycięzców R16
  const r16Winners = useMemo(() => CLDrawService.getR16Winners(fixtures), [fixtures]);

  // Losuj pary tym samym algorytmem co generateQFFixtures (ten sam seed)
  const pairs = useMemo(() => {
      ...
      <div className="relative z-10 flex flex-col h-full p-6 gap-4">
        ...
        <div className={GLASS_CARD + " p-6 flex items-center justify-between shrink-0"}>
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <p className="text-orange-400 text-[9px] font-black uppercase tracking-[0.5em]">UEFA Champions League</p>
        ...
        <div className={GLASS_CARD + " flex-1 overflow-y-auto p-6"}>
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            <p className="text-[9px] font-black uppercase tracking-[0.4em] text-slate-500 mb-5 text-center">
              4 pary · 1. mecz: 16 lutego · Rewanż: 2 marca
            </p>
