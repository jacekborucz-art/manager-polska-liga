...
export const StartMenu: React.FC = () => {
  ...
  const handleFullPackLoad = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    try {
      const result = await runWithProcessing(async () => {
        const raw = JSON.parse(await file.text());
        return importEditorFullPack(raw);
      }, {
        title: 'Ładowanie datapacka',
        message: 'System wczytuje kluby, zawodników, kontrakty i dane rozgrywek z wybranego pliku.',
