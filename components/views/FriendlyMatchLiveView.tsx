...
const nextMomentumSum = prev.momentumSum + prev.momentum;
  ...
  const nextLiveStats = { ...prev.liveStats };
        ...
        if (nextMinute > limit) {
           // Sparingi: logika dogrywki/karnych z warunków meczu
           const drawRule = activeFriendlyConditions?.drawRule ?? 'NONE';
           const isDraw = prev.homeScore === prev.awayScore;
           let needsET = false;
           let needsPens = false;
           if (isDraw && prev.period === 2) {
             needsET = (drawRule === 'ET_ONLY' || drawRule === 'ET_THEN_PENALTIES');
             needsPens = (drawRule === 'PENALTIES');
           }
           if (isDraw && prev.period === 4) {
             needsPens = (drawRule === 'ET_THEN_PENALTIES');
           }
           const logText = (prev.period === 1 || prev.period === 3) ? "Przerwa w grze."
             : needsET ? "Remis! Dogrywka!"
             : needsPens ? "Remis! Rzuty karne!"
             : "Sędzia kończy mecz!";
...
const applyHalftimeRegen = (fatigueMap: Record<string, number>, playersList: Player[]) => {
      ...
      nextFatigue[pId] = Math.min(cap, (nextFatigue[pId] || 100) + regenAmount);
           ...
           // --- HALFTIME AI DECISIONS (SPARING: masowa rotacja 5-7 zmian) ---
           if (prev.period === 1 || prev.period === 3) {
              const htMinute = prev.period === 1 ? 45 : 105;
              const aiSide: 'HOME' | 'AWAY' = userSide === 'HOME' ? 'AWAY' : 'HOME';
              const friendlyMaxSubs = activeFriendlyConditions?.maxSubstitutions ?? 5;
              const targetHtSubs = Math.floor(Math.random() * 3) + 5; // 5, 6 lub 7 zmian

              for (let htAttempt = 0; htAttempt < targetHtSubs; htAttempt++) {
                const htState = {
                  ...prev,
                  minute: htMinute,
                ...
                const currentAiSubs = aiSide === 'HOME' ? nextSubsCountHome : nextSubsCountAway;
                if (currentAiSubs >= friendlyMaxSubs) break;

                const decision = AiMatchDecisionServiceFriendlyMatch.makeDecisions(
                  htState, ctx, aiSide, true, true, friendlyMaxSubs
                );

                if (!decision.subRecord) break; // brak kandydatów — koniec rotacji
 ...
 const recoveredHomeFatigue = (prev.period === 1 || prev.period === 3) ? applyHalftimeRegen(prev.homeFatigue, ctx.homePlayers) : prev.homeFatigue;
       ...
       const engineComment = MatchEngineService.generateCommentary(nextMinute, currentSeed, ctx.homeClub.name, ctx.awayClub.name);
        ...
        if (nextMinute % 5 === 0 || immediateAiTrigger) {
           const decision = AiMatchDecisionServiceFriendlyMatch.makeDecisions(
             ...
             immediateAiTrigger,
             false,
             activeFriendlyConditions?.maxSubstitutions ?? 5
           );
   ...
   // TUTAJ WSTAW TEN KOD - Logika Nasycenia (Satiety Logic)
        ...
        const coachTurnoverRisk = activeSide === userSide
          ? coachCommands.user.turnoverRiskModifier
          : coachCommands.ai.turnoverRiskModifier;
        shotThreshold -= Math.max(0, Math.min(1, activeBuildUpProfile.turnoverRisk + coachTurnoverRisk)) *
          (opponentPressingNow ? 0.006 : 0.002);
        ...
        if (!forceZeroShotChance && rngEvent < uFoulThreshold) {
           ...
           const isPenalty = seededRng(currentSeed, nextMinute, 1700) < (0.0956 * activeIntensityRisk.penalty);
           ...
           if (isPenalty) {
            ...
            const kickerXI = (attackingSide === 'HOME' ? nextHomeLineup.startingXI : nextAwayLineup.startingXI).filter(id => id !== null) as string[];
              ...
              const injury = InjuryEventGenerator.maybeGenerateInjury(ctx, prev, { minute: nextMinute, teamSide: attackingSide, type: MatchEventType.PENALTY_AWARDED, text: '' } as MatchEvent, () => seededRng(currentSeed, nextMinute, 2000), attackingSide === userSide ? coachCommands.user.injuryMultiplier : coachCommands.ai.injuryMultiplier);
              ...
              if (newLog) {
                ...
                const injury = InjuryEventGenerator.maybeGenerateInjury(ctx, prev, { minute: nextMinute, teamSide: activeSide, type: MatchEventType.FOUL, text: '' } as MatchEvent, () => seededRng(currentSeed, nextMinute, 2000), injuredSide === userSide ? coachCommands.user.injuryMultiplier : coachCommands.ai.injuryMultiplier);
                if (injury) processInjury(injury);
              }
           }
        } 
   ...
   if (activeSide === 'HOME') { 
           ...
           if (newLog) {
             const injury = InjuryEventGenerator.maybeGenerateInjury(ctx, prev, { minute: nextMinute, teamSide: activeSide, type: immediateEventType, primaryPlayerId: scorer.id, text: '' } as MatchEvent, () => seededRng(currentSeed, nextMinute, 2500), activeSide === userSide ? coachCommands.user.injuryMultiplier : coachCommands.ai.injuryMultiplier);
             if (injury) processInjury(injury);
           }
        }
       ...
       if (type === MatchEventType.CORNER) {
        ...
        const autoRemoveInjured = (lineup: (string | null)[], injuries: Record<string, InjurySeverity>, side: 'HOME' | 'AWAY') => {
          // Zasada sparing: kontuzja krytyczna = zmiana awaryjna zawsze możliwa.
          // Gracz decyduje sam przez modal; AI uzupełnia luki przez PRIORYTET 2.
          if (side === userSide) return lineup;
          ...
          return lineup.map(id => {
            if (id && injuries[id] === InjurySeverity.SEVERE) {
               ...
               carriedOffLogs.push({
                 ...
                 minute: nextMinute,
                 text: `🚨 ${p?.lastName} zostaje zniesiony na noszach!`,
...
return {
 ...
 const handleFinishMatch = () => {
    ...
    // Obliczamy frekwencję — dla sparingów mała liczba kibiców (20–500)
    const attendance = Math.floor(20 + Math.random() * 480);
 ...
 const hasScored = matchState.awayGoals.some(g => g.playerName === p.lastName && !g.isMiss && !g.varDisallowed);
  ...
  {matchState.isFinished ? (
    ...
    {matchState.logs.length > 0 && (
      ...
      <MatchTacticsModal isOpen={isTacticsOpen} onClose={handleTacticsClose} club={userSide === 'HOME' ? ctx.homeClub : ctx.awayClub} lineup={userSide === 'HOME' ? matchState.homeLineup : matchState.awayLineup} players={userSide === 'HOME' ? ctx.homePlayers : ctx.awayPlayers} fatigue={userSide === 'HOME' ? matchState.homeFatigue : matchState.awayFatigue} subsCount={userSide === 'HOME' ? matchState.subsCountHome : matchState.subsCountAway} subsHistory={userSide === 'HOME' ? matchState.homeSubsHistory : matchState.awaySubsHistory} minute={matchState.minute} sentOffIds={matchState.sentOffIds} injs={userSide === 'HOME' ? matchState.homeInjuries : matchState.awayInjuries} maxSubs={activeFriendlyConditions?.maxSubstitutions ?? 5} injuryEmergencyMode={hasMandatorySub} />
