...
export const ELFinalDrawView: React.FC = () => {
  const { clubs, fixtures, userTeamId, navigateTo, advanceDay, confirmELFinalDraw } = useGame();

  const getClub = (id: string) => clubs.find(c => c.id === id);

  const finalists = useMemo(() => ELDrawService.getSFWinners(fixtures), [fixtures]);
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs mt-1">
                Para finałowa wyłoniona · Mecz jednorazowy · 20 maja
              </p>
            </div>
          </div>
          <button
            onClick={confirmELFinalDraw}
        ...
        <div className={GLASS_CARD + " flex-1 flex flex-col items-center justify-center p-8 gap-8"}>
          ...
          <div className="relative z-10 w-full max-w-2xl flex flex-col gap-6">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.5em] text-slate-500 text-center">
              Finał Ligi Europy · 20 maja
            </p>

            {userIsFinalist && (
              <div className="text-center">
                <span className="inline-block px-5 py-2 rounded-full bg-orange-500/10 border border-orange-500/30 text-orange-400 text-[10px] font-black uppercase tracking-[0.4em]">
